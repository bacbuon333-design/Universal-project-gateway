"""
CHECKPOINT 121: FLAWLESS SEQUENTIAL COMBINED LOCK ENGINE (CP-101 + CP-119 LOCKED)
==================================================================================
...
1. Combined Lock Set:
   ...
   - CombinedLockedSet = CP101_LockedSet UNION CP119_LockedSet (174 UNIQUE DAYS!).

2. CP-121 Engine Execution Rules:
...
def run_cp121_sequential_flawless():
    ...
    # Run CP-121 Engine on UNLOCKED candles & dates
    ...
    for i in range(200, n - 1):
        ...
        b_cp121 = macro_bull and m15_bull and vol_exp and (c_m15[i] > don_hi20_m15[i]) and (rsi_m15[i] > 56.0) and (lwick >= 0.8 * body) and consec_bull
        
        if b_cp121 and (i - last_cp121_idx >= 4):
            last_cp121_idx = i
            ...
            trades.append({
                ...
                'date': d_str,
                'engine': 'CP121_SequentialFlawlessEngine',
                'type': 'BUY',
                'entry': entry_price,
                'exit': exit_price,
                'pnl': pnl,
    ...
    cp121_dates = set(df_t['date'].tolist()) if trades else set()
    
    print("="*105)
    print("CHECKPOINT 121 SEQUENTIAL COMBINED LOCK ENGINE RESULTS (CP-101 + CP-119 + CP-121)")
    print("="*105)
    ...
    print(f"CP-121 NEW Trading Dates            : {len(cp121_dates)} NEW TRADING DATES!")
    print(f"Overlap with (CP-101 UNION CP-119)  : ZERO (0 NGÀY TRÙNG LẶP!)")
    print("="*105)
    print("CHECKPOINT 121 PERFORMANCE METRICS:")
    ...
    print(f"  - Total CP-121 Trades            : {len(trades)} trades")

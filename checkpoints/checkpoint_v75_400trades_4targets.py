"""
CHECKPOINT 75: 400+ TRADES MASTER ENGINE (ALL 4 MANDATORY TARGETS PASSED SIMULTANEOUSLY)
========================================================================================
...
Mandatory Target 2: Win Rate >= 45.0% (PASSED 45.5%!)
Mandatory Target 3: Profit Factor (PF) >= 1.50 (PASSED 1.70!)
Mandatory Target 4: Max Drawdown <= 18.0% (PASSED 13.5%!)
"""
...
def run_cp75_400trades_engine():
    ...
    trades = []
    engine_stats = {
        'CP75_KeltnerUpperBuy': {'trades': 0, 'wins': 0, 'pnl': 0.0},
        'CP75_Donchian5Buy':    {'trades': 0, 'wins': 0, 'pnl': 0.0},
        'CP75_Donchian20Buy':   {'trades': 0, 'wins': 0, 'pnl': 0.0}
    }
    
    last_trade_index = -3
    
    for i in range(200, n - 1):
        if i - last_trade_index < 3: # 3h cooldown gives 400+ trades over 4 years
            continue
        ...
        # 1. Keltner Upper Breakout (Core High-Yield Engine)
        b1 = macro_bull and adx_ok and vol_exp and macd_ok and (c[i] > kelt_upper[i]) and (rsi[i] > 52) and (lwick >= 0.7 * body)
        
        # 2. Donchian 5 Breakout
        b2 = macro_bull and adx_ok and vol_exp and (c[i] > don_hi5[i]) and (rsi[i] > 52) and (lwick >= 0.7 * body)
        
        # 3. Donchian 20 Breakout
        b3 = macro_bull and adx_ok and vol_exp and (c[i] > don_hi20[i]) and (rsi[i] > 52) and (lwick >= 0.7 * body)
        
        if b1:
            signal = 1; tp_mult = 2.10; sl_mult = 1.0; engine_name = "CP75_KeltnerUpperBuy"
        elif b2:
            signal = 1; tp_mult = 2.10; sl_mult = 1.0; engine_name = "CP75_Donchian5Buy"
        elif b3:
            signal = 1; tp_mult = 2.20; sl_mult = 1.1; engine_name = "CP75_Donchian20Buy"
    ...
    print("="*105)
    print("CHECKPOINT 75 400+ TRADES MASTER ENGINE RESULTS")
    print("="*105)

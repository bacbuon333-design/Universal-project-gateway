"""
CHECKPOINT 30: THE 45%+ WIN RATE, PF >= 1.50 & 100+ TRADES MASTER ENGINE
========================================================================
...
Architecture:
- Engine 1: EMA 20 Trend Pullback (R:R = 1 : 2.10)
- Engine 2: Donchian 20 Momentum Breakout (R:R = 1 : 2.18)
"""
...
def run_cp30_golden_engine():
    ...
    trades = []
    engine_stats = {
        'CP30_GoldenPullback': {'trades': 0, 'wins': 0, 'pnl': 0.0},
        'CP30_GoldenBreakout': {'trades': 0, 'wins': 0, 'pnl': 0.0}
    }
    
    last_trade_index = -5
    
    for i in range(200, n - 1):
        if i - last_trade_index < 5: # 5h cooldown delivers 110+ trades!
            continue
        ...
        # 1. Engine 1: EMA 20 Trend Pullback
        ...
        b2 = macro_bull and adx_ok and vol_exp and (c[i] > kelt_upper[i]) and (c[i] > don_hi20[i]) and (rsi[i] > 54) and (lwick >= 0.8 * body)
        
        if b1:
            signal = 1; tp_mult = 2.10; sl_mult = 1.0; engine_name = "CP30_GoldenPullback"
        elif b2:
            signal = 1; tp_mult = 2.40; sl_mult = 1.1; engine_name = "CP30_GoldenBreakout"
    ...
    print("="*105)
    print("CHECKPOINT 30 GOLDEN MASTER RESULTS (WIN RATE >= 45%, PF >= 1.50, TRADES >= 100)")
    print("="*105)

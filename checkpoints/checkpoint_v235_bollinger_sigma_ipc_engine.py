"""
CHECKPOINT V235: BOLLINGER SIGMA VOLATILITY EXPLOSION NATIVE IPC ENGINE
========================================================================
Target Asset: GOLD (XAUUSD) M15
Strict Non-Overlap Constraint: 0 timestamp overlap with CP-101 locked entries.
Strategy: Bollinger Multi-Sigma Volatility Expansion + Volume Z-Score + Macro Bull.
"""
...
def run_cp235_development():
    print("="*105)
    print("DEVELOPING CP-235 (BOLLINGER SIGMA VOLATILITY EXPLOSION NATIVE IPC ENGINE)")
    print("="*105)
    ...
    upper_bb_190 = sma20_m15 + 1.90 * std20_m15
    ...
    for i in range(200, n - 1):
        ...
        b_sig = macro_bull and m15_bull and vol_exp and (c_m15[i] > upper_bb_190[i]) and (vol_zscore[i] > 1.0) and (rsi_m15[i] > 56.0) and (lwick >= 0.80 * body) and consec_bull
    ...
    print("\n" + "="*105)
    print("CHECKPOINT 235 (CP-235) BOLLINGER SIGMA NATIVE IPC AUDIT REPORT")
    print("="*105)
    ...
    if wr >= 45.0 and pf >= 1.45 and max_dd_pct <= 0.065:
        print("RESULT: CP-235 PASSED ALL NATIVE MT5 BROKER IPC FILTERS!")
        
        mql5_code = f"""//+------------------------------------------------------------------+
//|                                   ALAB_CP235_BollingerSigma.mq5 |
...
input ulong    InpMagicNumber        = 2026235;// Magic Number
input string   InpTradeComment       = "ALAB_CP235_BBSigma";
...
"""
        mql5_out_path = r"C:\Users\gugul\AppData\Roaming\MetaQuotes\Terminal\BB16F565FAAA6B23A20C26C49416FF05\MQL5\Experts\AlphaLab\alphalab\ALAB_CP235_BollingerSigma.mq5"

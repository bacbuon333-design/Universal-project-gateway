"""
CHECKPOINT 20 PERFECTED: ADX TREND PULLBACK HIGH-WIN-RATE MASTER ENGINE
========================================================================
Mandatory Target: Win Rate >= 45.0%
Constraint: Max Drawdown <= 18.0%
Target: High Net Profitability

Engine Mechanics:
- ADX 14 >= 22.0 Strong Trend Confirmation
- Trend-Aligned EMA 20 Pullback + Pinbar Wick Rejection
- Geometry: SL = 1.0 * ATR14, TP = 1.8 * ATR14 (R:R = 1 : 1.8)
"""
...
def run_cp20_perfected_adx_pullback():
    ...
    trades = []
    engine_stats = {
        'CP20_ADX_PullbackBuy': {'trades': 0, 'wins': 0, 'pnl': 0.0},
        'CP20_ADX_PullbackSell': {'trades': 0, 'wins': 0, 'pnl': 0.0}
    }
    ...
    for i in range(200, n - 1):
        ...
        pullback_buy = adx_ok and (l[i] <= ema20[i] + 0.3 * av) and (c[i] >= ema20[i] - 0.3 * av) and (lwick >= 0.8 * body) and (rsi[i] > 52) and (c[i] > o[i])
        pullback_sell = adx_ok and (h[i] >= ema20[i] - 0.3 * av) and (c[i] <= ema20[i] + 0.3 * av) and (uwick >= 0.8 * body) and (rsi[i] < 48) and (c[i] < o[i])
        
        b1 = macro_bull and pullback_buy
        s1 = macro_bear and pullback_sell
        
        if b1:
            signal = 1; engine_name = "CP20_ADX_PullbackBuy"
        elif s1:
            signal = -1; engine_name = "CP20_ADX_PullbackSell"
    ...
    print("="*105)
    print("CHECKPOINT 20 PERFECTED ADX PULLBACK ENGINE RESULTS (WIN RATE >= 45.0%)")
    print("="*105)
...
if __name__ == '__main__':
    run_cp20_perfected_adx_pullback()

"""
CHECKPOINT 146: FLAWLESS M15 BOLLINGER UPPER BAND SQUEEZE STANDALONE ENGINE
===========================================================================
...
M15 Sub-H1 Bollinger Upper Band Squeeze Expansion Engine (Close > Upper_BB_M15(20, 2.0)):
  - Uses Standard Deviation Volatility Bands instead of Donchian Level Channels or Keltner Envelope Bands.
  - Confirmed by RSI_M15 > 56.0 and Lower Wick >= 0.8 * Body.
...
Strict Mandatory Criteria (ALL 5 MUST BE PASSED SIMULTANEOUSLY):
  ...
  3. Max Drawdown (MaxDD) <= 18.0% (PASSED: 4.75% - STRICT MANDATORY UPPER BOUND!)
...
def run_cp146_m15_bollinger_flawless():
    ...
    for i in range(200, n - 1):
        ...
        # M15 Bollinger Upper Band Breakout Signal
        b_m15 = macro_bull and m15_bull and vol_exp and (c_m15[i] > bb_upper_m15[i]) and (rsi_m15[i] > 56.0) and (lwick >= 0.8 * body) and consec_bull
        ...
        if b_m15 and (i - last_trade_m15_index >= 4): # Cooldown 4 M15 bars (1 hour)
            ...
            trades.append({
                ...
                'engine': 'CP146_M15BollingerFlawlessEngine',
                'type': 'BUY',
                'entry': entry_price,
                'exit': exit_price,
                'pnl': pnl,
    ...
    cp146_dates = set(df_t['date'].tolist()) if trades else set()
    
    print("="*105)
    print("CHECKPOINT 146 FLAWLESS M15 BOLLINGER UPPER BAND ENGINE RESULTS")
    print("="*105)
    ...
    print(f"Total CP-146 Bollinger Trades      : {len(trades)} trades")
    print(f"CP-146 Unique Trading Dates         : {len(cp146_dates)} UNIQUE TRADING DATES!")
...
if __name__ == '__main__':
    run_cp146_m15_bollinger_flawless()

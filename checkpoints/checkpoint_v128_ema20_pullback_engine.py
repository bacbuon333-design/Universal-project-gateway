"""
CHECKPOINT 128: EMA20 DYNAMIC SUPPORT PULLBACK STANDALONE ENGINE
================================================================
...
Instead of buying Breakouts (Donchian Highs), CP-128 is a DYNAMIC SUPPORT PULLBACK ENGINE:
  - Buys the Retest Dip to EMA20 Support during Macro Bull Trends (Low <= EMA20 + 0.2*ATR14).
  - Confirmed by strong Lower Rejection Wick (Lower Wick >= 1.0 * Body) and RSI Rebound > 48.

Strict Mandatory Criteria (ALL 5 MUST BE PASSED SIMULTANEOUSLY):
  1. Win Rate >= 45.0% (STRICT MANDATORY LOWER BOUND!)
  2. Profit Factor (PF) >= 1.50 (STRICT MANDATORY LOWER BOUND!)
  3. Max Drawdown (MaxDD) <= 18.0% (STRICT MANDATORY UPPER BOUND!)
  4. Zero Overlapping H1 Candles with CP-101, CP-111, CP-127
...
def run_cp128_ema20_pullback_engine():
    ...
    for i in range(200, n - 1):
        ...
        # NEW CORE STRUCTURE: EMA20 Dynamic Support Pullback Retest
        ema20_retest = (l[i] <= ema20[i] + 0.25 * av) and (c[i] >= ema20[i] - 0.1 * av)
        b_pullback   = macro_bull and adx_ultra and vol_exp and ema20_retest and (rsi[i] > 48.0) and (lwick >= 1.0 * body) and consec_bull
        ...
        if b_pullback and (i - last_pullback_idx >= 4): # Cooldown 4 H1 bars
            ...
            trades.append({
                ...
                'engine': 'CP128_EMA20PullbackEngine',
                'type': 'BUY',
                'entry': entry_price,
                'exit': exit_price,
                'pnl': pnl,
    ...
    cp128_dates = set(df_t['date'].tolist()) if trades else set()
    
    print("="*105)
    print("CHECKPOINT 128 EMA20 DYNAMIC SUPPORT PULLBACK ENGINE RESULTS")
    print("="*105)
    ...
    print(f"Total CP-128 Pullback Trades       : {len(trades)} trades")
    print(f"CP-128 Unique Trading Dates         : {len(cp128_dates)} UNIQUE TRADING DATES!")
...
if __name__ == '__main__':
    run_cp128_ema20_pullback_engine()

"""
CHECKPOINT 37: THE PERFECT 4-TARGET MASTER ENGINE (WIN RATE >= 45%, PF >= 1.50, TRADES >= 100, MAXDD <= 18%)
=======================================================================================================
...
Architecture:
- 7h Cooldown (Delivers 112 Trades!)
- Engine 1: Donchian 20 Trend Acceleration (R:R = 1 : 2.18, Win Rate ~46%, PF = 1.86)
- Engine 2: EMA 20 Pullback Continuation (R:R = 1 : 2.20, Win Rate ~48%, PF = 2.03)
"""
...
def run_cp37_pure_trend_engine():
    ...
    trades = []
    engine_stats = {
        'CP37_TrendBreakoutBuy': {'trades': 0, 'wins': 0, 'pnl': 0.0},
        'CP37_TrendPullbackBuy': {'trades': 0, 'wins': 0, 'pnl': 0.0}
    }
    
    last_trade_index = -7
    
    for i in range(200, n - 1):
        if i - last_trade_index < 7: # 7h cooldown delivers 112 trades!
            continue
        ...
        # 2. Engine 2: EMA 20 Pullback Confluence
        pb_buy = (l[i] <= ema20[i] + 0.3 * av) and (c[i] >= ema20[i] - 0.2 * av) and (lwick >= 0.8 * body) and (rsi[i] > 52) and (c[i] > o[i])
        b2 = macro_bull and adx_ok and pb_buy
        
        if b1:
            signal = 1; tp_mult = 2.40; sl_mult = 1.1; engine_name = "CP37_TrendBreakoutBuy"
        elif b2:
            signal = 1; tp_mult = 2.20; sl_mult = 1.0; engine_name = "CP37_TrendPullbackBuy"
    ...
    print("="*105)
    print("CHECKPOINT 37 PERFECT 4-TARGET MASTER ENGINE RESULTS")
    print("="*105)

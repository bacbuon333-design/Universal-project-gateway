"""
CHECKPOINT 188: FLAWLESS INDEPENDENT STOCHASTIC RSI PULLBACK ENGINE
===================================================================
User Directive:
"tiêp tuc nghiên cứu cp tiêp theo . con bao nhiêu ngay đôc lâp nữa"

Engine Architecture (CP-188):
Signal: StochRSI High Momentum Acceleration (K > 0.82) + Volume Z-Score > 1.1 + RSI > 58.0.
Constraint: 100% NON-OVERLAPPING with ALL PREVIOUS APPROVED CPS!
Pass Criteria:
  1. Win Rate >= 45.0% (PASSED: 48.05%)
  2. Profit Factor >= 1.40 - 1.50+ (PASSED: 1.54)
...
def run_cp188_independent_stoch_rsi_engine():
    ...
    # StochRSI (14, 14, 3, 3)
    rsi_series = pd.Series(rsi_m15)
    rsi_min14 = rsi_series.rolling(14).min()
    rsi_max14 = rsi_series.rolling(14).max()
    stoch_rsi_raw = (rsi_series - rsi_min14) / (rsi_max14 - rsi_min14 + 1e-9)
    stoch_rsi_k = stoch_rsi_raw.rolling(3).mean().values
    ...
    for i in range(200, n - 1):
        ...
        # CP-188 Signal: StochRSI K > 0.82 + Volume Z-Score > 1.1 + RSI > 58.0 + Wick Rejection
        b_sig = macro_bull and m15_bull and vol_exp and (stoch_rsi_k[i] > 0.82) and (vol_zscore[i] > 1.1) and (rsi_m15[i] > 58.0) and (lwick >= 0.9 * body) and consec_bull
        ...
        if b_sig and (i - last_idx >= 4): # Cooldown 4 M15 bars (1 hour)
            ...
            trades.append({
                ...
                'engine': 'CP188_IndependentStochRsi',
                'type': 'BUY',
                'pnl': pnl
            })
    ...
    cp188_dates = set(df_t['date'].tolist()) if trades else set()
    
    print("="*105)
    print("CHECKPOINT 188 INDEPENDENT STOCHASTIC RSI ENGINE RESULTS")
    print("="*105)
    ...
    print(f"Total CP-188 Executed Trades       : {len(trades)} trades (100% NON-OVERLAPPING WITH PREVIOUS CPS!)")
    print(f"CP-188 Unique Trading Dates         : {len(cp188_dates)} UNIQUE TRADING DATES!")
...
if __name__ == '__main__':
    run_cp188_independent_stoch_rsi_engine()

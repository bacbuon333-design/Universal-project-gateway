"""
CHECKPOINT V220: PULLBACK REJECTION NATIVE IPC BROKER ENGINE
============================================================
Target Asset: GOLD (XAUUSD) M15
Strict Non-Overlap Constraint: 0 timestamp overlap with CP-101 locked entries.
Strategy: Buys high-volume rejection wicks at EMA20 support during strong H1 macro bull trends.
          Avoids breakout top-buying, eliminating spread degradation on XMGlobal.
"""
...
def run_cp220_development():
    print("="*105)
    print("DEVELOPING CP-220 (PULLBACK REJECTION NATIVE IPC BROKER ENGINE)")
    print("="*105)
    ...
    for i in range(200, n - 1):
        ...
        # Pullback rejection signal: low touched below EMA20_m15, close closed back above EMA20_m15 with rejection wick
        touched_ema20 = (l_m15[i] <= ema20_m15[i]) and (c_m15[i] > ema20_m15[i])
        strong_wick   = (lwick >= 1.2 * body)
        
        b_sig = macro_bull and m15_bull and vol_exp and touched_ema20 and strong_wick and (vol_zscore[i] > 0.8) and (rsi_m15[i] > 50.0 and rsi_m15[i] < 68.0)
    ...
    print("\n" + "="*105)
    print("CHECKPOINT 220 (CP-220) PULLBACK REJECTION NATIVE IPC AUDIT REPORT")
    print("="*105)
    ...
    if wr >= 45.0 and pf >= 1.45 and max_dd_pct <= 0.065:
        print("RESULT: CP-220 PASSED ALL NATIVE MT5 BROKER IPC FILTERS!")
        
        mql5_code = f"""//+------------------------------------------------------------------+
//|                                ALAB_CP220_PullbackRejection.mq5 |
...
input ulong    InpMagicNumber        = 2026220;// Magic Number
input string   InpTradeComment       = "ALAB_CP220_Pullback";
...
{{
   ...
   if(lwick < 1.2 * body) return;
   
   double ema20_m15 = iMA(_Symbol, _Period, 20, 0, MODE_EMA, PRICE_CLOSE);
   if(low > ema20_m15 || close <= ema20_m15) return;
   ...
   double sl_dist = atr14 * 0.90 + 0.25;
   double tp_dist = atr14 * 2.20;
...
"""
        mql5_out_path = r"C:\Users\gugul\AppData\Roaming\MetaQuotes\Terminal\BB16F565FAAA6B23A20C26C49416FF05\MQL5\Experts\AlphaLab\alphalab\ALAB_CP220_PullbackRejection.mq5"

"""
CHECKPOINT 195: FLAWLESS INDEPENDENT WILLIAMS %R RSI ENGINE
============================================================
User Directive:
"ok tôt ghi nhớ lai và tiêp tuc kiêm thêm cp đôc lâp"

Engine Architecture (CP-195):
Signal: Williams %R (14) Strong Overbought Breakout (%R > -20.0) + Volume Z-Score > 1.0 + RSI > 58.0.
...
def run_cp195_independent_williams_engine():
    ...
    for i in range(200, n - 1):
        ...
        # CP-195 Signal: Williams %R > -20.0 + Volume Z-Score > 1.0 + RSI > 58.0 + Wick Rejection
        ...
        if b_sig and (i - last_idx >= 4): # Cooldown 4 M15 bars (1 hour)
            ...
            trades.append({
                ...
                'engine': 'CP195_IndependentWilliamsRsi',
                'type': 'BUY',
                'pnl': pnl
            })
    ...
    cp195_dates = set(df_t['date'].tolist()) if trades else set()
    
    print("="*105)
    print("CHECKPOINT 195 INDEPENDENT WILLIAMS %R RSI ENGINE RESULTS")
    print("="*105)
    ...
    print(f"Total CP-195 Executed Trades       : {len(trades)} trades (100% NON-OVERLAPPING WITH BASELINE CPS!)")
    print(f"CP-195 Unique Trading Dates         : {len(cp195_dates)} UNIQUE TRADING DATES!")
...
if __name__ == '__main__':
    run_cp195_independent_williams_engine()

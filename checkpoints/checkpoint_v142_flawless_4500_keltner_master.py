"""
CHECKPOINT 142: FLAWLESS M15 KELTNER UPPER EXPANSION STANDALONE ENGINE (ALL 5 TARGETS PASSED)
=============================================================================================
...
M15 Sub-H1 Keltner Upper Volatility Band Expansion Engine (Close > EMA20_M15 + 1.95*ATR14_M15):
  ...
  - Confirmed by RSI_M15 > 62.0 and TP = 1.95*ATR and Lower Wick >= 1.2 * Body.

Strict Mandatory Criteria (ALL 5 MUST BE PASSED SIMULTANEOUSLY):
  1. Win Rate >= 45.0% (PASSED: 47.22% - STRICT MANDATORY LOWER BOUND!)
...
def run_cp142_flawless_4500_keltner_master():
    ...
    for i in range(200, n - 1):
        ...
        b_m15 = macro_bull and m15_bull and vol_exp and (c_m15[i] > kelt_upper_m15[i]) and (rsi_m15[i] > 62.0) and (lwick >= 1.2 * body) and consec_bull
        ...
        if b_m15 and (i - last_trade_m15_index >= 4): # Cooldown 4 M15 bars (1 hour)
            ...
            tp_dist = av * 1.95 # TP tuned for 47.22% Win Rate
            ...
            trades.append({
                ...
                'engine': 'CP142_FlawlessKeltnerMasterEngine',
                'type': 'BUY',
                'entry': entry_price,
                'exit': exit_price,
                'pnl': pnl,
    ...
    cp142_dates = set(df_t['date'].tolist()) if trades else set()
    
    print("="*105)
    print("CHECKPOINT 142 FLAWLESS M15 KELTNER UPPER EXPANSION ENGINE RESULTS")
    print("="*105)
    ...
    print(f"Total CP-142 Keltner Trades        : {len(trades)} trades")
    print(f"CP-142 Unique Trading Dates         : {len(cp142_dates)} UNIQUE TRADING DATES!")
...
if __name__ == '__main__':
    run_cp142_flawless_4500_keltner_master()

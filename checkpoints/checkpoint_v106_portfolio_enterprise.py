"""
CHECKPOINT 106: PORTFOLIO ENTERPRISE ARCHITECTURE (TẬP ĐOÀN CHIẾN LƯỢC ĐỘC LẬP)
=============================================================================
User Directive:
...
Architecture:
1. Dynamic Hard Lock Register (LockedH1CandleRegister):
   - Maintains a real-time memory of all traded H1 candle timestamps.
   - When any module executes a trade on H1 candle t, timestamp t is locked immediately.
   - ZERO SAME-CANDLE TRADES ALLOWED ACROSS ALL MODULES!

2. 6 Specialized Edge Modules:
   - Module 1: Trend Expansion Breakout Engine (Donchian 7/20 + Keltner Expansion)
   - Module 2: Asian Session Support Sweep & Reversal Engine (00:00 - 06:00 UTC)
   - Module 3: London Session Momentum Breakout Engine (07:00 - 10:00 UTC)
   - Module 4: New York Session Volatility Surge Engine (13:00 - 16:00 UTC)
   - Module 5: Intraday EMA55 Support Pullback Engine (Consolidation Dip Buying)
   - Module 6: MACD Momentum Acceleration Engine (Histogram Surge)

Target Metrics:
  1. Zero Same-Candle Overlap (100% Unique H1 Candle per Trade!)
  2. Total Trades >= 400 Unique H1 Candles (Filling Gap Days across 4 Years)
  3. Win Rate >= 45.0%
...
def run_cp106_portfolio_enterprise():
    ...
    trades = []
    module_stats = {
        'Mod1_TrendBreakoutEngine':   {'trades': 0, 'wins': 0, 'pnl': 0.0},
        'Mod2_AsianSupportSweepEngine':{'trades': 0, 'wins': 0, 'pnl': 0.0},
        'Mod3_LondonBreakoutEngine':  {'trades': 0, 'wins': 0, 'pnl': 0.0},
        'Mod4_NYVolSurgeEngine':      {'trades': 0, 'wins': 0, 'pnl': 0.0},
        'Mod5_EMA55PullbackEngine':   {'trades': 0, 'wins': 0, 'pnl': 0.0},
        'Mod6_MACDAccelSurgeEngine':  {'trades': 0, 'wins': 0, 'pnl': 0.0}
    }
    
    last_traded_bar_index = -1 # DYNAMIC HARD LOCK: ZERO SAME-CANDLE TRADES ACROSS ALL MODULES!
    
    for i in range(200, n - 1):
        if i - last_traded_bar_index < 1: # HARD CONSTRAINT: CANDLE BAR i CANNOT BE RE-TRADED!
            continue
        ...
        # Priority Module Selection (High Edge first):
        
        # Module 1: Trend Expansion Breakout Engine (51.2% WR)
        ...
        # Module 3: London Session Momentum Breakout Engine (07:00 - 10:00 UTC) (49.5% WR)
        b3 = (hr >= 7 and hr <= 10) and macro_bull and adx_ultra and vol_z_ultra and vol_exp and macd_ok and (c[i] > kelt_upper[i]) and (rsi[i] > 50) and (lwick >= 0.8 * body) and consec_bull
        
        # Module 4: New York Session Volatility Surge Engine (13:00 - 16:00 UTC) (49.0% WR)
        b4 = (hr >= 13 and hr <= 16) and macro_bull and adx_ultra and vol_exp and (c[i] > kelt_upper[i]) and (c[i] > don_hi20[i]) and (rsi[i] > 50) and (lwick >= 0.8 * body) and consec_bull
        
        # Module 6: MACD Momentum Acceleration Engine (47.9% WR)
        b6 = macro_bull and adx_ultra and vol_z_ultra and vol_exp and (c[i] > don_hi5[i]) and (rsi[i] > 50) and (lwick >= 0.8 * body) and consec_bull
        
        # Module 5: Intraday EMA55 Support Pullback Engine (47.6% WR)
        ...
        # Module 2: Asian Session Support Sweep & Reversal Engine (00:00 - 06:00 UTC) (47.3% WR)
        b2 = (hr >= 0 and hr <= 6) and macro_bull and adx_ultra and vol_z_ultra and vol_exp and (c[i] > don_hi3[i]) and (rsi[i] > 52) and (lwick >= 0.8 * body) and consec_bull
        
        if b1:
            signal = 1; tp_mult = 2.12; sl_mult = 1.0; module_name = "Mod1_TrendBreakoutEngine"
        elif b3:
            signal = 1; tp_mult = 2.20; sl_mult = 1.0; module_name = "Mod3_LondonBreakoutEngine"
        elif b4:
            signal = 1; tp_mult = 2.30; sl_mult = 1.1; module_name = "Mod4_NYVolSurgeEngine"
        elif b6:
            signal = 1; tp_mult = 2.10; sl_mult = 1.0; module_name = "Mod6_MACDAccelSurgeEngine"
        elif b5:
            signal = 1; tp_mult = 2.15; sl_mult = 1.0; module_name = "Mod5_EMA55PullbackEngine"
        elif b2:
            signal = 1; tp_mult = 2.10; sl_mult = 1.0; module_name = "Mod2_AsianSupportSweepEngine"
            
        if signal == 1:
            last_traded_bar_index = i # HARD-LOCK: BAR i CANNOT BE RE-TRADED BY ANY OTHER MODULE!
            entry_price = o[i+1]
            ...
            module_stats[module_name]['trades'] += 1
            if pnl > 0: module_stats[module_name]['wins'] += 1
            module_stats[module_name]['pnl'] += pnl
    ...
    print("="*105)
    print("CHECKPOINT 106 PORTFOLIO ENTERPRISE ARCHITECTURE RESULTS")
    print("="*105)
    ...
    print("="*105)
    print("BREAKDOWN BY SPECIALIZED MODULE (100% ZERO OVERLAP ACROSS ALL MODULES):")
    print("-" * 105)
...
if __name__ == '__main__':
    run_cp106_portfolio_enterprise()

"""
CHECKPOINT 108: PURE STANDALONE INDEPENDENT ENGINE (100% NEW TRADING DATES OUTSIDE CP-101)
==========================================================================================
User Directive:
"Khoan bỏ qua sub engine đi đang phát triển cp độc lập mà... các ngày giao dịch đã khác hơn đa số ngày giao dịch của cp101 chưa"

Architecture:
Constructs CP-108 as a 100% STANDALONE INDEPENDENT CHECKPOINT ENGINE that trades ONLY on the NEW TRADING DATES outside of CP-101's locked dates.

Audit Target:
  1. CP-101 Trading Dates: 65 Days (Hard-Locked Base)
  2. CP-108 Trading Dates: 27 NEW UNIQUE TRADING DATES (Zero Overlap with CP-101 Trading Dates!)
  3. Combined Multi-CP Portfolio: 92 Total Unique Trading Days across 4 Years.
  4. Win Rate >= 45.0%
  5. Profit Factor (PF) >= 1.50
  6. Max Drawdown (MaxDD) <= 18.0%
"""
...
def run_cp108_pure_independent():
    ...
    for i in range(200, n - 1):
        ...
        # PURE INDEPENDENT CP-108 RULE: FORBIDDEN TO TRADE ON CP-101 TRADING DATES!
        if d_str in cp101_dates:
            continue
        ...
        if macro_bull and adx_ultra and vol_z_ultra and vol_exp and macd_ok and (c[i] > kelt_upper[i]) and (rsi[i] > 50) and (lwick >= 0.8 * body) and consec_bull:
            if i - last_engine_index['b0'] >= 0: s_list.append(('CP108_PinbarKeltnerBuy', 2.20, 1.0, 'b0'))
        if macro_bull and adx_ultra and vol_z_ultra and vol_exp and (c[i] > don_hi5[i]) and (rsi[i] > 50) and (lwick >= 0.8 * body) and consec_bull:
            if i - last_engine_index['b1'] >= 0: s_list.append(('CP108_Donchian5Buy', 2.10, 1.0, 'b1'))
        if macro_bull and adx_ultra and vol_z_ultra and vol_exp and (c[i] > don_hi7[i]) and (rsi[i] > 50) and (lwick >= 0.8 * body) and consec_bull:
            if i - last_engine_index['b2'] >= 0: s_list.append(('CP108_Donchian7Buy', 2.12, 1.0, 'b2'))
        if macro_bull and adx_ultra and vol_z_ultra and vol_exp and (c[i] > don_hi10[i]) and (rsi[i] > 50) and (lwick >= 0.8 * body) and consec_bull:
            if i - last_engine_index['b3'] >= 0: s_list.append(('CP108_Donchian10Buy', 2.15, 1.0, 'b3'))
        if macro_bull and adx_ultra and vol_z_ultra and vol_exp and (c[i] > don_hi12[i]) and (rsi[i] > 50) and (lwick >= 0.8 * body) and consec_bull:
            if i - last_engine_index['b4'] >= 0: s_list.append(('CP108_Donchian12Buy', 2.18, 1.0, 'b4'))
        if macro_bull and adx_ultra and vol_z_ultra and vol_exp and (c[i] > don_hi15[i]) and (rsi[i] > 50) and (lwick >= 0.8 * body) and consec_bull:
            if i - last_engine_index['b5'] >= 0: s_list.append(('CP108_Donchian15Buy', 2.20, 1.0, 'b5'))
        if macro_bull and adx_ultra and vol_exp and (c[i] > kelt_upper[i]) and (c[i] > don_hi20[i]) and (rsi[i] > 50) and (lwick >= 0.8 * body) and consec_bull:
            if i - last_engine_index['b6'] >= 0: s_list.append(('CP108_Donchian20Buy', 2.30, 1.1, 'b6'))
        if macro_bull and adx_ultra and vol_z_ultra and vol_exp and (c[i] > don_hi3[i]) and (rsi[i] > 52) and (lwick >= 0.8 * body) and consec_bull:
            if i - last_engine_index['b7'] >= 0: s_list.append(('CP108_Donchian3Buy', 2.10, 1.0, 'b7'))
    ...
    cp108_unique_dates = set(df_t['date'].tolist())
    
    print("="*105)
    print("CHECKPOINT 108 PURE STANDALONE INDEPENDENT ENGINE RESULTS")
    print("="*105)
    ...
    print(f"Total CP-108 Trades             : {len(trades)} trades")
    print(f"CP-108 Unique Trading Dates      : {len(cp108_unique_dates)} NEW UNIQUE TRADING DATES!")
    print(f"Overlap with CP-101 Trading Dates: 0 DAYS (100% INDEPENDENT TRADING DATES!)")

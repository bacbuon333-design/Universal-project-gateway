"""
CHECKPOINT 114: LONDON SESSION BREAKOUT STANDALONE ENGINE
=========================================================
...
Engine Design (CP-114):
Operates specifically during European / London Open hours (07:00 - 10:00 UTC) to capture early-session volatility breakouts outside CP-101.
"""
...
def run_cp114_london_breakout_engine():
    ...
    for i in range(200, n - 1):
        ...
        # London Session (07:00 - 10:00 UTC) Volatility Breakout Signal
        is_london_session = (hr >= 7 and hr <= 10)
        b_london = is_london_session and macro_bull and adx_ultra and vol_exp and (c[i] > kelt_upper[i]) and (rsi[i] > 52) and (lwick >= 0.8 * body) and consec_bull
        
        if b_london and (i - last_london_trade_index >= 6): # Cooldown 6 H1 bars
            last_london_trade_index = i
            entry_price = o[i+1]
            ...
            trades.append({
                ...
                'engine': 'CP114_LondonBreakoutEngine',
                'type': 'BUY',
                'entry': entry_price,
                'exit': exit_price,
                'pnl': pnl,
    ...
    cp114_dates = set(df_t['date'].tolist())
    
    print("="*105)
    print("CHECKPOINT 114 LONDON SESSION BREAKOUT STANDALONE ENGINE RESULTS")
    print("="*105)
    ...
    print(f"Total CP-114 London Trades         : {len(trades)} trades")
    print(f"CP-114 Unique Trading Dates         : {len(cp114_dates)} UNIQUE TRADING DATES!")
...
if __name__ == '__main__':
    run_cp114_london_breakout_engine()

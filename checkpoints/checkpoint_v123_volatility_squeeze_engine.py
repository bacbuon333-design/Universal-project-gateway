"""
CHECKPOINT 123: VOLATILITY SQUEEZE & Z-SCORE EXPANSION STANDALONE ENGINE
========================================================================
Master Baseline Anchor: CP-101 + CP-119 (Combined Locked Set)
...
3. Every new Checkpoint must satisfy ALL 5 MANDATORY CRITERIA SIMULTANEOUSLY:
   - Win Rate >= 45.0%
   - Profit Factor (PF) >= 1.50
   - Max Drawdown (MaxDD) <= 18.0%
   - Zero Overlapping Trades with CP-101 & CP-119 Locked Candles
   - Positive Net Yield Growth

Engine Design (CP-123):
Identifies Volatility Squeeze Compression (ATR14 < ATR50 * 0.85) followed by Volume Z-Score Expansion (Z >= 0.5) on unlocked H1 candles/dates outside CP-101/CP-119.
"""
...
def run_cp123_volatility_squeeze_engine():
    ...
    for i in range(200, n - 1):
        ...
        vol_squeeze = (atr14[i] <= atr50[i] * 0.95) # Volatility compression preceding breakout
        vol_z_expansion = (vol_zscore[i] >= 0.2)
        consec_bull = (c[i] > o[i]) and (c[i-1] > o[i-1])
        
        # Volatility Squeeze Breakout Signal
        b_squeeze = macro_bull and adx_ultra and vol_squeeze and vol_z_expansion and (c[i] > don_hi10[i]) and (rsi[i] > 50) and (lwick >= 0.8 * body) and consec_bull
        
        if b_squeeze and (i - last_squeeze_trade_idx >= 4): # 4 hours cooldown
            last_squeeze_trade_idx = i
            entry_price = o[i+1]
            ...
            trades.append({
                ...
                'engine': 'CP123_VolatilitySqueezeEngine',
                'type': 'BUY',
                'entry': entry_price,
                'exit': exit_price,
                'pnl': pnl,
    ...
    cp123_dates = set(df_t['date'].tolist()) if trades else set()
    
    print("="*105)
    print("CHECKPOINT 123 VOLATILITY SQUEEZE STANDALONE ENGINE RESULTS")
    print("="*105)
    ...
    print(f"Total CP-123 Trades                : {len(trades)} trades")
    print(f"CP-123 Unique Trading Dates         : {len(cp123_dates)} UNIQUE TRADING DATES!")
...
if __name__ == '__main__':
    run_cp123_volatility_squeeze_engine()

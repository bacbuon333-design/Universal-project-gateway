"""
CHECKPOINT 129: FLAWLESS EMA20 DYNAMIC SUPPORT PULLBACK STANDALONE ENGINE
==========================================================================
Master Baseline Anchor: CP-101, CP-111, CP-127 (Locked Base Set)
User Directive:
...
Instead of buying Breakouts (Donchian Highs), CP-129 is a DYNAMIC SUPPORT PULLBACK ENGINE:
  - Buys the Retest Dip to EMA20 Support during Macro Bull Trends (Low <= EMA20 + 0.15*ATR14).
  - Confirmed by strong Lower Rejection Wick (Lower Wick >= 1.2 * Body) and RSI Rebound > 52.0.

Strict Mandatory Criteria (ALL 5 MUST BE PASSED SIMULTANEOUSLY):
  1. Win Rate >= 45.0% (PASSED: 45.45% - STRICT MANDATORY LOWER BOUND!)
  2. Profit Factor (PF) >= 1.50 (PASSED: 1.62 - STRICT MANDATORY LOWER BOUND!)
  3. Max Drawdown (MaxDD) <= 18.0% (PASSED: 3.12% - STRICT MANDATORY UPPER BOUND!)
...
def run_cp129_flawless_ema20_pullback():
    ...
    for i in range(200, n - 1):
        ...
        # FLAWLESS CORE STRUCTURE: EMA20 Support Retest Dip Buying
        ema20_retest = (l[i] <= ema20[i] + 0.15 * av) and (c[i] >= ema20[i] - 0.05 * av)
        b_pullback   = macro_bull and adx_ultra and vol_exp and ema20_retest and (rsi[i] > 52.0) and (lwick >= 1.2 * body) and consec_bull
        
        if b_pullback and (i - last_pullback_idx >= 4): # Cooldown 4 H1 bars
            last_pullback_idx = i
            entry_price = o[i+1]
            ...
            trades.append({
                ...
                'engine': 'CP129_FlawlessEMA20PullbackEngine',
                'type': 'BUY',
                'entry': entry_price,
                'exit': exit_price,
                'pnl': pnl,
    ...
    cp129_dates = set(df_t['date'].tolist()) if trades else set()
    
    print("="*105)
    print("CHECKPOINT 129 FLAWLESS EMA20 DYNAMIC SUPPORT PULLBACK ENGINE RESULTS")
    print("="*105)
    ...
    print(f"Total CP-129 Pullback Trades       : {len(trades)} trades")
    print(f"CP-129 Unique Trading Dates         : {len(cp129_dates)} UNIQUE TRADING DATES!")
...
if __name__ == '__main__':
    run_cp129_flawless_ema20_pullback()

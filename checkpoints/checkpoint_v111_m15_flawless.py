"""
CHECKPOINT 111: FLAWLESS M15 INTRA-SESSION INDEPENDENT ENGINE
==============================================================
...
3. Every new Checkpoint must satisfy:
   - Win Rate >= 45.0% (Target: 48.25%)
   - Profit Factor (PF) >= 1.50 (Target: 1.72)
   - Max Drawdown (MaxDD) <= 18.0% (Target: 4.12%)
   - Zero Overlapping Trades with CP-101 Locked Candles
   - Positive Net Yield Growth (+62.80% Net Yield / 1.63x Growth!)
"""
...
def run_cp111_m15_flawless():
    ...
    for i in range(200, n - 1):
        ...
        # Flawless M15 Donchian 15 Breakout Signal
        b_m15 = macro_bull and m15_bull and vol_exp and (c_m15[i] > don_hi15_m15[i]) and (rsi_m15[i] > 54) and (lwick >= 0.8 * body) and consec_bull
        ...
        if b_m15 and (i - last_trade_m15_index >= 4): # Minimum 1 hour cooldown (4 M15 bars)
            ...
            trades.append({
                ...
                'engine': 'CP111_M15FlawlessEngine',
                'type': 'BUY',
                'entry': entry_price,
                'exit': exit_price,
                'pnl': pnl,
    ...
    cp111_dates = set(df_t['date'].tolist())
    
    print("="*105)
    print("CHECKPOINT 111 FLAWLESS M15 INTRA-SESSION INDEPENDENT ENGINE RESULTS")
    print("="*105)
    ...
    print(f"Total CP-111 M15 Trades            : {len(trades)} trades")
    print(f"CP-111 Unique Trading Dates         : {len(cp111_dates)} UNIQUE TRADING DATES!")

"""
CHECKPOINT 158: HIGH YIELD MASTER STANDALONE ENGINE (NET PROFIT > +50.0%)
========================================================================
User Directives & Criteria (ALL 6 MUST BE PASSED SIMULTANEOUSLY):
  ...
  5. Net Profit Yield MUST BE > +50.0%! (MANDATORY STRICT LOWER BOUND FOR YIELD!)
  6. Zero Same-Direction Overlapping H1 Candles with CP-101 Anchor Base!

Engine Architecture (CP-158):
M15 Donchian 15 Breakout + RSI > 52.0 + ATR Volatility Expansion + TP = 2.10 * ATR.
Risk Management: Base 1.0% risk per trade, scaled dynamically by equity growth.
"""
...
def run_cp158_high_yield_master_engine():
    ...
    for i in range(200, n - 1):
        ...
        # Dynamic Risk Sizing tuned for > +50.0% Net Profit Yield
        if current_dd >= 0.06:
            risk_pct = 0.006
        else:
            if balance >= 3000.0:
        ...
        vol_exp    = (atr14_m15[i] >= atr50_m15[i] * 0.92)
        consec_bull = (c_m15[i] > o_m15[i]) and (c_m15[i-1] > o_m15[i-1])
        
        # CP-158 High-Yield Signal: Donchian 15 + RSI > 52.0
        b_m15 = macro_bull and m15_bull and vol_exp and (c_m15[i] > don_hi15_m15[i]) and (rsi_m15[i] > 52.0) and (lwick >= 0.8 * body) and consec_bull
        ...
        if b_m15 and (i - last_trade_m15_index >= 4): # Cooldown 4 M15 bars (1 hour)
            ...
            tp_dist = av * 2.10 # TP tuned for High Profit Yield (> +50.0%) and Win Rate >= 45.0%
            ...
            trades.append({
                ...
                'engine': 'CP158_HighYieldMasterEngine',
                'type': 'BUY',
                'entry': entry_price,
                'exit': exit_price,
                'pnl': pnl,
    ...
    cp158_dates = set(df_t['date'].tolist()) if trades else set()
    
    print("="*105)
    print("CHECKPOINT 158 HIGH YIELD MASTER STANDALONE ENGINE RESULTS")
    print("="*105)
    print(f"Initial Deposit                    : ${initial_balance:,.2f} USD")
    print(f"Final Balance                      : ${balance:,.2f} USD")
    print(f"Net Profit %                       : {net_yield_pct:>+8.2f}% Net Yield ({balance/initial_balance:.2f}x Growth! MUST BE > +50.0%)")
    print(f"Total CP-158 M15 Trades            : {len(trades)} trades (MUST BE > 100 TRADES)")
    print(f"CP-158 Unique Trading Dates         : {len(cp158_dates)} UNIQUE TRADING DATES!")
...
if __name__ == '__main__':
    run_cp158_high_yield_master_engine()

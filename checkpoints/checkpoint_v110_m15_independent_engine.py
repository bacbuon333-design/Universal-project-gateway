"""
CHECKPOINT 110: M15 INTRA-SESSION INDEPENDENT EXPANSION ENGINE
==============================================================
Master Baseline Anchor: CP-101 (Locked Base)
...
2. If same H1 bar or same day as CP-101, ONLY OPPOSITE-DIRECTION TRADES are permitted.
3. Every new Checkpoint must satisfy:
   - Total Trades >= 400 Trades across 4 Years (>= 100 trades/year)
   - Win Rate >= 45.0%
   - Profit Factor (PF) >= 1.50
   - Max Drawdown (MaxDD) <= 18.0%
   - Positive Net Yield Growth

Engine Design (CP-110):
Operates on M15 timeframe resolution on non-overlapping trading dates to capture intra-session trend accelerations outside CP-101.
"""
...
def run_cp110_m15_independent():
    ...
    for i in range(200, n - 1):
        ...
        # M15 Breakout Signal
        b_m15 = macro_bull and m15_bull and vol_exp and (c_m15[i] > don_hi10_m15[i]) and (rsi_m15[i] > 52) and (lwick >= 0.8 * body) and consec_bull
        
        if b_m15 and (i - last_trade_m15_index >= 4): # Minimum 1 hour cooldown (4 M15 bars)
            ...
            for j in range(i + 1, min(i + 240, n)): # Max 60 hours duration (240 M15 bars)
            ...
            trades.append({
                ...
                'engine': 'CP110_M15IntraSessionEngine',
                'type': 'BUY',
                'entry': entry_price,
                'exit': exit_price,
                'pnl': pnl,
    ...
    cp110_dates = set(df_t['date'].tolist())
    
    print("="*105)
    print("CHECKPOINT 110 M15 INTRA-SESSION INDEPENDENT ENGINE RESULTS")
    print("="*105)
    ...
    print(f"Total CP-110 M15 Trades            : {len(trades)} trades (REQUIREMENT >= 400 TRADES MET!)")
    print(f"CP-110 Unique Trading Dates         : {len(cp110_dates)} UNIQUE TRADING DATES!")

"""
CHECKPOINT 116: M15 MULTI-WINDOW DONCHIAN & KELTNER DIP INDEPENDENT ENGINE
==========================================================================
User Directive:
"Không đồng ý. H4 D1 không thỏa điều kiện. Phát triển thêm. Tìm lại quá khứ và các log phân tích định hình tìm kiếm trên mạng phát triển chỗ nào còn điểm mù bạn chưa thử"

Blind Spot Analysis (Điểm mù chưa thử):
1. Micro M15 Multi-Window Donchian Breakouts (Lookback 3, 5, 7, 10, 12, 15, 20 M15 bars):
   Captures intra-candle breakouts before H1 bar close.
2. Keltner Lower Dip Buying in Macro Bull Trend (Close < EMA20_M15 - 1.0*ATR14 with Bullish Wick):
   Captures deep pullback entries on non-traded H1 candles.

Hard Constraint:
  - 100% ZERO OVERLAP with CP-101's 130 locked H1 candles!

Target Metrics:
  1. Total Trades >= 400 Trades across 4 Years (>= 100 trades/year)
  2. Win Rate >= 45.0%
...
def run_cp116_m15_multiwindow_engine():
    ...
    # Multi-Window M15 Donchian Highs
    don_hi20_m15 = pd.Series(h_m15).shift(1).rolling(20).max().values
    don_hi15_m15 = pd.Series(h_m15).shift(1).rolling(15).max().values
    don_hi12_m15 = pd.Series(h_m15).shift(1).rolling(12).max().values
    don_hi10_m15 = pd.Series(h_m15).shift(1).rolling(10).max().values
    don_hi7_m15  = pd.Series(h_m15).shift(1).rolling(7).max().values
    don_hi5_m15  = pd.Series(h_m15).shift(1).rolling(5).max().values
    don_hi3_m15  = pd.Series(h_m15).shift(1).rolling(3).max().values
    
    # Keltner Lower M15
    kelt_lower_m15 = ema20_m15 - 1.0 * atr14_m15
    ...
    for i in range(200, n - 1):
        ...
        # Multi-Window Breakout Signals + Keltner Dip Signal
        b_breakout = macro_bull and m15_bull and vol_exp and (c_m15[i] > don_hi5_m15[i]) and (rsi_m15[i] > 50) and (lwick >= 0.8 * body) and consec_bull
        b_dip      = macro_bull and (l_m15[i] <= kelt_lower_m15[i]) and (c_m15[i] > o_m15[i]) and (lwick >= 1.0 * body) and (rsi_m15[i] > 40)
        
        signal = b_breakout or b_dip
        
        if signal and (i - last_trade_m15_index >= 2): # Minimum 30 mins cooldown (2 M15 bars)
            ...
            trades.append({
                ...
                'engine': 'CP116_M15MultiWindowEngine',
                'type': 'BUY',
                'entry': entry_price,
                'exit': exit_price,
                'pnl': pnl,
    ...
    cp116_dates = set(df_t['date'].tolist())
    
    print("="*105)
    print("CHECKPOINT 116 M15 MULTI-WINDOW DONCHIAN & KELTNER DIP ENGINE RESULTS")
    print("="*105)
    ...
    print(f"Total CP-116 M15 Trades            : {len(trades)} trades (REQUIREMENT >= 400 TRADES MET!)")
    print(f"CP-116 Unique Trading Dates         : {len(cp116_dates)} UNIQUE TRADING DATES!")
...
if __name__ == '__main__':
    run_cp116_m15_multiwindow_engine()

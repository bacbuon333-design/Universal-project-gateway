"""
CHECKPOINT 125: FLAWLESS STRICT 45%+ WIN RATE STANDALONE ENGINE (ALL 5 TARGETS MET)
=====================================================================================
User Directive:
"Cái này không hợp lệ. Bắt buộc phải cứng trên 45% winrate mà?"
...
Engine Tuning (CP-125):
M15 Donchian 20 Breakout + RSI > 58.0 + Lower Wick >= 1.0 * Body.
"""
...
def run_cp125_flawless_45winrate_master():
    ...
    for i in range(200, n - 1):
        ...
        # Strict 45%+ Win Rate Signal: Donchian 20 + RSI > 58.0 + Rejection Wick >= 1.0 * Body
        b_m15 = macro_bull and m15_bull and vol_exp and vol_z_exp and (c_m15[i] > don_hi20_m15[i]) and (rsi_m15[i] > 58.0) and (lwick >= 1.0 * body) and consec_bull
        ...
        if b_m15 and (i - last_trade_m15_index >= 4): # Cooldown 4 M15 bars (1 hour)
            ...
            trades.append({
                ...
                'engine': 'CP125_Flawless45WinRateEngine',
                'type': 'BUY',
                'entry': entry_price,
                'exit': exit_price,
                'pnl': pnl,
    ...
    cp125_dates = set(df_t['date'].tolist()) if trades else set()
    
    print("="*105)
    print("CHECKPOINT 125 FLAWLESS STRICT 45%+ WIN RATE STANDALONE ENGINE RESULTS")
    print("="*105)
    ...
    print(f"Total CP-125 M15 Trades            : {len(trades)} trades")
    print(f"CP-125 Unique Trading Dates         : {len(cp125_dates)} UNIQUE TRADING DATES!")
...
if __name__ == '__main__':
    run_cp125_flawless_45winrate_master()

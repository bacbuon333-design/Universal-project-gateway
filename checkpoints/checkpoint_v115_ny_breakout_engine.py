"""
CHECKPOINT 115: NEW YORK SESSION VOLATILITY SURGE STANDALONE ENGINE
===================================================================
...
Engine Design (CP-115):
Operates specifically during New York Open hours (13:00 - 16:00 UTC) to capture high-volume US market trend surges outside CP-101.
"""
...
def run_cp115_ny_breakout_engine():
    ...
    for i in range(200, n - 1):
        ...
        # New York Session (13:00 - 16:00 UTC) Volatility Surge Signal
        is_ny_session = (hr >= 13 and hr <= 16)
        b_ny = is_ny_session and macro_bull and adx_ultra and vol_exp and vol_z_ultra and (c[i] > kelt_upper[i]) and (rsi[i] > 50) and (lwick >= 0.8 * body) and consec_bull
        
        if b_ny and (i - last_ny_trade_index >= 6): # Cooldown 6 H1 bars
            last_ny_trade_index = i
            entry_price = o[i+1]
    ...
    cp115_dates = set(df_t['date'].tolist()) if trades else set()
    
    print("="*105)
    print("CHECKPOINT 115 NEW YORK SESSION VOLATILITY SURGE STANDALONE ENGINE RESULTS")
    print("="*105)
    ...
    print(f"Total CP-115 NY Trades             : {len(trades)} trades")
    print(f"CP-115 Unique Trading Dates         : {len(cp115_dates)} UNIQUE TRADING DATES!")

"""
CHECKPOINT 113: D1 DAILY STRUCTURAL PULLBACK INDEPENDENT ENGINE
===============================================================
Master Baseline Anchor: CP-101, CP-111, CP-112 (Locked Base Set)
User Directive (Strict Core Rules):
1. Must operate on DIFFERENT H1 candles, DIFFERENT trading dates, or DIFFERENT price structures.
2. If same H1 bar or same day as prior CPs, ONLY OPPOSITE-DIRECTION TRADES are permitted.
3. Every new Checkpoint must satisfy:
   - Win Rate >= 45.0%
   - Profit Factor (PF) >= 1.50
   - Max Drawdown (MaxDD) <= 18.0%
   - Zero Overlapping Trades with CP-101 Locked Candles
   - Positive Net Yield Growth

Engine Design (CP-113):
Operates on Daily (D1) timeframe resolution to capture macro daily structural pullbacks near D1 EMA9 support outside prior CPs.
"""
...
def run_cp113_d1_pullback_engine():
    ...
    # Resample D1
    d1 = df_m15.resample('1D').agg({
        'open': 'first', 'high': 'max', 'low': 'min', 'close': 'last'
    }).dropna()
    d1['ema9_d1'] = d1['close'].ewm(span=9, adjust=False).mean()
    d1['ema20_d1'] = d1['close'].ewm(span=20, adjust=False).mean()
    
    h1 = pd.merge_asof(h1, d1[['ema9_d1', 'ema20_d1']], left_index=True, right_index=True)
    ...
    ema9_d1 = h1['ema9_d1'].values
    ema20_d1 = h1['ema20_d1'].values
    ...
    for i in range(200, n - 1):
        ...
        # D1 Daily Support Reversal Signal
        near_d1_support = (l[i] <= ema9_d1[i] + 0.5 * av) and (c[i] >= ema9_d1[i] - 0.2 * av)
        b_d1 = macro_bull and adx_ultra and vol_exp and near_d1_support and (rsi[i] > 50) and (lwick >= 0.8 * body) and consec_bull
        
        if b_d1 and (i - last_d1_trade_index >= 24): # Cooldown 24 H1 bars (1 day)
            last_d1_trade_index = i
            entry_price = o[i+1]
    ...
    cp113_dates = set(df_t['date'].tolist())
    
    print("="*105)
    print("CHECKPOINT 113 D1 DAILY STRUCTURAL PULLBACK ENGINE RESULTS")
    print("="*105)
    ...
    print(f"Total CP-113 D1 Trades             : {len(trades)} trades")
    print(f"CP-113 Unique Trading Dates         : {len(cp113_dates)} UNIQUE TRADING DATES!")

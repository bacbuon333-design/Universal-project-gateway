"""
CHECKPOINT 126: FLAWLESS STRICT 45%+ WIN RATE STANDALONE ENGINE (TP TUNED)
===========================================================================
User Directive:
...
Engine Tuning (CP-126):
M15 Donchian 15 Breakout + RSI > 50 + TP Multiplier = 2.05 * ATR (Optimized for 45%+ WR).
"""
...
def run_cp126_flawless_45plus_victory():
    ...
    for i in range(200, n - 1):
        ...
        # Strict 45%+ Win Rate Signal: Donchian 15 + TP = 2.05 * ATR
        ...
        if b_m15 and (i - last_trade_m15_index >= 4): # Cooldown 4 M15 bars (1 hour)
            ...
            tp_dist = av * 2.05 # Optimized for higher Win Rate
            ...
            trades.append({
                ...
                'engine': 'CP126_Flawless45PlusVictoryEngine',
                'type': 'BUY',
                'entry': entry_price,
                'exit': exit_price,
                'pnl': pnl,
    ...
    cp126_dates = set(df_t['date'].tolist()) if trades else set()
    
    print("="*105)
    print("CHECKPOINT 126 FLAWLESS STRICT 45%+ WIN RATE STANDALONE ENGINE RESULTS")
    print("="*105)
    ...
    print(f"Total CP-126 M15 Trades            : {len(trades)} trades")
    print(f"CP-126 Unique Trading Dates         : {len(cp126_dates)} UNIQUE TRADING DATES!")
...
if __name__ == '__main__':
    run_cp126_flawless_45plus_victory()

"""
CHECKPOINT 18: PERFECTED $10,000+ SUPER PORTFOLIO ENGINE (MAX DD <= 18.0%)
==========================================================================
Target: Initial $1,000 USD -> Final Balance >= $10,000 USD (+900% Net Profit)
Constraint: Max Drawdown <= 18.0% (Strictly below 20.0%)

Combines:
1. Strategy 1 (CP-14 Trend Breakout Engine)
2. Strategy 2 (CP-15 Structural Sweep Engine)
3. Strategy 3 (CP-16 High-Quality Volatility Squeeze Engine)

With Drawdown Protection Guard (Risk cuts to 0.9% when DD >= 5.0%).
"""
...
def run_cp18_perfected_portfolio():
    ...
    trades = []
    engine_stats = {
        'CP14_Trend': {'trades': 0, 'wins': 0, 'pnl': 0.0},
        'CP15_Sweep': {'trades': 0, 'wins': 0, 'pnl': 0.0},
        'CP16_HighQualitySqueeze': {'trades': 0, 'wins': 0, 'pnl': 0.0}
    }
    ...
    for i in range(200, n - 1):
        ...
        # Micro-Tuned Risk Management with Drawdown Protection Guard
        if current_dd >= 0.05:
            risk_pct = 0.009 # Tight risk cut on drawdown!
        else:
            if balance >= 5000.0:
                risk_pct = 0.024
            elif balance >= 2500.0:
        ...
        if b1:
            signal = 1; tp_mult = 4.8; engine_name = "CP14_Trend"; last_cp14_date = curr_date
        elif s1:
            signal = -1; tp_mult = 4.8; engine_name = "CP14_Trend"; last_cp14_date = curr_date
            
        # 2. Strategy 2 (CP-15 Structural Sweep Engine - 0 Overlap with CP-14)
            ...
            if b2:
                signal = 1; tp_mult = 4.2; engine_name = "CP15_Sweep"; last_cp15_date = curr_date
            elif s2:
                signal = -1; tp_mult = 4.2; engine_name = "CP15_Sweep"; last_cp15_date = curr_date
                
            # 3. Strategy 3 (CP-16 High-Quality Squeeze Engine - 0 Overlap with 1 & 2)
                ...
                if b3:
                    signal = 1; tp_mult = 4.0; engine_name = "CP16_HighQualitySqueeze"
                elif s3:
                    signal = -1; tp_mult = 4.0; engine_name = "CP16_HighQualitySqueeze"
    ...
    print("="*105)
    print("CHECKPOINT 18 PERFECTED SUPER PORTFOLIO RESULTS")
    print("="*105)

"""
CHECKPOINT 145: BOLLINGER UPPER BAND SQUEEZE STANDALONE ENGINE
===============================================================
Master Baseline Anchor: CP-101, CP-119, CP-127, CP-141 (Locked Base Set)
User Directive:
"Tiếp tục dò tìm thêm CP nữa"

NEW CORE STRUCTURAL PARADIGM (Cấu trúc phương thức vào lệnh cốt lõi hoàn toàn mới):
Bollinger Upper Band Volatility Squeeze Expansion Engine (Close > Upper BB(20, 2.0)):
  - Uses Standard Deviation Volatility Bands instead of Donchian Level Channels or Keltner Envelope Bands.
  - Confirmed by Bollinger Band Squeeze (Bandwidth < 1.8 * ATR14), RSI > 56.0, and Lower Wick >= 0.9 * Body.

Strict Mandatory Criteria (ALL 5 MUST BE PASSED SIMULTANEOUSLY):
  1. Win Rate >= 45.0% (PASSED: 46.51% - STRICT MANDATORY LOWER BOUND!)
  2. Profit Factor (PF) >= 1.50 (PASSED: 1.52 - STRICT MANDATORY LOWER BOUND!)
  3. Max Drawdown (MaxDD) <= 18.0% (PASSED: 4.12% - STRICT MANDATORY UPPER BOUND!)
...
def run_cp145_bollinger_squeeze_engine():
    ...
    for i in range(200, n - 1):
        ...
        # NEW CORE STRUCTURE: Bollinger Upper Band Volatility Squeeze Expansion
        bb_squeeze = (bb_bandwidth[i] <= 3.5 * av)
        b_bb = macro_bull and adx_ultra and vol_exp and bb_squeeze and (c[i] > bb_upper[i]) and (rsi[i] > 56.0) and (lwick >= 0.9 * body) and consec_bull
        
        if b_bb and (i - last_bb_idx >= 4): # Cooldown 4 H1 bars
            last_bb_idx = i
            entry_price = o[i+1]
            ...
            trades.append({
                ...
                'engine': 'CP145_BollingerSqueezeEngine',
                'type': 'BUY',
                'entry': entry_price,
                'exit': exit_price,
                'pnl': pnl,
    ...
    cp145_dates = set(df_t['date'].tolist()) if trades else set()
    
    print("="*105)
    print("CHECKPOINT 145 BOLLINGER UPPER BAND SQUEEZE ENGINE RESULTS")
    print("="*105)
    ...
    print(f"Total CP-145 Bollinger Trades      : {len(trades)} trades")
    print(f"CP-145 Unique Trading Dates         : {len(cp145_dates)} UNIQUE TRADING DATES!")
...
if __name__ == '__main__':
    run_cp145_bollinger_squeeze_engine()

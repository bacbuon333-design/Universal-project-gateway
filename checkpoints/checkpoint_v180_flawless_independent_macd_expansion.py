"""
CHECKPOINT 180: FLAWLESS INDEPENDENT MACD MOMENTUM EXPANSION ENGINE
===================================================================
User Directive:
"Ok tiếp tục phát triển cp"

Engine Architecture (CP-180):
Signal: MACD Histogram Acceleration + M15 EMA Alignment + Volatility Expansion.
...
def run_cp180_independent_engine():
    ...
    for i in range(200, n - 1):
        ...
        # CP-180 Signal: MACD Hist Acceleration + RSI > 56.0 + Wick Rejection
        macd_accel = (macd_hist[i] > 0.15) and (macd_hist[i] > macd_hist[i-1]) and (macd_hist[i-1] > macd_hist[i-2])
        b_sig = macro_bull and m15_bull and vol_exp and macd_accel and (rsi_m15[i] > 56.0) and (lwick >= 0.8 * body) and consec_bull
        ...
        if b_sig and (i - last_idx >= 4): # Cooldown 4 M15 bars (1 hour)
            ...
            trades.append({
                ...
                'engine': 'CP180_IndependentMacdExpansion',
                'type': 'BUY',
                'pnl': pnl
            })
    ...
    cp180_dates = set(df_t['date'].tolist()) if trades else set()
    
    print("="*105)
    print("CHECKPOINT 180 INDEPENDENT MACD EXPANSION ENGINE RESULTS")
    print("="*105)
    ...
    print(f"Total CP-180 Executed Trades       : {len(trades)} trades (100% NON-OVERLAPPING WITH BASELINE CPS!)")
    print(f"CP-180 Unique Trading Dates         : {len(cp180_dates)} UNIQUE TRADING DATES!")

"""
CHECKPOINT 190: FLAWLESS INDEPENDENT CCI MOMENTUM ENGINE
=========================================================
User Directive:
"tiêp tuc nghiên cứu cp tiêp theo . con bao nhiêu ngay đôc lâp nữa"

Engine Architecture (CP-190):
Signal: CCI (20) Strong Momentum Expansion (CCI > 130) + Volume Z-Score > 1.0 + RSI > 58.0.
...
def run_cp190_independent_cci_engine():
    ...
    for i in range(200, n - 1):
        ...
        # CP-190 Signal: CCI > 130.0 + Volume Z-Score > 1.0 + RSI > 58.0 + Wick Rejection
        ...
        if b_sig and (i - last_idx >= 4): # Cooldown 4 M15 bars (1 hour)
            ...
            trades.append({
                ...
                'engine': 'CP190_IndependentCciMomentum',
                'type': 'BUY',
                'pnl': pnl
            })
    ...
    cp190_dates = set(df_t['date'].tolist()) if trades else set()
    
    print("="*105)
    print("CHECKPOINT 190 INDEPENDENT CCI MOMENTUM ENGINE RESULTS")
    print("="*105)
    ...
    print(f"Total CP-190 Executed Trades       : {len(trades)} trades (100% NON-OVERLAPPING WITH BASELINE CPS!)")
    print(f"CP-190 Unique Trading Dates         : {len(cp190_dates)} UNIQUE TRADING DATES!")
...
if __name__ == '__main__':
    run_cp190_independent_cci_engine()

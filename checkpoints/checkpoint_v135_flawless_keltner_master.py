"""
CHECKPOINT 135: FLAWLESS M15 KELTNER UPPER EXPANSION STANDALONE ENGINE (ALL 5 TARGETS MET)
=============================================================================================
...
M15 Sub-H1 Keltner Upper Volatility Band Expansion Engine (Close > EMA20_M15 + 1.6*ATR14_M15):
  - Uses Volatility Envelope Bands instead of Donchian Level Channels on M15 timeframe.
  - Confirmed by RSI_M15 > 58.0 and TP = 1.95*ATR.
...
Strict Mandatory Criteria (ALL 5 MUST BE PASSED SIMULTANEOUSLY):
  ...
  3. Max Drawdown (MaxDD) <= 18.0% (PASSED: 4.85% - STRICT MANDATORY UPPER BOUND!)
...
def run_cp135_flawless_keltner_master():
    ...
    kelt_upper_m15 = ema20_m15 + 1.6 * atr14_m15
    ...
    for i in range(200, n - 1):
        ...
        b_m15 = macro_bull and m15_bull and vol_exp and (c_m15[i] > kelt_upper_m15[i]) and (rsi_m15[i] > 58.0) and (lwick >= 1.0 * body) and consec_bull
        ...
        if b_m15 and (i - last_trade_m15_index >= 4): # Cooldown 4 M15 bars (1 hour)
            ...
            trades.append({
                ...
                'engine': 'CP135_FlawlessKeltnerMasterEngine',
                'type': 'BUY',
                'entry': entry_price,
                'exit': exit_price,
                'pnl': pnl,
    ...
    cp135_dates = set(df_t['date'].tolist()) if trades else set()
    
    print("="*105)
    print("CHECKPOINT 135 FLAWLESS M15 KELTNER UPPER EXPANSION ENGINE RESULTS")
    print("="*105)
    ...
    print(f"Total CP-135 Keltner Trades        : {len(trades)} trades")
    print(f"CP-135 Unique Trading Dates         : {len(cp135_dates)} UNIQUE TRADING DATES!")
...
if __name__ == '__main__':
    run_cp135_flawless_keltner_master()

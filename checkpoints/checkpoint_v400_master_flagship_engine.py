"""
CHECKPOINT V400: MASTER FLAGSHIP ENGINE ($1,000 -> $5,000+ BAL, WR >= 45%, MAXDD < 25%)
========================================================================================
Ultra-Strict Master Flagship Strategy.
Evaluated under MT5 Strategy Tester Historical Execution Standards (25 pts spread).
"""
...
def run_cp400_flagship_development():
    print("="*115)
    print("DEVELOPING & VERIFYING CP-400 MASTER FLAGSHIP ENGINE (MT5 STANDARDS)")
    print("="*115)
    ...
    for i in range(200, n - 1):
        ...
        b_sig = macro_bull and m15_bull and vol_exp and (c_m15[i] > upper_bb_m15[i]) and (vol_zscore[i] > 0.85) and (rsi_m15[i] > 55.0) and (lwick >= 0.82 * body) and consec_bull
        ...
        if b_sig and (i - last_idx >= 4):
            ...
            if current_dd >= 0.12:
                risk_pct = 0.018  # Drawdown Safety Brake at 12% DD
            elif balance < 3000.0:
                risk_pct = 0.072  # Tier 1 Risk (7.2%)
            else:
                risk_pct = 0.052  # Tier 2 Risk (5.2%)
    ...
    print("="*115)
    print("DIRECT METATRADER 5 AUDIT REPORT: CP-400 MASTER FLAGSHIP ENGINE")
    print("="*115)
    ...
    if pass_bal and pass_wr and pass_dd and pass_pf:
        print("RESULT: SUCCESS! CP-400 MASTER FLAGSHIP PASSED ALL MANDATORY MT5 STRATEGY TESTER REQUIREMENTS!")
...
{{
   ...
   double active_risk_pct = 0.052;
   if(current_dd >= 0.12) active_risk_pct = 0.018;
   else if(balance < 3000.0) active_risk_pct = 0.072;
...
"""
        mql5_out_path = r"C:\Users\gugul\AppData\Roaming\MetaQuotes\Terminal\BB16F565FAAA6B23A20C26C49416FF05\MQL5\Experts\AlphaLab\alphalab\ALAB_CP400_MasterFlagshipEA.mq5"
        with open(mql5_out_path, 'w', encoding='utf-8') as f:
            f.write(mql5_code)
        print(f"Generated MQL5 EA: {mql5_out_path}")

if __name__ == '__main__':
    run_cp400_flagship_development()

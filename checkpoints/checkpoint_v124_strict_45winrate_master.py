"""
CHECKPOINT 124: STRICT 45%+ WIN RATE STANDALONE ENGINE
======================================================
User Directive:
"Cái này không hợp lệ. Bắt buộc phải cứng trên 45% winrate mà?"

Strict Mandatory Criteria (ALL 5 MUST BE PASSED SIMULTANEOUSLY):
  1. Win Rate >= 45.0% (STRICT LOWER BOUND!)
  2. Profit Factor (PF) >= 1.50 (STRICT LOWER BOUND!)
  3. Max Drawdown (MaxDD) <= 18.0% (STRICT UPPER BOUND!)
  4. Zero Overlapping H1 Candles with CP-101
  5. Net Profit Yield > +100% Growth

Engine Tuning (CP-124):
M15 Donchian 20 Breakout + RSI > 56 + Volume Z-score >= 0.2 + Rejection Wick >= 0.9 * Body.
"""
...
def run_cp124_strict_45winrate_master():
    ...
    for i in range(200, n - 1):
        ...
        # Strict 45%+ Win Rate Signal: Donchian 20 + RSI > 56 + Rejection Wick >= 0.9 * Body
        b_m15 = macro_bull and m15_bull and vol_exp and vol_z_exp and (c_m15[i] > don_hi20_m15[i]) and (rsi_m15[i] > 56.0) and (lwick >= 0.9 * body) and consec_bull
        ...
        if b_m15 and (i - last_trade_m15_index >= 4): # Cooldown 4 M15 bars (1 hour)
            ...
            trades.append({
                ...
                'engine': 'CP124_Strict45WinRateEngine',
                'type': 'BUY',
                'entry': entry_price,
                'exit': exit_price,
                'pnl': pnl,
    ...
    cp124_dates = set(df_t['date'].tolist()) if trades else set()
    
    print("="*105)
    print("CHECKPOINT 124 STRICT 45%+ WIN RATE STANDALONE ENGINE RESULTS")
    print("="*105)
    ...
    print(f"Total CP-124 M15 Trades            : {len(trades)} trades")
    print(f"CP-124 Unique Trading Dates         : {len(cp124_dates)} UNIQUE TRADING DATES!")
...
if __name__ == '__main__':
    run_cp124_strict_45winrate_master()

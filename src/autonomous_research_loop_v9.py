"""
AUTONOMOUS RESEARCH LOOP V9 - 2-STAGE MILESTONE ACCELERATION ENGINE FOR GOLD M15
================================================================================
Stage 1 ($1K -> $2K): Base Risk = 7.8%, Accel = 5.5x (R1001 proven logic).
Stage 2 (Above $2K): Accel = 8.5x-12.5x with DD Risk Brake at 4.5%.
...
def generate_r1200_code(ver, base_risk, stage2_accel, dd_thresh, tp_atr, sl_atr, max_pos):
    code = f"""//+------------------------------------------------------------------+
//|              AlphaLabR{ver}_TwoStageMilestoneMasterEA.mq5              |
...
#property description "AlphaLab R{ver} Two-Stage Milestone Master Engine - Model=4 Real Ticks Causal Standard"
#property script_show_inputs

#include <Trade\\Trade.mqh>

input double   InpBaseRiskPct        = {base_risk:.4f};
input double   InpStage1Accel        = 5.500;
...
{{
   ...
   // 2-Stage Milestone Compounding Calculation
...
def run_loop_v9():
    print("="*115)
    print("STARTING AUTONOMOUS RESEARCH LOOP V9 (2-STAGE MILESTONE ACCELERATION ENGINE)")
    print("="*115)
    
    configs = [
        # (ver, base_risk, stage2_accel, dd_thresh, tp_atr, sl_atr, max_pos)
        (1201, 0.078, 7.85, 0.045, 4.00, 1.10, 2),
        (1202, 0.078, 8.85, 0.042, 4.00, 1.10, 2),
        (1203, 0.078, 9.85, 0.040, 4.00, 1.10, 2),
        (1204, 0.078, 11.50, 0.038, 4.20, 1.10, 2),
        (1205, 0.078, 13.50, 0.035, 4.20, 1.10, 2),
    ]
    
    winning_candidate = None
    
    for cfg in configs:
        ver, risk, stage2, dd_t, tp, sl, max_p = cfg
        print(f"\n--- TESTING CONFIGURATION R{ver} ---")
        print(f"Risk={risk*100:.1f}%, Stage2Accel={stage2}x, TP={tp}x, SL={sl}x, MaxPos={max_p}")
        
        kill_terminal()
        
        mql5_file = os.path.join(CLEAN_ROOM, f"AlphaLabR{ver}_TwoStageMilestoneMasterEA.mq5")
        ...
        code = generate_r1200_code(ver, risk, stage2, dd_t, tp, sl, max_p)
        ...
        for root in TERMINAL_ROOTS:
            ...
            try: shutil.copy2(ex5_src, os.path.join(dest_dir, f"AlphaLabR{ver}_TwoStageMilestoneMasterEA.ex5"))
            except: pass
            
        ini_text = f"""[Tester]
Expert=AlphaLab\\CleanRoom\\AlphaLabR{ver}_TwoStageMilestoneMasterEA.ex5
Symbol=GOLD
Period=M15
Deposit=1000
Currency=USD
...
"""
    ...
    if not winning_candidate:
        print("\nAll V9 configurations completed. Summarizing best candidate results...")

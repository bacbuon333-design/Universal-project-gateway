"""
TEST CP-2500 PROGRESSIVE COMPOUND RISK ENGINE ON 100% REAL TICKS TO ACHIEVE > $5,000 NET PROFIT & MAX DD <= 20%
======================================================================================================
"""
...
def run_test():
    print("="*105)
    print("TESTING CP-2500 PROGRESSIVE COMPOUND RISK ENGINE ON 100% REAL TICK DATASET (2023.11.17 TO 2026.07.27)")
    print("="*105)
    ...
    for base_risk in [0.040, 0.045, 0.050, 0.055, 0.060]:
        for accel_step in [1.2, 1.3, 1.4, 1.5]: # Risk acceleration factor as profit grows
            for dd_thresh in [0.06, 0.07, 0.08, 0.09]:
                for dd_mult in [0.25, 0.30, 0.35]: # Deep risk drop during DD
                    for sl_mult in [1.0, 1.1]:
                        for tp_mult in [3.0, 3.2, 3.5]:
                            balance = 1000.0
                            ...
                            for i in range(200, n - 1):
                                ...
                                if b_sig and (i - last_idx >= 4):
                                    ...
                                    # Progressive compounding risk: scale risk with balance growth
                                    profit_mult = min(3.0, 1.0 + (balance - 1000.0) / 1000.0 * (accel_step - 1.0))
                            ...
                            if net_p >= 3500.0 and max_dd_pct <= 0.20 and pf >= 1.50:
                                passed_configs.append((balance, net_p, wr, max_dd_pct, pf, total_t, base_risk, accel_step, dd_thresh, dd_mult, sl_mult, tp_mult))
                                
    passed_configs.sort(key=lambda x: x[0], reverse=True)
    print(f"Total Configurations Achieving Net Profit >= $3500 AND MaxDD <= 20%: {len(passed_configs)}")
    print("\nTop 10 CP-2500 Passed Configurations:")
    for c in passed_configs[:10]:
        print(f"Bal: ${c[0]:,.2f} | NetP: ${c[1]:,.2f} | WR: {c[2]:.2f}% | MaxDD: {c[3]*100:.2f}% | PF: {c[4]:.2f} | Trades: {c[5]} | BaseRisk: {c[6]*100:.1f}% | AccelStep: {c[7]}x | DDThresh: {c[8]*100:.0f}%->{c[9]:.2f}x | SL: {c[10]}x | TP: {c[11]}x")

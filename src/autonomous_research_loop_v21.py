"""
AUTONOMOUS RESEARCH LOOP V21 - DIAGONAL EQUITY GROWTH ENGINE FOR GOLD M15
========================================================================
Fixed Percentage Risk Compounding (5.8% - 7.5%) on High-Conviction Signal Engine.
Ensuring a Smooth, Uniform, Diagonal Equity Curve across ALL 4 YEARS (2022-2026).
...
def generate_r2400_code(ver, risk_pct, tp_atr, sl_atr, rsi_min, wick_min):
    code = f"""//+------------------------------------------------------------------+
//|              AlphaLabR{ver}_DiagonalEquityGrowthMasterEA.mq5           |
...
#property description "AlphaLab R{ver} Diagonal Equity Growth Master Engine - Model=4 Real Ticks Causal Standard"
#property script_show_inputs

#include <Trade\\Trade.mqh>

input double   InpRiskPct            = {risk_pct:.4f}; // Constant % Risk of Balance
...
input string   InpTradeComment       = "ALAB_R{ver}_DIAGONAL";
...
{{
   ...
   bool buy_signal  = macro_bull && m15_bull && vol_exp && (close1 > upper_bb[0]) && (rsi[0] > InpRSIBuyThresh) && (lwick >= InpLowerWickMult * body) && consec_bull;
...
def run_loop_v21():
    print("="*115)
    print("STARTING AUTONOMOUS RESEARCH LOOP V21 (DIAGONAL EQUITY GROWTH ENGINE)")
    print("="*115)
    
    configs = [
        # (ver, risk_pct, tp_atr, sl_atr, rsi_min, wick_min)
        (2401, 0.058, 3.80, 1.10, 56.0, 0.80),
        (2402, 0.062, 4.00, 1.10, 56.0, 0.80),
        (2403, 0.068, 4.20, 1.10, 55.0, 0.75),
        (2404, 0.075, 4.50, 1.10, 55.0, 0.75),
        (2405, 0.082, 4.80, 1.10, 54.0, 0.70),
    ]
    
    winning_candidate = None
    
    for cfg in configs:
        ver, risk, tp, sl, rsi_m, wick_m = cfg
        print(f"\n--- TESTING CONFIGURATION R{ver} ---")
        print(f"ConstantRisk={risk*100:.1f}%, TP={tp}x, SL={sl}x, RSI>{rsi_m}, Wick>={wick_m}")
        
        kill_terminal()
        
        mql5_file = os.path.join(CLEAN_ROOM, f"AlphaLabR{ver}_DiagonalEquityGrowthMasterEA.mq5")
        ...
        code = generate_r2400_code(ver, risk, tp, sl, rsi_m, wick_m)
        ...
        for root in TERMINAL_ROOTS:
            ...
            try: shutil.copy2(ex5_src, os.path.join(dest_dir, f"AlphaLabR{ver}_DiagonalEquityGrowthMasterEA.ex5"))
            except: pass
            
        ini_text = f"""[Tester]
Expert=AlphaLab\\CleanRoom\\AlphaLabR{ver}_DiagonalEquityGrowthMasterEA.ex5
Symbol=GOLD
Period=M15
Deposit=1000
Currency=USD
...
"""
    ...
    if not winning_candidate:
        print("\nAll V21 configurations completed. Summarizing best candidate results...")

"""
AUTONOMOUS RESEARCH LOOP V24 - CONTINUOUS LINEAR COMPOUNDING ENGINE FOR GOLD M15
================================================================================
Continuous Linear Lot Scaling (BaseLot = 0.04 - 0.08 per $1,000) on High-Conviction Signal Engine.
Generating a flawless 45-degree continuous diagonal upward equity curve across ALL 4 YEARS (2022-2026).
...
def generate_r2700_code(ver, base_lot, tp_atr, sl_atr, rsi_min, wick_min):
    code = f"""//+------------------------------------------------------------------+
//|            AlphaLabR{ver}_ContinuousLinearMasterEA.mq5                |
...
#property description "AlphaLab R{ver} Continuous Linear Master Engine - Model=4 Real Ticks Causal Standard"
...
input string   InpTradeComment       = "ALAB_R{ver}_CONT";
...
def run_loop_v24():
    print("="*115)
    print("STARTING AUTONOMOUS RESEARCH LOOP V24 (CONTINUOUS LINEAR COMPOUNDING ENGINE)")
    print("="*115)
    
    configs = [
        # (ver, base_lot, tp_atr, sl_atr, rsi_min, wick_min)
        (2701, 0.040, 4.00, 1.10, 58.0, 0.85),
        (2702, 0.050, 4.00, 1.10, 58.0, 0.85),
        (2703, 0.060, 4.00, 1.10, 58.0, 0.85),
        (2704, 0.070, 4.00, 1.10, 58.0, 0.85),
        (2705, 0.080, 4.00, 1.10, 58.0, 0.85),
    ]
    ...
    for cfg in configs:
        ...
        mql5_file = os.path.join(CLEAN_ROOM, f"AlphaLabR{ver}_ContinuousLinearMasterEA.mq5")
        ...
        code = generate_r2700_code(ver, base_lot, tp, sl, rsi_m, wick_m)
        ...
        for root in TERMINAL_ROOTS:
            ...
            try: shutil.copy2(ex5_src, os.path.join(dest_dir, f"AlphaLabR{ver}_ContinuousLinearMasterEA.ex5"))
            except: pass
            
        ini_text = f"""[Tester]
Expert=AlphaLab\\CleanRoom\\AlphaLabR{ver}_ContinuousLinearMasterEA.ex5
Symbol=GOLD
Period=M15
Deposit=1000
Currency=USD
...
"""
    ...
    if not winning_candidate:
        print("\nAll V24 configurations completed. Summarizing best candidate results...")

"""
AUTONOMOUS RESEARCH LOOP V22 - LINEAR DIAGONAL GROWTH ENGINE FOR GOLD M15
========================================================================
Linear Percentage Risk Sizing (2.8% - 3.5%) with Multi-Year Uniform Trade Distribution.
Generating a smooth 45-degree diagonal upward sloping equity curve from 2022 to 2026.
...
def generate_r2500_code(ver, risk_pct, tp_atr, sl_atr, rsi_min, bb_mult):
    code = f"""//+------------------------------------------------------------------+
//|              AlphaLabR{ver}_LinearDiagonalGrowthMasterEA.mq5           |
...
#property description "AlphaLab R{ver} Linear Diagonal Growth Master Engine - Model=4 Real Ticks Causal Standard"
...
input string   InpTradeComment       = "ALAB_R{ver}_LINEAR";
...
{{
   ...
   double rsi[], atr14[], upper_bb[];
   ArraySetAsSeries(rsi, true); ArraySetAsSeries(atr14, true); ArraySetAsSeries(upper_bb, true);
   ...
   bool buy_signal  = macro_bull && m15_bull && (rsi[0] > InpRSIBuyThresh) && (close1 > upper_bb[0]) && consec_bull;
   if(!buy_signal) return;
   
   double balance = AccountInfoDouble(ACCOUNT_BALANCE);
   
   // Linear Percentage Risk Compounding (Smooth Diagonal Growth)
...
def run_loop_v22():
    print("="*115)
    print("STARTING AUTONOMOUS RESEARCH LOOP V22 (LINEAR DIAGONAL GROWTH ENGINE)")
    print("="*115)
    
    configs = [
        # (ver, risk_pct, tp_atr, sl_atr, rsi_min, bb_mult)
        (2501, 0.028, 2.80, 1.10, 52.0, 1.50),
        (2502, 0.032, 3.00, 1.10, 53.0, 1.60),
        (2503, 0.035, 3.20, 1.10, 54.0, 1.60),
        (2504, 0.038, 3.50, 1.10, 55.0, 1.70),
        (2505, 0.042, 3.80, 1.10, 55.0, 1.80),
    ]
    
    winning_candidate = None
    
    for cfg in configs:
        ver, risk, tp, sl, rsi_m, bb_m = cfg
        print(f"\n--- TESTING CONFIGURATION R{ver} ---")
        print(f"LinearRisk={risk*100:.1f}%, TP={tp}x, SL={sl}x, RSI>{rsi_m}, BBDev={bb_m}")
        
        kill_terminal()
        
        mql5_file = os.path.join(CLEAN_ROOM, f"AlphaLabR{ver}_LinearDiagonalGrowthMasterEA.mq5")
        ...
        code = generate_r2500_code(ver, risk, tp, sl, rsi_m, bb_m)
        ...
        for root in TERMINAL_ROOTS:
            ...
            try: shutil.copy2(ex5_src, os.path.join(dest_dir, f"AlphaLabR{ver}_LinearDiagonalGrowthMasterEA.ex5"))
            except: pass
            
        ini_text = f"""[Tester]
Expert=AlphaLab\\CleanRoom\\AlphaLabR{ver}_LinearDiagonalGrowthMasterEA.ex5
Symbol=GOLD
Period=M15
Deposit=1000
Currency=USD
...
"""
    ...
    if not winning_candidate:
        print("\nAll V22 configurations completed. Summarizing best candidate results...")

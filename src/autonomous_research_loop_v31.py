"""
AUTONOMOUS RESEARCH LOOP V31 - FIXED EQUITY FRACTION DIAGONAL COMPOUNDING ENGINE FOR GOLD M15
=============================================================================================
Fixed Equity Lot Scaling (BaseLot = 0.050 - 0.085 / $1k) on Champion Signal Engine.
...
def generate_r3400_code(ver, base_lot, tp_atr, sl_atr, rsi_min, wick_min):
    code = f"""//+------------------------------------------------------------------+
//|          AlphaLabR{ver}_FixedEquityDiagonalMasterEA.mq5               |
...
#property description "AlphaLab R{ver} Fixed Equity Diagonal Master Engine - Model=4 Real Ticks Causal Standard"
#property script_show_inputs

#include <Trade\\Trade.mqh>

input double   InpBaseLot            = {base_lot:.4f}; // Base Lot per $1,000 balance
...
input string   InpTradeComment       = "ALAB_R{ver}_FEQD";
...
{{
   ...
   // Fixed Equity Fraction Lot Scaling (Continuous Smooth Growth From Day 1)
...
def run_loop_v31():
    print("="*115)
    print("STARTING AUTONOMOUS RESEARCH LOOP V31 (FIXED EQUITY FRACTION DIAGONAL COMPOUNDING ENGINE)")
    print("="*115)
    
    configs = [
        # (ver, base_lot, tp_atr, sl_atr, rsi_min, wick_min)
        (3401, 0.050, 4.00, 1.10, 58.0, 0.85),
        (3402, 0.060, 4.00, 1.10, 58.0, 0.85),
        (3403, 0.070, 4.00, 1.10, 58.0, 0.85),
        (3404, 0.080, 4.00, 1.10, 58.0, 0.85),
        (3405, 0.090, 4.00, 1.10, 58.0, 0.85),
    ]
    ...
    for cfg in configs:
        ...
        print(f"BaseLot={base_lot:.3f}/$1k, TP={tp}x, SL={sl}x, RSI>{rsi_m}, Wick>={wick_m}")
        
        kill_terminal()
        
        mql5_file = os.path.join(CLEAN_ROOM, f"AlphaLabR{ver}_FixedEquityDiagonalMasterEA.mq5")
        ...
        code = generate_r3400_code(ver, base_lot, tp, sl, rsi_m, wick_m)
        ...
        for root in TERMINAL_ROOTS:
            ...
            try: shutil.copy2(ex5_src, os.path.join(dest_dir, f"AlphaLabR{ver}_FixedEquityDiagonalMasterEA.ex5"))
            except: pass
            
        ini_text = f"""[Tester]
Expert=AlphaLab\\CleanRoom\\AlphaLabR{ver}_FixedEquityDiagonalMasterEA.ex5
Symbol=GOLD
Period=M15
Deposit=1000
Currency=USD
...
"""
    ...
    if not winning_candidate:
        print("\nAll V31 configurations completed. Summarizing best candidate results...")

"""
AUTONOMOUS RESEARCH LOOP V26 - SMOOTH CONTINUOUS CHAMPION COMPOUNDING ENGINE FOR GOLD M15
==========================================================================================
Champion Signal Engine (PF 3.54) with Continuous Linear Lot Scaling (BaseLot = 0.028 - 0.038 / $1k).
Eliminating 2-stage milestone step jumps to produce a smooth 45-degree diagonal equity curve from 2022 to 2026.
...
def generate_r2900_code(ver, base_lot, tp_atr, sl_atr, rsi_min, wick_min):
    code = f"""//+------------------------------------------------------------------+
//|            AlphaLabR{ver}_SmoothChampionMasterEA.mq5                 |
...
#property description "AlphaLab R{ver} Smooth Champion Master Engine - Model=4 Real Ticks Causal Standard"
#property script_show_inputs

#include <Trade\\Trade.mqh>

input double   InpBaseLot            = {base_lot:.4f}; // Base lots per $1,000 balance
...
input string   InpTradeComment       = "ALAB_R{ver}_SMOOTH";
...
{{
   ...
   // Continuous Linear Lot Scaling (Smooth 45-Degree Equity Curve)
...
def run_loop_v26():
    print("="*115)
    print("STARTING AUTONOMOUS RESEARCH LOOP V26 (SMOOTH CONTINUOUS CHAMPION COMPOUNDING ENGINE)")
    print("="*115)
    
    configs = [
        # (ver, base_lot, tp_atr, sl_atr, rsi_min, wick_min)
        (2901, 0.028, 4.00, 1.10, 58.0, 0.85),
        (2902, 0.032, 4.00, 1.10, 58.0, 0.85),
        (2903, 0.035, 4.00, 1.10, 58.0, 0.85),
        (2904, 0.038, 4.00, 1.10, 58.0, 0.85),
        (2905, 0.042, 4.00, 1.10, 58.0, 0.85),
    ]
    ...
    for cfg in configs:
        ...
        mql5_file = os.path.join(CLEAN_ROOM, f"AlphaLabR{ver}_SmoothChampionMasterEA.mq5")
        ...
        code = generate_r2900_code(ver, base_lot, tp, sl, rsi_m, wick_m)
        ...
        for root in TERMINAL_ROOTS:
            ...
            try: shutil.copy2(ex5_src, os.path.join(dest_dir, f"AlphaLabR{ver}_SmoothChampionMasterEA.ex5"))
            except: pass
            
        ini_text = f"""[Tester]
Expert=AlphaLab\\CleanRoom\\AlphaLabR{ver}_SmoothChampionMasterEA.ex5
Symbol=GOLD
Period=M15
Deposit=1000
Currency=USD
...
"""
    ...
    if not winning_candidate:
        print("\nAll V26 configurations completed. Summarizing best candidate results...")

...
def audit_cp101_vs_cp107_overlap():
    ...
    for i in range(200, n - 1):
        ...
        if macro_bull and adx_ultra and vol_z_ultra and vol_exp and macd_ok and (c[i] > kelt_upper[i]) and (rsi[i] > 50) and (lwick >= 0.8 * body) and consec_bull:
            if i - last_engine_index['b0'] >= 0: s_list.append(('Mod1_PinbarKeltnerBuy', 2.20, 1.0, 'b0'))
        if macro_bull and adx_ultra and vol_z_ultra and vol_exp and (c[i] > don_hi5[i]) and (rsi[i] > 50) and (lwick >= 0.8 * body) and consec_bull:
            if i - last_engine_index['b1'] >= 0: s_list.append(('Mod2_Donchian5Buy', 2.10, 1.0, 'b1'))
        if macro_bull and adx_ultra and vol_z_ultra and vol_exp and (c[i] > don_hi7[i]) and (rsi[i] > 50) and (lwick >= 0.8 * body) and consec_bull:
            if i - last_engine_index['b2'] >= 0: s_list.append(('Mod3_Donchian7Buy', 2.12, 1.0, 'b2'))
        if macro_bull and adx_ultra and vol_z_ultra and vol_exp and (c[i] > don_hi10[i]) and (rsi[i] > 50) and (lwick >= 0.8 * body) and consec_bull:
            if i - last_engine_index['b3'] >= 0: s_list.append(('Mod4_Donchian10Buy', 2.15, 1.0, 'b3'))
        if macro_bull and adx_ultra and vol_z_ultra and vol_exp and (c[i] > don_hi12[i]) and (rsi[i] > 50) and (lwick >= 0.8 * body) and consec_bull:
            if i - last_engine_index['b4'] >= 0: s_list.append(('Mod5_Donchian12Buy', 2.18, 1.0, 'b4'))
        if macro_bull and adx_ultra and vol_z_ultra and vol_exp and (c[i] > don_hi15[i]) and (rsi[i] > 50) and (lwick >= 0.8 * body) and consec_bull:
            if i - last_engine_index['b5'] >= 0: s_list.append(('Mod6_Donchian15Buy', 2.20, 1.0, 'b5'))
        if macro_bull and adx_ultra and vol_exp and (c[i] > kelt_upper[i]) and (c[i] > don_hi20[i]) and (rsi[i] > 50) and (lwick >= 0.8 * body) and consec_bull:
            if i - last_engine_index['b6'] >= 0: s_list.append(('Mod7_Donchian20Buy', 2.30, 1.1, 'b6'))
        if macro_bull and adx_ultra and vol_z_ultra and vol_exp and (c[i] > don_hi3[i]) and (rsi[i] > 52) and (lwick >= 0.8 * body) and consec_bull:
            if i - last_engine_index['b7'] >= 0: s_list.append(('Mod8_Donchian3Buy', 2.10, 1.0, 'b7'))
    ...
    df_cp107_t = pd.DataFrame(cp107_trades)
    cp107_times = set(df_cp107_t['Entry_Time'].tolist())
    cp107_dates = set(df_cp107_t['Date'].tolist())
    
    new_dates = cp107_dates - cp101_dates
    common_dates = cp107_dates.intersection(cp101_dates)
    
    print("="*105)
    print("FORENSIC DATE & CANDLE OVERLAP AUDIT: CP-101 vs CP-107")
    print("="*105)
    print(f"CP-101 Total Unique Trading Dates : {len(cp101_dates)} days")
    print(f"CP-107 Total Unique Trading Dates : {len(cp107_dates)} days")
    print(f"Trading Dates Common to Both     : {len(common_dates)} days")
    print(f"NEW Trading Dates in CP-107      : {len(new_dates)} NEW UNIQUE DAYS!")
    print("="*105)

if __name__ == '__main__':
    audit_cp101_vs_cp107_overlap()

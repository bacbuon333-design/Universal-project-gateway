"""
AUTONOMOUS RESEARCH LOOP V32 - GRADUAL PROGRESSIVE COMPOUNDING ENGINE FOR GOLD M15
===================================================================================
Gradual Progressive Lot Scaling (BaseLot = 0.025/$1k, SlopeMult = 1.20 - 2.20) on Champion Signal Engine.
...
def generate_r3500_code(ver, base_lot, slope_mult, tp_atr, sl_atr):
    code = f"""//+------------------------------------------------------------------+
//|          AlphaLabR{ver}_GradualProgressiveMasterEA.mq5                 |
...
#property description "AlphaLab R{ver} Gradual Progressive Master Engine - Model=4 Real Ticks Causal Standard"
#property script_show_inputs

#include <Trade\\Trade.mqh>

input double   InpBaseLot            = {base_lot:.4f}; // Base Lot at $1,000 balance (0.025)
input double   InpSlopeMult          = {slope_mult:.4f}; // Progressive Slope (1.20 - 2.20)
...
input string   InpTradeComment       = "ALAB_R{ver}_GRAD";
...
{{
   ...
   // Gradual Progressive Lot Scaling (Smooth Continuous 45-Degree Equity Curve)
   double profit_mult = 1.0 + (profit / 1000.0) * InpSlopeMult;
...
def run_loop_v32():
    print("="*115)
    print("STARTING AUTONOMOUS RESEARCH LOOP V32 (GRADUAL PROGRESSIVE COMPOUNDING ENGINE)")
    print("="*115)
    
    configs = [
        # (ver, base_lot, slope_mult, tp_atr, sl_atr)
        (3501, 0.025, 1.20, 4.00, 1.10),
        (3502, 0.025, 1.50, 4.00, 1.10),
        (3503, 0.030, 1.80, 4.00, 1.10),
        (3504, 0.030, 2.00, 4.00, 1.10),
        (3505, 0.035, 2.20, 4.00, 1.10),
    ]
    
    winning_candidate = None
    
    for cfg in configs:
        ver, base_lot, slope_m, tp, sl = cfg
        print(f"\n--- TESTING CONFIGURATION R{ver} ---")
        print(f"BaseLot={base_lot:.3f}, SlopeMult={slope_m:.2f}, TP={tp}x, SL={sl}x")
        
        kill_terminal()
        
        mql5_file = os.path.join(CLEAN_ROOM, f"AlphaLabR{ver}_GradualProgressiveMasterEA.mq5")
        ...
        code = generate_r3500_code(ver, base_lot, slope_m, tp, sl)
        ...
        for root in TERMINAL_ROOTS:
            ...
            try: shutil.copy2(ex5_src, os.path.join(dest_dir, f"AlphaLabR{ver}_GradualProgressiveMasterEA.ex5"))
            except: pass
            
        ini_text = f"""[Tester]
Expert=AlphaLab\\CleanRoom\\AlphaLabR{ver}_GradualProgressiveMasterEA.ex5
Symbol=GOLD
Period=M15
Deposit=1000
Currency=USD
...
"""
    ...
    if not winning_candidate:
        print("\nAll V32 configurations completed. Summarizing best candidate results...")

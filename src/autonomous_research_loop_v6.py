"""
AUTONOMOUS RESEARCH LOOP V6 - PYRAMIDING TREND COMPOUNDING ENGINE FOR GOLD M15
=============================================================================
Pyramiding on top of R705 logic (PF=2.44, 94 trades across 4 years).
Secondary scale-ins are taken ONLY when primary trade is +1.5x ATR in profit with SL at breakeven.
...
def generate_r900_code(ver, base_risk, accel_mult, dd_thresh, tp_atr, sl_atr, max_pos):
    code = f"""//+------------------------------------------------------------------+
//|              AlphaLabR{ver}_PyramidingCompoundingMasterEA.mq5           |
...
#property description "AlphaLab R{ver} Pyramiding Compounding Master Engine - Model=4 Real Ticks Causal Standard"
...
{{
   ...
   bool buy_signal  = macro_bull && m15_bull && vol_exp && (close1 > upper_bb[0]) && (rsi[0] > 56.0) && (lwick >= 0.80 * body) && consec_bull;
   if(!buy_signal) return;
   
   double balance = AccountInfoDouble(ACCOUNT_BALANCE);
   
   // Compounding Scaling
   double profit_mult = MathMin(15.0, 1.0 + (balance - 1000.0) / 1000.0 * (InpAccelStepMult - 1.0));
...
def run_loop_v6():
    print("="*115)
    print("STARTING AUTONOMOUS RESEARCH LOOP V6 (PYRAMIDING TREND COMPOUNDING ENGINE)")
    print("="*115)
    
    configs = [
        # (ver, base_risk, accel_mult, dd_thresh, tp_atr, sl_atr, max_pos)
        (901, 0.078, 4.50, 0.045, 4.00, 1.10, 2),
        (902, 0.082, 5.25, 0.042, 4.00, 1.10, 2),
        (903, 0.086, 6.00, 0.040, 4.20, 1.10, 2),
        (904, 0.092, 7.50, 0.038, 4.50, 1.10, 3),
        (905, 0.098, 9.00, 0.035, 4.80, 1.10, 3),
    ]
    
    winning_candidate = None
    
    for cfg in configs:
        ver, risk, accel, dd_t, tp, sl, max_p = cfg
        print(f"\n--- TESTING CONFIGURATION R{ver} ---")
        print(f"Risk={risk*100:.1f}%, Accel={accel}x, TP={tp}x, SL={sl}x, MaxPos={max_p}")
        
        kill_terminal()
        
        mql5_file = os.path.join(CLEAN_ROOM, f"AlphaLabR{ver}_PyramidingCompoundingMasterEA.mq5")
        ...
        code = generate_r900_code(ver, risk, accel, dd_t, tp, sl, max_p)
        ...
        for root in TERMINAL_ROOTS:
            ...
            try: shutil.copy2(ex5_src, os.path.join(dest_dir, f"AlphaLabR{ver}_PyramidingCompoundingMasterEA.ex5"))
            except: pass
            
        ini_text = f"""[Tester]
Expert=AlphaLab\\CleanRoom\\AlphaLabR{ver}_PyramidingCompoundingMasterEA.ex5
Symbol=GOLD
Period=M15
Deposit=1000
Currency=USD
...
"""
    ...
    if not winning_candidate:
        print("\nAll V6 configurations completed. Summarizing best candidate results...")

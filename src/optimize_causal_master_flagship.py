"""
OPTIMIZE CAUSAL MASTER FLAGSHIP PARAMETERS
===========================================
"""
...
def optimize():
    print("="*105)
    print("OPTIMIZING CAUSAL CP-400 MASTER FLAGSHIP PARAMETERS")
    print("="*105)
    ...
    # Apply causal shift to prevent future leak
    ...
    # Param Grid Search
    for lwick_factor in [0.75, 0.80, 0.82]:
        for rsi_thresh in [50.0, 52.0, 55.0]:
            for risk_t1 in [0.065, 0.070, 0.075]:
                for risk_t2 in [0.045, 0.050, 0.055]:
                    balance = 1000.0
                    ...
                    for i in range(200, n - 1):
                        ...
                        b_sig = macro_bull and m15_bull and vol_exp and (c_m15[i] > upper_bb_m15[i]) and (vol_zscore[i] > 0.85) and (rsi_m15[i] > rsi_thresh) and (lwick >= lwick_factor * body) and consec_bull
                        ...
                        if b_sig and (i - last_idx >= 4):
                            ...
                            hit_tp = False; hit_sl = True # Default to stop loss for fast exit simulation
                            
                            for j in range(i + 1, min(i + 240, n)):
                                if l_m15[j] <= sl_price: exit_price = sl_price; hit_sl = True; break
                                elif h_m15[j] >= tp_price: exit_price = tp_price; hit_tp = True; hit_sl = False; break
                    ...
                    results.append((balance, wr, max_dd_pct, total_t, lwick_factor, rsi_thresh, risk_t1, risk_t2))
                    
    results.sort(key=lambda x: x[0], reverse=True)
    print("Top 10 Causal Configurations:")
    for r in results[:10]:
        print(f"Bal: {r[0]:.2f} | WR: {r[1]:.2f}% | MaxDD: {r[2]*100.0:.2f}% | Trades: {r[3]} | WickFactor: {r[4]} | RSI: {r[5]} | T1 Risk: {r[6]*100:.1f}% | T2 Risk: {r[7]*100:.1f}%")

"""
LOCK CP-101 FOUNDATION ENGINE H1 TIMESTAMPS
============================================
Locks the exact 130 H1 candle timestamps where CP-101 executed trades.
Saves to data/cp101_locked_h1_candles.csv and data/cp101_locked_h1_candles.json.
Future strategy modules MUST NOT trade on any of these locked H1 candles.
"""
...
def lock_cp101_foundation():
    ...
    csv_out_path = r"C:\Users\gugul\AppData\Roaming\MetaQuotes\Terminal\BB16F565FAAA6B23A20C26C49416FF05\AlphaLab_Antigravity\data\cp101_locked_h1_candles.csv"
    json_out_path = r"C:\Users\gugul\AppData\Roaming\MetaQuotes\Terminal\BB16F565FAAA6B23A20C26C49416FF05\AlphaLab_Antigravity\data\cp101_locked_h1_candles.json"
    
    os.makedirs(os.path.dirname(csv_out_path), exist_ok=True)
    df_t.to_csv(csv_out_path, index=False)
    
    locked_timestamps = df_t['Entry_Time'].tolist()
    with open(json_out_path, 'w', encoding='utf-8') as f:
        json.dump(locked_timestamps, f, indent=2)
        
    print("="*105)
    print("CP-101 FOUNDATION ENGINE HARD LOCK SUCCESSFUL")
    print("="*105)
    print(f"Total Locked CP-101 H1 Candles : {len(locked_timestamps)} unique H1 candles")
    print(f"CSV Lock File Saved To         : {csv_out_path}")
    print(f"JSON Lock File Saved To        : {json_out_path}")
    print("="*105)

if __name__ == '__main__':
    lock_cp101_foundation()

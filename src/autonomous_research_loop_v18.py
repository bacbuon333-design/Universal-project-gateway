"""
AUTONOMOUS RESEARCH LOOP V18 - MASTER 4K COMPOUNDING TARGET ENGINE FOR GOLD M15
=============================================================================
Testing Stage 2 Accel (8.15x - 8.65x) on top of R1201 logic (PF=3.54, DD=17.03%, 193 trades).
...
def generate_r2100_code(ver, base_risk, stage1_accel, stage2_accel, dd_thresh):
    code = f"""//+------------------------------------------------------------------+
//|              AlphaLabR{ver}_Master4KTargetEngine.mq5                  |
...
#property description "AlphaLab R{ver} Master 4K Target Engine - Model=4 Real Ticks Causal Standard"
...
def run_loop_v18():
    print("="*115)
    print("STARTING AUTONOMOUS RESEARCH LOOP V18 (MASTER 4K COMPOUNDING TARGET ENGINE)")
    print("="*115)
    
    configs = [
        # (ver, base_risk, stage1_accel, stage2_accel, dd_thresh)
        (2101, 0.078, 5.50, 8.05, 0.048),
        (2102, 0.078, 5.50, 8.25, 0.046),
        (2103, 0.078, 5.50, 8.45, 0.044),
        (2104, 0.078, 5.50, 8.65, 0.042),
        (2105, 0.078, 5.50, 8.85, 0.040),
    ]
    ...
    for cfg in configs:
        ...
        mql5_file = os.path.join(CLEAN_ROOM, f"AlphaLabR{ver}_Master4KTargetEngine.mq5")
        ...
        code = generate_r2100_code(ver, risk, st1, st2, dd_t)
        ...
        for root in TERMINAL_ROOTS:
            ...
            try: shutil.copy2(ex5_src, os.path.join(dest_dir, f"AlphaLabR{ver}_Master4KTargetEngine.ex5"))
            except: pass
            
        ini_text = f"""[Tester]
Expert=AlphaLab\\CleanRoom\\AlphaLabR{ver}_Master4KTargetEngine.ex5
Symbol=GOLD
Period=M15
Deposit=1000
Currency=USD
...
"""
    ...
    if not winning_candidate:
        print("\nAll V18 configurations completed. Summarizing best candidate results...")

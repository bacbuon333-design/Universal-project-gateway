"""
AUTONOMOUS RESEARCH LOOP V19 - STEADY EQUITY GROWTH ENGINE FOR GOLD M15
========================================================================
Integrating MT5 Free Robots Logic (Candlestick Patterns + RSI + MA Trend + BUY & SELL Directional Symmetry).
Using Fixed Risk Compounding (3.5% - 5.5%) to ensure a SMOOTH, STEADY UPWARD SLOPING equity curve across ALL 4 YEARS (2022-2026).
Target: Net Profit > $4,000.00 USD, PF >= 1.50, Max DD <= 20.00%, Steady Growth, Model=4 Real Ticks.
"""
...
def generate_r2200_code(ver, risk_pct, tp_atr, sl_atr, rsi_buy, rsi_sell):
    code = f"""//+------------------------------------------------------------------+
//|              AlphaLabR{ver}_SteadyEquityGrowthMasterEA.mq5             |
...
#property description "AlphaLab R{ver} Steady Equity Growth Master Engine - Free Robots Pattern Standard"
#property script_show_inputs

#include <Trade\\Trade.mqh>

input double   InpRiskPct            = {risk_pct:.4f}; // Risk per trade (% of balance)
input double   InpTakeProfitATRMult  = {tp_atr:.4f};
input double   InpStopLossATRMult    = {sl_atr:.4f};
input double   InpRSIBuyThresh       = {rsi_buy:.1f};
input double   InpRSISellThresh      = {rsi_sell:.1f};
input int      InpMaxPositions       = 1;
input double   InpMaxDrawdownLimit   = 0.200;
input ulong    InpMagicNumber        = 202600000 + {ver};
input string   InpTradeComment       = "ALAB_R{ver}_STEADY";
...
int h_ema9_m15, h_ema20_m15, h_rsi_m15, h_atr14_m15, h_atr50_m15;
...
{{
   ...
   IndicatorRelease(h_rsi_m15); IndicatorRelease(h_atr14_m15); IndicatorRelease(h_atr50_m15);
}}
   ...
   {{
      ...
      {{
         ...
         {{
            ...
            if(open_price - bid >= 1.20 * av && (current_sl > open_price || current_sl == 0))
            {{
               double new_sl = NormalizeDouble(open_price - 0.10, _Digits);
               trade.PositionModify(ticket, new_sl, current_tp);
            }}
...
{{
   ...
   double rsi[], atr14[], atr50[];
   ArraySetAsSeries(rsi, true); ArraySetAsSeries(atr14, true); ArraySetAsSeries(atr50, true);
   ...
   double body2  = MathAbs(close2 - open2) + 1e-5;
   
   // Candlestick Pattern Logic (Free Robots Engulfing & Momentum)
   bool bull_engulf = (close2 < open2) && (close1 > open1) && (close1 >= open2) && (open1 <= close2);
   bool bear_engulf = (close2 > open2) && (close1 < open1) && (close1 <= open2) && (open1 >= close2);
   
   bool macro_bull  = (ema9_h1[0] > ema20_h1[0]);
   bool macro_bear  = (ema9_h1[0] < ema20_h1[0]);
   bool m15_bull    = (ema9_m15[0] > ema20_m15[0]);
   bool m15_bear    = (ema9_m15[0] < ema20_m15[0]);
   
   bool buy_signal  = macro_bull && m15_bull && (rsi[0] > InpRSIBuyThresh) && (bull_engulf || (close1 > open1 && close2 > open2));
   bool sell_signal = macro_bear && m15_bear && (rsi[0] < InpRSISellThresh) && (bear_engulf || (close1 < open1 && close2 < open2));
...
def run_loop_v19():
    print("="*115)
    print("STARTING AUTONOMOUS RESEARCH LOOP V19 (STEADY EQUITY GROWTH ENGINE)")
    print("="*115)
    
    configs = [
        # (ver, risk_pct, tp_atr, sl_atr, rsi_buy, rsi_sell)
        (2201, 0.045, 2.50, 1.20, 52.0, 48.0),
        (2202, 0.050, 2.80, 1.20, 53.0, 47.0),
        (2203, 0.055, 3.00, 1.20, 54.0, 46.0),
        (2204, 0.060, 3.20, 1.20, 55.0, 45.0),
        (2205, 0.065, 3.50, 1.20, 55.0, 45.0),
    ]
    
    winning_candidate = None
    
    for cfg in configs:
        ver, risk, tp, sl, rsi_b, rsi_s = cfg
        print(f"\n--- TESTING CONFIGURATION R{ver} ---")
        print(f"FixedRisk={risk*100:.1f}%, TP={tp}x, SL={sl}x, RSIBuy>{rsi_b}, RSISell<{rsi_s}")
        
        kill_terminal()
        
        mql5_file = os.path.join(CLEAN_ROOM, f"AlphaLabR{ver}_SteadyEquityGrowthMasterEA.mq5")
        ...
        code = generate_r2200_code(ver, risk, tp, sl, rsi_b, rsi_s)
        ...
        for root in TERMINAL_ROOTS:
            ...
            try: shutil.copy2(ex5_src, os.path.join(dest_dir, f"AlphaLabR{ver}_SteadyEquityGrowthMasterEA.ex5"))
            except: pass
            
        ini_text = f"""[Tester]
Expert=AlphaLab\\CleanRoom\\AlphaLabR{ver}_SteadyEquityGrowthMasterEA.ex5
Symbol=GOLD
Period=M15
Deposit=1000
Currency=USD
...
"""
    ...
    if not winning_candidate:
        print("\nAll V19 configurations completed. Summarizing best candidate results...")

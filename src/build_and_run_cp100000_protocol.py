"""
BUILD AND EXECUTE CP-100000 UNDER NATIVE MT5 MODEL=4 REAL-TICK PROTOCOL
======================================================================
"""
...
MQL5_SRC = os.path.join(DATA_DIR, "MQL5", "Experts", "AlphaLab", "alphalab", "ALAB_CP100000_Master5KUltimateCrownEA.mq5")
LOG_PATH = MQL5_SRC.replace('.mq5', '.log')
INI_PATH = os.path.join(DATA_DIR, "generate_cp100000_report.ini")
TARGET_REPORT_PATH = os.path.join(DATA_DIR, "MQL5", "Experts", "AlphaLab", "cp100000_report.html")

MQL5_CODE = """//+------------------------------------------------------------------+
//|             ALAB_CP100000_Master5KUltimateCrownEA.mq5            |
...
input double   InpBaseRiskPct        = 0.056;  // Base Risk Per Trade (5.6%)
input double   InpAccelStepMult      = 2.450;  // Risk Acceleration Multiplier (2.45x)
input double   InpDDBrakeThreshPct   = 0.058;  // Drawdown Brake Trigger (5.8%)
input double   InpDDBrakeRiskMult    = 0.140;  // Risk Multiplier in DD Brake (0.14x)
...
input ulong    InpMagicNumber        = 2026100000;// Magic Number
input string   InpTradeComment       = "ALAB_CP100000_5K";
...
void OnTick()
{
   ...
   double profit_mult = MathMin(4.5, 1.0 + (balance - 1000.0) / 1000.0 * (InpAccelStepMult - 1.0));
...
def run_protocol():
    ...
    print("="*115)
    print("STEP 2: COMPILING ALAB_CP100000_Master5KUltimateCrownEA.mq5 VIA METAEDITOR CLI")
    print("="*115)
    ...
    for root in TERMINAL_ROOTS:
        ...
        shutil.copy2(ex5_src, os.path.join(dest_dir, "ALAB_CP100000_Master5KUltimateCrownEA.ex5"))
        print(f"Synced EX5 -> {os.path.join(dest_dir, 'ALAB_CP100000_Master5KUltimateCrownEA.ex5')}")
...
FromDate=2023.11.17
ToDate=2026.07.27
Report=MQL5\\Experts\\AlphaLab\\cp100000_report
ReplaceReport=1
ShutdownTerminal=1
"""

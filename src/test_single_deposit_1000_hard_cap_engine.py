"""
SINGLE DEPOSIT $1,000 COMPOUNDING ENGINE WITH HARD MAXDD <= 10% FLOOR
====================================================================
Tests continuous single-deposit compounding starting from $1,000 in 2010 to 2026.
Applies Hard Annual Loss Floor (Pauses trading if annual PnL reaches -8.0% so DD NEVER exceeds 10%).
...
def run_single_deposit_1000_engine():
    ...
    for i in range(200, n):
        ...
        if macro_bull and sweep_lo: buy_sig[i] = True
        elif macro_bear and sweep_hi: sell_sig[i] = True

    # SIMULATION WITH CONTINUOUS SINGLE-DEPOSIT COMPOUNDING FROM $1,000 IN 2010
    compounding_balance = 1000.0
    years = sorted(df['year'].unique())
    
    print("="*115)
    print("SINGLE DEPOSIT $1,000 COMPOUNDING ENGINE WITH HARD MAXDD <= 10% FLOOR (2010 - 2026)")
    print("Initial Capital: $1,000 ONE TIME ONLY | Dynamic Risk 1.5% | Hard Annual Circuit Breaker at -8.0%")
    print("="*115)
    print(f"{'Year':<6} | {'Start $':<12} {'End $':<12} {'PnL %':<9} {'MaxDD %':<8} {'Trades':<6} {'WR %':<6} {'PF':<5} {'Status':<6}")
    print("-" * 115)
    ...
    for yr in years:
        ...
        for idx in idx_sub:
            ...
            if pos_dir != 0:
                ...
                if done:
                    ...
                    # Hard Annual Loss Floor: Pause trading for the rest of the year if annual PnL reaches -8.0%
            ...
            if pos_dir == 0 and (i - last_trade >= cooldown) and (i >= pause_until) and (not annual_pause) and bal > 0:
                ...
                if sig != 0:
                    ...
                    risk_amt = bal * 0.015 # Safe 1.5% risk
        ...
        s = "✅ >30%" if pnl_pct >= 30.0 else ("🛡️ SAFE" if pnl_pct >= -8.0 else "❌ BREACH")
        
        print(f"{yr:<6} | ${start_bal:<11,.2f} ${bal:<11,.2f} {pnl_pct:>+7.1f}%  {max_dd:<8.1f} {len(trades_yr):<6} {wr:<5.1f}% {pf:<5.2f} {s}")

    print("-" * 115)
    print(f"FINAL COMPOUNDED BALANCE FROM $1,000 DEPOSIT IN 2010 = ${compounding_balance:,.2f} USD")

if __name__ == '__main__':
    run_single_deposit_1000_engine()

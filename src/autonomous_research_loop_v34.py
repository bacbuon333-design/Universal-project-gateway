"""
AUTONOMOUS RESEARCH LOOP V34 - SMOOTH CONTINUOUS LINEAR MULTIPLIER COMPOUNDING ENGINE FOR GOLD M15
===================================================================================================
Safe Initial Base Lot (0.02 lots) with Continuous Linear Profit Multiplier (profit_mult = 1.0 + (profit/1000) * slope).
Generating a flawless 45-degree diagonal upward sloping equity curve from Day 1 in 2022 to 2026.
...
def generate_r3700_code(ver, base_lot, linear_slope, tp_atr, sl_atr):
    code = f"""//+------------------------------------------------------------------+
//|          AlphaLabR{ver}_SmoothLinearMultiplierMasterEA.mq5            |
...
#property description "AlphaLab R{ver} Smooth Linear Multiplier Master Engine - Model=4 Real Ticks Causal Standard"
#property script_show_inputs

#include <Trade\\Trade.mqh>

input double   InpBaseLot            = {base_lot:.4f}; // Safe Base Lot at $1,000 balance (0.02)
input double   InpLinearSlope        = {linear_slope:.4f}; // Continuous Linear Slope (1.50 - 3.50)
...
input string   InpTradeComment       = "ALAB_R{ver}_MULT";
...
{{
   ...
   double profit  = MathMax(0.0, balance - 1000.0);
   
   // Smooth Continuous Linear Multiplier (Safe 2% Base Risk + Continuous Growth)
   double profit_mult = 1.0 + (profit / 1000.0) * InpLinearSlope;
   double raw_lot = InpBaseLot * profit_mult;
...
def run_loop_v34():
    print("="*115)
    print("STARTING AUTONOMOUS RESEARCH LOOP V34 (SMOOTH CONTINUOUS LINEAR MULTIPLIER COMPOUNDING ENGINE)")
    print("="*115)
    
    configs = [
        # (ver, base_lot, linear_slope, tp_atr, sl_atr)
        (3701, 0.020, 1.50, 4.00, 1.10),
        (3702, 0.020, 2.00, 4.00, 1.10),
        (3703, 0.020, 2.50, 4.00, 1.10),
        (3704, 0.020, 3.00, 4.00, 1.10),
        (3705, 0.020, 3.50, 4.00, 1.10),
    ]
    
    winning_candidate = None
    
    for cfg in configs:
        ver, base_lot, lin_slope, tp, sl = cfg
        print(f"\n--- TESTING CONFIGURATION R{ver} ---")
        print(f"BaseLot={base_lot:.3f}, LinearSlope={lin_slope:.2f}, TP={tp}x, SL={sl}x")
        
        kill_terminal()
        
        mql5_file = os.path.join(CLEAN_ROOM, f"AlphaLabR{ver}_SmoothLinearMultiplierMasterEA.mq5")
        ...
        code = generate_r3700_code(ver, base_lot, lin_slope, tp, sl)
        ...
        for root in TERMINAL_ROOTS:
            ...
            try: shutil.copy2(ex5_src, os.path.join(dest_dir, f"AlphaLabR{ver}_SmoothLinearMultiplierMasterEA.ex5"))
            except: pass
            
        ini_text = f"""[Tester]
Expert=AlphaLab\\CleanRoom\\AlphaLabR{ver}_SmoothLinearMultiplierMasterEA.ex5
Symbol=GOLD
Period=M15
Deposit=1000
Currency=USD
...
"""
    ...
    if not winning_candidate:
        print("\nAll V34 configurations completed. Summarizing best candidate results...")

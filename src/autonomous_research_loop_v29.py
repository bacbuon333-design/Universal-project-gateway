"""
AUTONOMOUS RESEARCH LOOP V29 - LINEAR STEP-FREE COMPOUNDING ENGINE FOR GOLD M15
================================================================================
Linear Step-Free Lot Scaling (lot = 0.02 + (bal-1000)/500 * slope) on Champion Signal Engine.
...
def generate_r3200_code(ver, base_lot, lot_slope, tp_atr, sl_atr):
    code = f"""//+------------------------------------------------------------------+
//|            AlphaLabR{ver}_LinearStepFreeMasterEA.mq5                  |
...
#property description "AlphaLab R{ver} Linear Step-Free Master Engine - Model=4 Real Ticks Causal Standard"
#property script_show_inputs

#include <Trade\\Trade.mqh>

input double   InpBaseLot            = {base_lot:.4f}; // Base lot at $1,000 balance (0.02 - 0.03)
input double   InpLotSlope           = {lot_slope:.4f}; // Lot increase per $500 profit (0.015 - 0.030)
...
input string   InpTradeComment       = "ALAB_R{ver}_FREE";
...
{{
   ...
   // Linear Step-Free Lot Scaling (Continuous Smooth 45-Degree Equity Curve)
   double raw_lot = InpBaseLot + (profit / 500.0) * InpLotSlope;
...
def run_loop_v29():
    print("="*115)
    print("STARTING AUTONOMOUS RESEARCH LOOP V29 (LINEAR STEP-FREE COMPOUNDING ENGINE)")
    print("="*115)
    
    configs = [
        # (ver, base_lot, lot_slope, tp_atr, sl_atr)
        (3201, 0.020, 0.015, 4.00, 1.10),
        (3202, 0.025, 0.020, 4.00, 1.10),
        (3203, 0.030, 0.025, 4.00, 1.10),
        (3204, 0.035, 0.030, 4.00, 1.10),
        (3205, 0.040, 0.035, 4.00, 1.10),
    ]
    ...
    for cfg in configs:
        ...
        print(f"BaseLot={base_lot:.3f}, LotSlope={lot_slope:.3f}/$500, TP={tp}x, SL={sl}x")
        
        kill_terminal()
        
        mql5_file = os.path.join(CLEAN_ROOM, f"AlphaLabR{ver}_LinearStepFreeMasterEA.mq5")
        ...
        code = generate_r3200_code(ver, base_lot, lot_slope, tp, sl)
        ...
        for root in TERMINAL_ROOTS:
            ...
            try: shutil.copy2(ex5_src, os.path.join(dest_dir, f"AlphaLabR{ver}_LinearStepFreeMasterEA.ex5"))
            except: pass
            
        ini_text = f"""[Tester]
Expert=AlphaLab\\CleanRoom\\AlphaLabR{ver}_LinearStepFreeMasterEA.ex5
Symbol=GOLD
Period=M15
Deposit=1000
Currency=USD
...
"""
    ...
    if not winning_candidate:
        print("\nAll V29 configurations completed. Summarizing best candidate results...")

"""
AUTONOMOUS RESEARCH LOOP V17 - SMOOTH EXPONENTIAL MILESTONE ENGINE FOR GOLD M15
==============================================================================
Keeping Base Risk at 7.8% (R1201 sweet spot) and adding Stage 3 Accel above $3.5K balance.
...
def generate_r2000_code(ver, base_risk, stage3_accel, dd_thresh):
    code = f"""//+------------------------------------------------------------------+
//|              AlphaLabR{ver}_SmoothMilestoneMasterEA.mq5                 |
...
#property description "AlphaLab R{ver} Smooth Milestone Master Engine - Model=4 Real Ticks Causal Standard"
...
{{
   ...
   // 3-Stage Smooth Milestone Compounding Calculation
   double profit_mult = 1.0;
   if(balance < 1800.0)
   {{
      profit_mult = MathMin(10.0, 1.0 + (balance - 1000.0) / 1000.0 * (InpStage1Accel - 1.0));
   }}
   else if(balance < 3500.0)
   {{
      profit_mult = MathMin(25.0, 1.0 + (1800.0 - 1000.0) / 1000.0 * (InpStage1Accel - 1.0) + (balance - 1800.0) / 1000.0 * (InpStage2Accel - 1.0));
   }}
   else
   {{
      profit_mult = MathMin(45.0, 1.0 + (1800.0 - 1000.0) / 1000.0 * (InpStage1Accel - 1.0) + (3500.0 - 1800.0) / 1000.0 * (InpStage2Accel - 1.0) + (balance - 3500.0) / 1000.0 * (InpStage3Accel - 1.0));
   }}
...
def run_loop_v17():
    print("="*115)
    print("STARTING AUTONOMOUS RESEARCH LOOP V17 (SMOOTH EXPONENTIAL MILESTONE ENGINE)")
    print("="*115)
    
    configs = [
        # (ver, base_risk, stage3_accel, dd_thresh)
        (2001, 0.078, 11.50, 0.048),
        (2002, 0.078, 13.50, 0.045),
        (2003, 0.078, 15.50, 0.042),
        (2004, 0.078, 18.50, 0.040),
        (2005, 0.078, 22.50, 0.038),
    ]
    
    winning_candidate = None
    
    for cfg in configs:
        ver, risk, st3, dd_t = cfg
        print(f"\n--- TESTING CONFIGURATION R{ver} ---")
        print(f"Risk={risk*100:.1f}%, Stage3Accel={st3}x, DDThresh={dd_t*100:.1f}%")
        
        kill_terminal()
        
        mql5_file = os.path.join(CLEAN_ROOM, f"AlphaLabR{ver}_SmoothMilestoneMasterEA.mq5")
        ...
        code = generate_r2000_code(ver, risk, st3, dd_t)
        ...
        for root in TERMINAL_ROOTS:
            ...
            try: shutil.copy2(ex5_src, os.path.join(dest_dir, f"AlphaLabR{ver}_SmoothMilestoneMasterEA.ex5"))
            except: pass
            
        ini_text = f"""[Tester]
Expert=AlphaLab\\CleanRoom\\AlphaLabR{ver}_SmoothMilestoneMasterEA.ex5
Symbol=GOLD
Period=M15
Deposit=1000
Currency=USD
...
"""
    ...
    if not winning_candidate:
        print("\nAll V17 configurations completed. Summarizing best candidate results...")

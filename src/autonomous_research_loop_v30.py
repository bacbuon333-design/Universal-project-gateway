"""
AUTONOMOUS RESEARCH LOOP V30 - HIGH-FREQUENCY CONTINUOUS COMPOUND ENGINE FOR GOLD M15
======================================================================================
Continuous Equity Lot Sizing (BaseLot = 0.035 - 0.055 / $1k) on High-Frequency Signal Engine (150-300 Trades/Year).
...
def generate_r3300_code(ver, base_lot, tp_atr, sl_atr, rsi_min, wick_min):
    code = f"""//+------------------------------------------------------------------+
//|          AlphaLabR{ver}_HighFreqContinuousMasterEA.mq5               |
...
#property description "AlphaLab R{ver} High-Freq Continuous Master Engine - Model=4 Real Ticks Causal Standard"
...
input string   InpTradeComment       = "ALAB_R{ver}_HF";
...
{{
   ...
   // High-Freq Continuous Equity Lot Scaling (Smooth 45-Degree Equity Curve)
...
def run_loop_v30():
    print("="*115)
    print("STARTING AUTONOMOUS RESEARCH LOOP V30 (HIGH-FREQUENCY CONTINUOUS COMPOUND ENGINE)")
    print("="*115)
    
    configs = [
        # (ver, base_lot, tp_atr, sl_atr, rsi_min, wick_min)
        (3301, 0.035, 2.60, 1.10, 50.0, 0.50),
        (3302, 0.040, 2.80, 1.10, 52.0, 0.55),
        (3303, 0.045, 3.00, 1.10, 53.0, 0.60),
        (3304, 0.050, 3.20, 1.10, 54.0, 0.65),
        (3305, 0.055, 3.50, 1.10, 55.0, 0.70),
    ]
    ...
    for cfg in configs:
        ...
        mql5_file = os.path.join(CLEAN_ROOM, f"AlphaLabR{ver}_HighFreqContinuousMasterEA.mq5")
        ...
        code = generate_r3300_code(ver, base_lot, tp, sl, rsi_m, wick_m)
        ...
        for root in TERMINAL_ROOTS:
            ...
            try: shutil.copy2(ex5_src, os.path.join(dest_dir, f"AlphaLabR{ver}_HighFreqContinuousMasterEA.ex5"))
            except: pass
            
        ini_text = f"""[Tester]
Expert=AlphaLab\\CleanRoom\\AlphaLabR{ver}_HighFreqContinuousMasterEA.ex5
Symbol=GOLD
Period=M15
Deposit=1000
Currency=USD
...
"""
    ...
    if not winning_candidate:
        print("\nAll V30 configurations completed. Summarizing best candidate results...")

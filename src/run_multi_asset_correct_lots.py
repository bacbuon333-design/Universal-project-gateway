"""
CORRECTED MULTI-ASSET CROSS-VALIDATION GAUNTLET FOR CHECKPOINT 5
===============================================================
Tests Checkpoint 5 across XAUUSD, EURUSD, GBPUSD, USDJPY, BTCUSD
with 100% correct lot sizing & contract value math.
"""
...
# Corrected Pip Values per 0.01 lot ($ per pip for 0.01 lot)
# XAUUSD: 1 pip (0.01) = $0.01 per 0.01 lot ($1.00 per 1.00 lot)
# EURUSD: 1 pip (0.0001) = $0.10 per 0.01 lot ($10.00 per 1.00 lot)
# GBPUSD: 1 pip (0.0001) = $0.10 per 0.01 lot ($10.00 per 1.00 lot)
# USDJPY: 1 pip (0.01) = $0.067 per 0.01 lot ($6.70 per 1.00 lot)
# BTCUSD: 1 pt (1.0) = $0.01 per 0.01 lot ($1.00 per 1.00 lot)

ASSET_SPECS = {
    'XAUUSD': {'pip': 0.01,   'pip_val_001': 0.01,  'sp_pips': 25.0, 'comm_001': 0.07, 'slip_pips': 5.0, 'name': 'Gold (Commodity)'},
    'EURUSD': {'pip': 0.0001, 'pip_val_001': 0.10,  'sp_pips': 1.0,  'comm_001': 0.07, 'slip_pips': 0.2, 'name': 'Euro (Major FX)'},
    'GBPUSD': {'pip': 0.0001, 'pip_val_001': 0.10,  'sp_pips': 1.2,  'comm_001': 0.07, 'slip_pips': 0.2, 'name': 'Pound (Major FX)'},
    'USDJPY': {'pip': 0.01,   'pip_val_001': 0.067, 'sp_pips': 1.2,  'comm_001': 0.07, 'slip_pips': 0.2, 'name': 'Yen (Major FX)'},
    'BTCUSD': {'pip': 1.0,    'pip_val_001': 0.01,  'sp_pips': 35.0, 'comm_001': 0.07, 'slip_pips': 5.0, 'name': 'Bitcoin (Crypto)'}
}
...
def run_cp5_correct_lots(sym, spec, risk_pct=0.025):
    df = get_asset_df(sym)
    ...
    for i in range(50, n-1):
        ...
        if pos_dir == 0 and (i - last_trade >= cooldown) and bal > 0:
            ...
            if sig != 0:
                ...
                # Corrected lot sizing: risk_amt / (sl_pts * pval001 per 0.01 lot)
...
def main():
    print("="*105)
    print("CORRECTED MULTI-ASSET CROSS-VALIDATION GAUNTLET FOR CHECKPOINT 5")
    ...
    for sym in ['XAUUSD', 'EURUSD', 'GBPUSD', 'USDJPY', 'BTCUSD']:
        res = run_cp5_correct_lots(sym, ASSET_SPECS[sym], risk_pct=0.025)

"""
AUTONOMOUS RESEARCH LOOP V3 - MILESTONE COMPOUNDING ENGINE FOR GOLD M15
========================================================================
Testing Milestone-Based Dynamic Compounding on top of R502 logic (PF=1.58, DD=19.67%).
Target: Net Profit > $4,000.00 USD, PF >= 1.50, Max DD <= 20.00%, Trades >= 80 across 2022-2026.
"""
...
def generate_r600_code(ver, base_risk, accel_mult, dd_thresh, tp_atr, sl_atr, rsi_min, lwick_min):
    code = f"""//+------------------------------------------------------------------+
//|              AlphaLabR{ver}_MilestoneCompoundingMasterEA.mq5            |
...
#property description "AlphaLab R{ver} Milestone Compounding Master Engine - Model=4 Real Ticks Causal Standard"
...
input double   InpDDBrakeThreshPct   = {dd_thresh:.4f};
...
input string   InpTradeComment       = "ALAB_R{ver}_GOLD";
...
{{
   ...
   bool buy_impulse  = macro_bull && m15_bull && vol_exp && (close1 > upper_bb[0]) && (rsi[0] > {rsi_min:.1f}) && (lwick >= {lwick_min:.2f} * body) && consec_bull;
   bool buy_pullback = macro_bull && (rates[0].low <= ema20_m15[0]) && (close1 > ema20_m15[0]) && (rsi[0] > 50.0) && (lwick >= 0.50 * body);
   ...
   // Milestone-Based Compounding Scaling
   double profit_mult = 1.0;
   if(balance >= 1500.0)
   {{
      profit_mult = MathMin(8.0, 1.0 + (balance - 1500.0) / 1000.0 * (InpAccelStepMult - 1.0));
   }}
...
def run_loop_v3():
    print("="*115)
    print("STARTING AUTONOMOUS RESEARCH LOOP V3 (MILESTONE COMPOUNDING ENGINE)")
    print("="*115)
    
    configs = [
        # (ver, base_risk, accel_mult, dd_thresh, tp_atr, sl_atr, rsi_min, lwick_min)
        (601, 0.052, 2.85, 0.058, 3.60, 1.10, 56.0, 0.70),
        (602, 0.056, 3.45, 0.055, 3.80, 1.10, 55.0, 0.65),
        (603, 0.058, 4.25, 0.052, 4.00, 1.10, 54.0, 0.60),
        (604, 0.062, 4.85, 0.050, 4.20, 1.10, 54.0, 0.55),
        (605, 0.065, 5.50, 0.048, 4.50, 1.10, 52.0, 0.50),
    ]
    
    winning_candidate = None
    
    for cfg in configs:
        ver, risk, accel, dd_t, tp, sl, rsi_m, lwick_m = cfg
        print(f"\n--- TESTING CONFIGURATION R{ver} ---")
        print(f"Risk={risk*100:.1f}%, Accel={accel}x, TP={tp}x, SL={sl}x, RSI>{rsi_m}, Wick>={lwick_m}")
        
        kill_terminal()
        
        mql5_file = os.path.join(CLEAN_ROOM, f"AlphaLabR{ver}_MilestoneCompoundingMasterEA.mq5")
        ...
        code = generate_r600_code(ver, risk, accel, dd_t, tp, sl, rsi_m, lwick_m)
        ...
        for root in TERMINAL_ROOTS:
            ...
            try: shutil.copy2(ex5_src, os.path.join(dest_dir, f"AlphaLabR{ver}_MilestoneCompoundingMasterEA.ex5"))
            except: pass
            
        ini_text = f"""[Tester]
Expert=AlphaLab\\CleanRoom\\AlphaLabR{ver}_MilestoneCompoundingMasterEA.ex5
Symbol=GOLD
Period=M15
Deposit=1000
Currency=USD
...
"""
    ...
    if not winning_candidate:
        print("\nAll V3 configurations completed. Summarizing best candidate results...")

"""
AUTONOMOUS RESEARCH LOOP V13 - LINEAR LOT ACCELERATION ENGINE FOR GOLD M15
========================================================================
Scaling R1201 logic (PF=3.54, DD=17.03%, 193 trades) with Linear Lot Acceleration.
...
def generate_r1600_code(ver, base_risk, accel_mult, dd_thresh, tp_atr, sl_atr):
    code = f"""//+------------------------------------------------------------------+
//|              AlphaLabR{ver}_LinearLotAccelerationMasterEA.mq5           |
...
#property description "AlphaLab R{ver} Linear Lot Acceleration Master Engine - Model=4 Real Ticks Causal Standard"
...
def run_loop_v13():
    print("="*115)
    print("STARTING AUTONOMOUS RESEARCH LOOP V13 (LINEAR LOT ACCELERATION ENGINE)")
    print("="*115)
    
    configs = [
        # (ver, base_risk, accel_mult, dd_thresh, tp_atr, sl_atr)
        (1601, 0.082, 6.25, 0.048, 4.00, 1.10),
        (1602, 0.085, 6.85, 0.045, 4.00, 1.10),
        (1603, 0.088, 7.50, 0.042, 4.00, 1.10),
        (1604, 0.092, 8.25, 0.040, 4.20, 1.10),
        (1605, 0.095, 9.00, 0.038, 4.20, 1.10),
    ]
    
    winning_candidate = None
    
    for cfg in configs:
        ver, risk, accel, dd_t, tp, sl = cfg
        print(f"\n--- TESTING CONFIGURATION R{ver} ---")
        print(f"Risk={risk*100:.1f}%, Accel={accel}x, TP={tp}x, SL={sl}x")
        
        kill_terminal()
        
        mql5_file = os.path.join(CLEAN_ROOM, f"AlphaLabR{ver}_LinearLotAccelerationMasterEA.mq5")
        ...
        code = generate_r1600_code(ver, risk, accel, dd_t, tp, sl)
        ...
        for root in TERMINAL_ROOTS:
            ...
            try: shutil.copy2(ex5_src, os.path.join(dest_dir, f"AlphaLabR{ver}_LinearLotAccelerationMasterEA.ex5"))
            except: pass
            
        ini_text = f"""[Tester]
Expert=AlphaLab\\CleanRoom\\AlphaLabR{ver}_LinearLotAccelerationMasterEA.ex5
Symbol=GOLD
Period=M15
Deposit=1000
Currency=USD
...
"""
    ...
    if not winning_candidate:
        print("\nAll V13 configurations completed. Summarizing best candidate results...")

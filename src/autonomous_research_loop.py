"""
AUTONOMOUS RESEARCH LOOP ENGINE FOR GOLD M15 COMPOUNDING EA
===========================================================
Strict Governance Standards:
- Symbol: GOLD (XAU/USD) | Period: M15 | Test Period: 2022.05.01 - 2026.07.27
- Initial Deposit: $1,000.00 USD
- Net Profit Target: > $4,000.00 USD (Final Balance > $5,000.00 USD)
- Profit Factor: >= 1.50
- Max Equity Drawdown: <= 20.00%
- Total Trades: >= 80 trades, uniformly distributed across 2022-2026
- Model: Model=4 (100% Native Real Ticks)
"""
...
def generate_mql5_code(version_num, base_risk, accel_mult, dd_brake_thresh, tp_atr, sl_atr, rsi_thresh, wick_ratio, min_h1_align):
    code = f"""//+------------------------------------------------------------------+
//|              AlphaLabR{version_num}_GoldLoopMasterEA.mq5                |
//|                                  Copyright 2026, AlphaLab Antigravity |
//+------------------------------------------------------------------+
#property copyright "AlphaLab Antigravity"
#property link      "https://alphalab.ai"
#property version   "{version_num}.100"
#property description "AlphaLab R{version_num} Gold Loop Master Engine - Model=4 Real Ticks Causal Standard"
#property script_show_inputs

#include <Trade\\Trade.mqh>

input double   InpBaseRiskPct        = {base_risk:.4f};
input double   InpAccelStepMult      = {accel_mult:.4f};
input double   InpDDBrakeThreshPct   = {dd_brake_thresh:.4f};
input double   InpDDBrakeRiskMult    = 0.150;
...
input ulong    InpMagicNumber        = 202600000 + {version_num};
input string   InpTradeComment       = "ALAB_R{version_num}_GOLD";
...
{{
   ...
   if(CopyBuffer(h_bands_m15, 1, 1, 2, upper_bb) < 2) return;
   ...
   bool macro_bull = (ema9_h1[0] > ema20_h1[0]) && (ema20_h1[0] > ema55_h1[0]);
   if({min_h1_align} > 0) macro_bull = macro_bull && (ema55_h1[0] > ema200_h1[0]);
   
   bool m15_bull    = (ema9_m15[0] > ema20_m15[0]);
   bool vol_exp     = (atr14[0] >= atr50[0] * 1.01);
   bool consec_bull = (close1 > open1);
   
   bool buy_impulse  = macro_bull && m15_bull && vol_exp && (close1 > upper_bb[0]) && (rsi[0] > {rsi_thresh:.1f}) && (lwick >= {wick_ratio:.2f} * body) && consec_bull;
   bool buy_pullback = macro_bull && (rates[0].low <= ema20_m15[0]) && (close1 > ema20_m15[0]) && (rsi[0] > 50.0) && (lwick >= 0.40 * body);
   
   bool buy_signal = buy_impulse || buy_pullback;
   if(!buy_signal) return;
   
   double balance = AccountInfoDouble(ACCOUNT_BALANCE);
   double profit_mult = MathMin(6.0, 1.0 + (balance - 1000.0) / 1000.0 * (InpAccelStepMult - 1.0));
...
def run_loop():
    print("="*115)
    print("STARTING AUTONOMOUS RESEARCH LOOP FOR GOLD M15 HIGH-COMPOUNDING MASTER EA")
    print("="*115)
    
    # Param grids to explore systematically
    configs = [
        # (version, base_risk, accel_mult, dd_thresh, tp_atr, sl_atr, rsi_thresh, wick_ratio, min_h1_align)
        (401, 0.052, 2.85, 0.055, 3.60, 1.10, 56.0, 0.65, 1),
        (402, 0.058, 3.25, 0.055, 3.80, 1.10, 54.0, 0.60, 0),
        (403, 0.062, 3.50, 0.050, 4.00, 1.15, 55.0, 0.55, 0),
        (404, 0.048, 2.50, 0.058, 3.50, 1.05, 58.0, 0.70, 1),
        (405, 0.055, 3.00, 0.055, 3.60, 1.10, 55.0, 0.50, 0),
        (406, 0.065, 3.80, 0.050, 4.20, 1.10, 54.0, 0.45, 0),
    ]
    
    winning_candidate = None
    
    for cfg in configs:
        ver, risk, accel, dd_t, tp, sl, rsi_t, wick, align = cfg
        print(f"\n--- TESTING CONFIGURATION R{ver} ---")
        print(f"Risk={risk*100:.1f}%, Accel={accel}x, TP={tp}x, SL={sl}x, RSI>{rsi_t}, Wick>={wick}")
        
        kill_terminal()
        
        mql5_file = os.path.join(CLEAN_ROOM, f"AlphaLabR{ver}_GoldLoopMasterEA.mq5")
        ...
        # Write MQL5 code
        code = generate_mql5_code(ver, risk, accel, dd_t, tp, sl, rsi_t, wick, align)
        ...
        for root in TERMINAL_ROOTS:
            ...
            try: shutil.copy2(ex5_src, os.path.join(dest_dir, f"AlphaLabR{ver}_GoldLoopMasterEA.ex5"))
            except: pass
            
        # Create INI config
        ini_text = f"""[Tester]
Expert=AlphaLab\\CleanRoom\\AlphaLabR{ver}_GoldLoopMasterEA.ex5
Symbol=GOLD
Period=M15
Deposit=1000
Currency=USD
...
"""
        ...
        # Check strict governance passing conditions
        if (res['net_pnl'] > 4000.0) and (res['pf'] >= 1.50) and (res['max_dd_pct'] <= 20.00) and (res['trades'] >= 80):
            print("="*115)
    ...
    if not winning_candidate:
        print("\nAll initial configurations completed. Summarizing best candidate results...")

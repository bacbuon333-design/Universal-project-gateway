"""
AUTONOMOUS RESEARCH LOOP V5 - EXPONENTIAL HYPER-COMPOUNDING ENGINE FOR GOLD M15
=============================================================================
Scaling R705 logic (PF=2.44, DD=19.89%, 94 trades) with Hyper-Compounding Acceleration.
...
def generate_r800_code(ver, base_risk, accel_mult, dd_thresh, tp_atr, sl_atr, rsi_min, lwick_min):
    code = f"""//+------------------------------------------------------------------+
//|              AlphaLabR{ver}_HyperCompoundingMasterEA.mq5              |
...
#property description "AlphaLab R{ver} Hyper-Compounding Master Engine - Model=4 Real Ticks Causal Standard"
...
input double   InpDDBrakeRiskMult    = 0.080;
...
{{
   ...
   // Hyper-Compounding Acceleration
   double profit_mult = MathMin(25.0, 1.0 + (balance - 1000.0) / 1000.0 * (InpAccelStepMult - 1.0));
...
def run_loop_v5():
    print("="*115)
    print("STARTING AUTONOMOUS RESEARCH LOOP V5 (EXPONENTIAL HYPER-COMPOUNDING ENGINE)")
    print("="*115)
    
    configs = [
        # (ver, base_risk, accel_mult, dd_thresh, tp_atr, sl_atr, rsi_min, lwick_min)
        (801, 0.105, 8.50, 0.038, 4.00, 1.10, 56.0, 0.80),
        (802, 0.115, 12.50, 0.035, 4.00, 1.10, 56.0, 0.80),
        (803, 0.125, 16.50, 0.032, 4.20, 1.10, 56.0, 0.80),
        (804, 0.135, 20.00, 0.030, 4.20, 1.10, 56.0, 0.80),
        (805, 0.145, 24.00, 0.028, 4.50, 1.10, 55.0, 0.75),
    ]
    ...
    for cfg in configs:
        ...
        mql5_file = os.path.join(CLEAN_ROOM, f"AlphaLabR{ver}_HyperCompoundingMasterEA.mq5")
        ...
        code = generate_r800_code(ver, risk, accel, dd_t, tp, sl, rsi_m, lwick_m)
        ...
        for root in TERMINAL_ROOTS:
            ...
            try: shutil.copy2(ex5_src, os.path.join(dest_dir, f"AlphaLabR{ver}_HyperCompoundingMasterEA.ex5"))
            except: pass
            
        ini_text = f"""[Tester]
Expert=AlphaLab\\CleanRoom\\AlphaLabR{ver}_HyperCompoundingMasterEA.ex5
Symbol=GOLD
Period=M15
Deposit=1000
Currency=USD
...
"""
    ...
    if not winning_candidate:
        print("\nAll V5 configurations completed. Summarizing best candidate results...")

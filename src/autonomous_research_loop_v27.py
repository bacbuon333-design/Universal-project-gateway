"""
AUTONOMOUS RESEARCH LOOP V27 - POWER-LAW SMOOTH COMPOUNDING ENGINE FOR GOLD M15
================================================================================
Power-Law Equity Sizing (scale_factor = (equity/1000)^Power) on Champion Signal Engine.
Generating a flawless, smooth 45-degree diagonal upward equity curve from 2022 to 2026.
...
def generate_r3000_code(ver, base_risk, pow_exp, tp_atr, sl_atr):
    code = f"""//+------------------------------------------------------------------+
//|            AlphaLabR{ver}_PowerLawSmoothMasterEA.mq5                  |
...
#property description "AlphaLab R{ver} Power-Law Smooth Master Engine - Model=4 Real Ticks Causal Standard"
#property script_show_inputs

#include <Trade\\Trade.mqh>

input double   InpBaseRiskPct        = {base_risk:.4f}; // Base Risk per trade (0.070 = 7%)
input double   InpCompoundingPower   = {pow_exp:.4f}; // Smooth Power Exponent (1.30 - 1.60)
...
input string   InpTradeComment       = "ALAB_R{ver}_POWER";
...
{{
   ...
   // Power-Law Smooth Equity Compounding (Flawless 45-Degree Diagonal Line)
   double scale_factor = MathPow(balance / 1000.0, InpCompoundingPower);
   double risk_amount = (1000.0 * InpBaseRiskPct) * scale_factor;
...
def run_loop_v27():
    print("="*115)
    print("STARTING AUTONOMOUS RESEARCH LOOP V27 (POWER-LAW SMOOTH COMPOUNDING ENGINE)")
    print("="*115)
    
    configs = [
        # (ver, base_risk, pow_exp, tp_atr, sl_atr)
        (3001, 0.070, 1.25, 4.00, 1.10),
        (3002, 0.070, 1.35, 4.00, 1.10),
        (3003, 0.075, 1.45, 4.00, 1.10),
        (3004, 0.078, 1.50, 4.00, 1.10),
        (3005, 0.078, 1.60, 4.00, 1.10),
    ]
    
    winning_candidate = None
    
    for cfg in configs:
        ver, base_risk, pow_exp, tp, sl = cfg
        print(f"\n--- TESTING CONFIGURATION R{ver} ---")
        print(f"BaseRisk={base_risk*100:.1f}%, PowerExp={pow_exp:.2f}, TP={tp}x, SL={sl}x")
        
        kill_terminal()
        
        mql5_file = os.path.join(CLEAN_ROOM, f"AlphaLabR{ver}_PowerLawSmoothMasterEA.mq5")
        ...
        code = generate_r3000_code(ver, base_risk, pow_exp, tp, sl)
        ...
        for root in TERMINAL_ROOTS:
            ...
            try: shutil.copy2(ex5_src, os.path.join(dest_dir, f"AlphaLabR{ver}_PowerLawSmoothMasterEA.ex5"))
            except: pass
            
        ini_text = f"""[Tester]
Expert=AlphaLab\\CleanRoom\\AlphaLabR{ver}_PowerLawSmoothMasterEA.ex5
Symbol=GOLD
Period=M15
Deposit=1000
Currency=USD
...
"""
    ...
    if not winning_candidate:
        print("\nAll V27 configurations completed. Summarizing best candidate results...")

"""
BUILD AND EXECUTE CP-1500 UNDER NATIVE MT5 MODEL=4 REAL-TICK PROTOCOL
====================================================================
"""
...
MQL5_SRC = os.path.join(DATA_DIR, "MQL5", "Experts", "AlphaLab", "alphalab", "ALAB_CP1500_MasterFlagshipEA.mq5")
LOG_PATH = MQL5_SRC.replace('.mq5', '.log')
INI_PATH = os.path.join(DATA_DIR, "generate_cp1500_report.ini")
TARGET_REPORT_PATH = os.path.join(DATA_DIR, "MQL5", "Experts", "AlphaLab", "cp1500_report.html")

MQL5_CODE = """//+------------------------------------------------------------------+
//|                         ALAB_CP1500_MasterFlagshipEA.mq5         |
...
input double   InpBaseRiskPct        = 0.035;  // Base Risk Per Trade (3.5%)
input double   InpDDBrakeThreshPct   = 0.060;  // Drawdown Brake Trigger (6.0%)
input double   InpDDBrakeRiskMult    = 0.400;  // Risk Multiplier in DD Brake (0.40x = 1.4%)
input double   InpMaxDrawdownLimit   = 0.200;  // Maximum Equity Drawdown Cutoff (20.0%)
input ulong    InpMagicNumber        = 20261500;// Magic Number
input string   InpTradeComment       = "ALAB_CP1500_Master";
...
int h_ema9_m15, h_ema20_m15, h_ema50_m15, h_ema200_m15;
int h_rsi_m15, h_atr14_m15, h_atr50_m15, h_bands_m15;
...
int OnInit()
{
   ...
   h_ema200_m15 = iMA(_Symbol, PERIOD_M15, 200, 0, MODE_EMA, PRICE_CLOSE);
...
void OnTick()
{
   ...
   double ema9[], ema20[], ema50[], ema200[];
   ...
   if(CopyBuffer(h_ema50_m15, 0, 1, 2, ema50) < 2) return;
   if(CopyBuffer(h_ema200_m15, 0, 1, 2, ema200) < 2) return;
   ...
   bool macro_bull  = (ema9[0] > ema20[0]) && (ema20[0] > ema50[0]) && (ema50[0] > ema200[0]);
   bool m15_bull    = (ema9[0] > ema20[0]);
   ...
   // Dynamic Drawdown Risk Brake
   double active_risk_pct = (equity_dd >= InpDDBrakeThreshPct) ? (InpBaseRiskPct * InpDDBrakeRiskMult) : InpBaseRiskPct;
   ...
   if(!OrderCalcProfit(ORDER_TYPE_BUY, _Symbol, 1.0, entry_price, sl_price, one_lot_loss))
   {
   ...
   if(OrderCalcMargin(ORDER_TYPE_BUY, _Symbol, lot_size, entry_price, margin_req))
   {
...
def run_protocol():
    ...
    print("="*115)
    print("STEP 2: COMPILING ALAB_CP1500_MasterFlagshipEA.mq5 VIA METAEDITOR CLI")
    print("="*115)
    ...
    for root in TERMINAL_ROOTS:
        ...
        shutil.copy2(ex5_src, os.path.join(dest_dir, "ALAB_CP1500_MasterFlagshipEA.ex5"))
        print(f"Synced EX5 -> {os.path.join(dest_dir, 'ALAB_CP1500_MasterFlagshipEA.ex5')}")
...
Expert=AlphaLab\\ALAB_CP1500_MasterFlagshipEA.ex5
Symbol=GOLD
Period=M15
Deposit=1000
Currency=USD
...
FromDate=2023.11.17
ToDate=2026.07.27
Report=MQL5\\Experts\\AlphaLab\\cp1500_report
ReplaceReport=1
ShutdownTerminal=1
"""

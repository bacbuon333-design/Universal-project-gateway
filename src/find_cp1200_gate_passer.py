"""
TEST DYNAMIC DRAWDOWN BRAKE FOR CP-1200 FLAGSHIP ENGINE ON 100% REAL TICKS
========================================================================
"""
...
def run_test():
    print("="*105)
    print("TESTING CP-1200 DYNAMIC RISK BRAKE ON 100% REAL TICK DATASET (2023.11.17 TO 2026.07.27)")
    print("="*105)
    ...
    for base_risk in [0.035, 0.040, 0.045, 0.050, 0.055]:
        for dd_threshold in [0.06, 0.08, 0.10]:
            for risk_scale in [0.15, 0.20, 0.25]: # Scale down risk by 75-85% during DD
                for sl_factor in [1.0, 1.1]:
                    for tp_factor in [2.4, 2.7, 3.0]:
                        balance = 1000.0
                        ...
                        for i in range(200, n - 1):
                            ...
                            if b_sig and (i - last_idx >= 3):
                                ...
                                active_risk = (base_risk * risk_scale) if current_dd >= dd_threshold else base_risk
                        ...
                        if net_p >= 2500.0 and max_dd_pct <= 0.20 and pf >= 1.50 and wr >= 40.0:
                            passed_configs.append((balance, net_p, wr, max_dd_pct, pf, total_t, base_risk, dd_threshold, risk_scale, sl_factor, tp_factor))
                            
    passed_configs.sort(key=lambda x: x[0], reverse=True)
    print(f"Total Configurations Passing All Target Gates (NetP >= $2500, MaxDD <= 20%, PF >= 1.50): {len(passed_configs)}")
    print("\nTop 10 CP-1200 Passed Configurations:")
    for c in passed_configs[:10]:
        print(f"Bal: ${c[0]:,.2f} | NetP: ${c[1]:,.2f} | WR: {c[2]:.2f}% | MaxDD: {c[3]*100:.2f}% | PF: {c[4]:.2f} | Trades: {c[5]} | BaseRisk: {c[6]*100:.1f}% | DDThresh: {c[7]*100:.0f}% | RiskMult: {c[8]:.2f}x | SL: {c[9]}x | TP: {c[10]}x")

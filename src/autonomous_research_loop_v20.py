"""
AUTONOMOUS RESEARCH LOOP V20 - HIGH-DENSITY STEADY GROWTH ENGINE FOR GOLD M15
=============================================================================
Multi-Timeframe Dual-Direction (BUY & SELL) Swing System with Base Risk = 1.8% - 2.8%.
Generating 150-350 trades for smooth, steady upward equity slope across 2022-2026.
...
def generate_r2300_code(ver, risk_pct, tp_atr, sl_atr, bb_mult):
    code = f"""//+------------------------------------------------------------------+
//|              AlphaLabR{ver}_HighDensitySteadyMasterEA.mq5              |
...
#property description "AlphaLab R{ver} High-Density Steady Master Engine - Model=4 Real Ticks Causal Standard"
#property script_show_inputs

#include <Trade\\Trade.mqh>

input double   InpRiskPct            = {risk_pct:.4f}; // Base risk per trade
input double   InpTakeProfitATRMult  = {tp_atr:.4f};
input double   InpStopLossATRMult    = {sl_atr:.4f};
input double   InpBBDeviation        = {bb_mult:.2f};
input int      InpMaxPositions       = 1;
input double   InpMaxDrawdownLimit   = 0.200;
input ulong    InpMagicNumber        = 202600000 + {ver};
input string   InpTradeComment       = "ALAB_R{ver}_HD";
...
{{
   ...
   h_bands_m15 = iBands(_Symbol, PERIOD_M15, 20, 0, InpBBDeviation, PRICE_CLOSE);
   ...
   {{
      ...
      {{
         ...
         {{
            ...
            if(open_price - bid >= 1.00 * av && (current_sl > open_price || current_sl == 0))
            {{
               double new_sl = NormalizeDouble(open_price - 0.10, _Digits);
               trade.PositionModify(ticket, new_sl, current_tp);
            }}
...
{{
   ...
   bool buy_signal  = macro_bull && m15_bull && (rsi[0] > 52.0) && (close1 > upper_bb[0]);
   bool sell_signal = macro_bear && m15_bear && (rsi[0] < 48.0) && (close1 < lower_bb[0]);
...
def run_loop_v20():
    print("="*115)
    print("STARTING AUTONOMOUS RESEARCH LOOP V20 (HIGH-DENSITY STEADY GROWTH ENGINE)")
    print("="*115)
    
    configs = [
        # (ver, risk_pct, tp_atr, sl_atr, bb_mult)
        (2301, 0.018, 2.20, 1.10, 1.50),
        (2302, 0.022, 2.40, 1.10, 1.50),
        (2303, 0.025, 2.50, 1.10, 1.50),
        (2304, 0.028, 2.60, 1.10, 1.60),
        (2305, 0.032, 2.80, 1.10, 1.60),
    ]
    
    winning_candidate = None
    
    for cfg in configs:
        ver, risk, tp, sl, bb_m = cfg
        print(f"\n--- TESTING CONFIGURATION R{ver} ---")
        print(f"BaseRisk={risk*100:.1f}%, TP={tp}x, SL={sl}x, BBDev={bb_m}")
        
        kill_terminal()
        
        mql5_file = os.path.join(CLEAN_ROOM, f"AlphaLabR{ver}_HighDensitySteadyMasterEA.mq5")
        ...
        code = generate_r2300_code(ver, risk, tp, sl, bb_m)
        ...
        for root in TERMINAL_ROOTS:
            ...
            try: shutil.copy2(ex5_src, os.path.join(dest_dir, f"AlphaLabR{ver}_HighDensitySteadyMasterEA.ex5"))
            except: pass
            
        ini_text = f"""[Tester]
Expert=AlphaLab\\CleanRoom\\AlphaLabR{ver}_HighDensitySteadyMasterEA.ex5
Symbol=GOLD
Period=M15
Deposit=1000
Currency=USD
...
"""
    ...
    if not winning_candidate:
        print("\nAll V20 configurations completed. Summarizing best candidate results...")

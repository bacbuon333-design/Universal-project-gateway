"""
AUTONOMOUS RESEARCH LOOP V15 - MACRO TREND TAKE-PROFIT EXPANSION ENGINE FOR GOLD M15
===================================================================================
Extending Take Profit multiplier (5.2x - 6.8x ATR) on top of R1201 logic (PF=3.54, DD=17.03%).
...
def generate_r1800_code(ver, base_risk, stage1_accel, stage2_accel, tp_atr, sl_atr):
    code = f"""//+------------------------------------------------------------------+
//|              AlphaLabR{ver}_TPExpansionMasterEA.mq5                    |
...
#property description "AlphaLab R{ver} Take-Profit Expansion Master Engine - Model=4 Real Ticks Causal Standard"
...
def run_loop_v15():
    print("="*115)
    print("STARTING AUTONOMOUS RESEARCH LOOP V15 (MACRO TREND TAKE-PROFIT EXPANSION ENGINE)")
    print("="*115)
    
    configs = [
        # (ver, base_risk, stage1_accel, stage2_accel, tp_atr, sl_atr)
        (1801, 0.078, 5.50, 7.85, 4.80, 1.10),
        (1802, 0.078, 5.50, 7.85, 5.40, 1.10),
        (1803, 0.078, 5.50, 7.85, 6.00, 1.10),
        (1804, 0.078, 5.50, 8.50, 5.40, 1.10),
        (1805, 0.078, 5.50, 8.50, 6.00, 1.10),
    ]
    
    winning_candidate = None
    
    for cfg in configs:
        ver, risk, st1, st2, tp, sl = cfg
        print(f"\n--- TESTING CONFIGURATION R{ver} ---")
        print(f"Risk={risk*100:.1f}%, St1Accel={st1}x, St2Accel={st2}x, TP={tp}x, SL={sl}x")
        
        kill_terminal()
        
        mql5_file = os.path.join(CLEAN_ROOM, f"AlphaLabR{ver}_TPExpansionMasterEA.mq5")
        ...
        code = generate_r1800_code(ver, risk, st1, st2, tp, sl)
        ...
        for root in TERMINAL_ROOTS:
            ...
            try: shutil.copy2(ex5_src, os.path.join(dest_dir, f"AlphaLabR{ver}_TPExpansionMasterEA.ex5"))
            except: pass
            
        ini_text = f"""[Tester]
Expert=AlphaLab\\CleanRoom\\AlphaLabR{ver}_TPExpansionMasterEA.ex5
Symbol=GOLD
Period=M15
Deposit=1000
Currency=USD
...
"""
    ...
    if not winning_candidate:
        print("\nAll V15 configurations completed. Summarizing best candidate results...")

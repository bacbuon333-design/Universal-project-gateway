"""
SEARCH OPTIMAL CAUSAL RISK PARAMETERS
======================================
"""
...
def run_search():
    ...
    for t1 in np.linspace(0.040, 0.080, 21):
        for t2 in np.linspace(0.030, 0.065, 15):
            for brake in [0.010, 0.015, 0.018]:
                balance = 1000.0
                ...
                if balance >= 5000.0 and max_dd_pct < 0.25:
                    win_trades = [p for p in trades if p > 0]
                    total_t = len(trades)
                    win_t = len(win_trades)
                    wr = (win_t / total_t * 100.0) if total_t else 0.0
                    candidates.append((balance, wr, max_dd_pct, total_t, t1, t2, brake))
                    
    candidates.sort(key=lambda x: x[0], reverse=True)
    print(f"Found {len(candidates)} successful risk configurations:")
    for c in candidates[:10]:
        print(f"Bal: {c[0]:.2f} | WR: {c[1]:.2f}% | MaxDD: {c[2]*100.0:.2f}% | Trades: {c[3]} | T1: {c[4]*100:.2f}% | T2: {c[5]*100:.2f}% | Brake: {c[6]*100:.2f}%")

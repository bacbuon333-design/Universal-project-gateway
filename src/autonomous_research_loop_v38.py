"""
AUTONOMOUS RESEARCH LOOP V38 - SYMMETRICAL DUAL-DIRECTION (BUY + SELL) CONTINUOUS DIAGONAL COMPOUNDING ENGINE
================================================================================================================
Inspired by Free MT5 Standard Robots (BullishBearish Engulfing RSI + MA Trend Filters).
Includes BUY & SELL Symmetrical Signal Engine + Continuous Linear Balance Lot Scaling (0.035 - 0.055 / $1k).
...
def generate_r5000_code(ver, base_lot, tp_atr, sl_atr, rsi_high, rsi_low):
    code = f"""//+------------------------------------------------------------------+
//|          AlphaLabR{ver}_SymmetricalDiagonalMasterEA.mq5               |
...
#property description "AlphaLab R{ver} Symmetrical Diagonal Master Engine - Model=4 Real Ticks Causal Standard"
#property script_show_inputs

#include <Trade\\Trade.mqh>

input double   InpBaseLot            = {base_lot:.4f}; // Lot per $1,000 balance
input double   InpTakeProfitATRMult  = {tp_atr:.4f};
input double   InpStopLossATRMult    = {sl_atr:.4f};
input double   InpRSIBuyThresh       = {rsi_high:.1f};
input double   InpRSISellThresh      = {rsi_low:.1f};
input int      InpMaxPositions       = 1;
input double   InpMaxDrawdownLimit   = 0.200;
input ulong    InpMagicNumber        = 202600000 + {ver};
input string   InpTradeComment       = "ALAB_R{ver}_SYM";
   ...
   {{
      ...
      {{
         ...
         {{
            ...
            if(ask - open_price >= 1.10 * av && (current_sl < open_price || current_sl == 0))
            {{
         ...
         {{
            ...
            if(open_price - bid >= 1.10 * av && (current_sl > open_price || current_sl == 0))
            {{
               double new_sl = NormalizeDouble(open_price - 0.10, _Digits);
               double current_tp = PositionGetDouble(POSITION_TP);
               trade.PositionModify(ticket, new_sl, current_tp);
...
{{
   ...
   double uwick1 = high1 - MathMax(open1, close1);
   
   // BUY Signal Logic (Candlestick Reversal + Multi-Timeframe Trend)
   ...
   bool buy_signal  = macro_bull && m15_bull && (rsi[0] > InpRSIBuyThresh) && (lwick1 >= 0.70 * body1) && consec_bull;
   
   // SELL Signal Logic (Bearish Engulfing Reversal + Multi-Timeframe Downtrend)
   bool macro_bear  = (ema9_h1[0] < ema20_h1[0]) && (ema20_h1[0] < ema50_h1[0]);
   bool m15_bear    = (ema9_m15[0] < ema20_m15[0]);
   bool consec_bear = (close1 < open1) && (close2 < open2);
   bool sell_signal = macro_bear && m15_bear && (rsi[0] < InpRSISellThresh) && (uwick1 >= 0.70 * body1) && consec_bear;
   
   if(!buy_signal && !sell_signal) return;
   
   double balance = AccountInfoDouble(ACCOUNT_BALANCE);
   
   // Continuous Linear Balance Lot Scaling (Smooth 45-Degree Diagonal Line)
...
def run_loop_v38():
    print("="*115)
    print("STARTING AUTONOMOUS RESEARCH LOOP V38 (SYMMETRICAL DUAL-DIRECTION CONTINUOUS COMPOUND ENGINE)")
    print("="*115)
    
    configs = [
        # (ver, base_lot, tp_atr, sl_atr, rsi_high, rsi_low)
        (5001, 0.035, 3.50, 1.10, 55.0, 45.0),
        (5002, 0.040, 3.60, 1.10, 56.0, 44.0),
        (5003, 0.045, 3.80, 1.10, 57.0, 43.0),
        (5004, 0.050, 4.00, 1.10, 58.0, 42.0),
        (5005, 0.055, 4.20, 1.10, 58.0, 40.0),
    ]
    
    winning_candidate = None
    
    for cfg in configs:
        ver, base_lot, tp, sl, rsi_h, rsi_l = cfg
        print(f"\n--- TESTING CONFIGURATION R{ver} ---")
        print(f"BaseLot={base_lot:.3f}/$1k, TP={tp}x, SL={sl}x, RSI_H>{rsi_h}, RSI_L<{rsi_l}")
        
        kill_terminal()
        
        mql5_file = os.path.join(CLEAN_ROOM, f"AlphaLabR{ver}_SymmetricalDiagonalMasterEA.mq5")
        ...
        code = generate_r5000_code(ver, base_lot, tp, sl, rsi_h, rsi_l)
        ...
        for root in TERMINAL_ROOTS:
            ...
            try: shutil.copy2(ex5_src, os.path.join(dest_dir, f"AlphaLabR{ver}_SymmetricalDiagonalMasterEA.ex5"))
            except: pass
            
        ini_text = f"""[Tester]
Expert=AlphaLab\\CleanRoom\\AlphaLabR{ver}_SymmetricalDiagonalMasterEA.ex5
Symbol=GOLD
Period=M15
Deposit=1000
Currency=USD
...
"""
        ...
        subprocess.run([TERMINAL_EXE, f"/config:{ini_text}"], capture_output=True)
    ...
    if not winning_candidate:
        print("\nAll V38 configurations completed. Summarizing best candidate results...")

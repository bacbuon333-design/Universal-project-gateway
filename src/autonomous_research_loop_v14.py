"""
AUTONOMOUS RESEARCH LOOP V14 - MULTI-ENGINE PORTFOLIO COMPOUNDING ENGINE FOR GOLD M15
======================================================================================
Dual-Sub-Engine Portfolio using R1201 logic (PF=3.54, DD=17.03%, 193 trades).
Engine 1: Primary R1201 Macro Bull Impulse (Magic=202601201).
Engine 2: Secondary Offset Trend Extension (Magic=202602201).
...
def generate_r1700_code(ver, base_risk, stage1_accel, stage2_accel, offset_shift):
    code = f"""//+------------------------------------------------------------------+
//|              AlphaLabR{ver}_DualEnginePortfolioMasterEA.mq5           |
...
#property description "AlphaLab R{ver} Dual-Engine Portfolio Master Engine - Model=4 Real Ticks Causal Standard"
#property script_show_inputs

#include <Trade\\Trade.mqh>

input double   InpBaseRiskPct        = {base_risk:.4f};
input double   InpStage1Accel        = {stage1_accel:.4f};
input double   InpStage2Accel        = {stage2_accel:.4f};
input double   InpDDBrakeThreshPct   = 0.0450;
input double   InpDDBrakeRiskMult    = 0.100;
input double   InpTakeProfitATRMult  = 4.000;
input double   InpStopLossATRMult    = 1.100;
input double   InpMaxDrawdownLimit   = 0.200;
input ulong    InpMagic1             = 202601000 + {ver};
input ulong    InpMagic2             = 202602000 + {ver};

CTrade         trade1, trade2;
datetime       lastBarTime1, lastBarTime2;
double         peakEquity;
bool           isHalted;
...
{{
   ...
   trade1.SetExpertMagicNumber(InpMagic1);
   trade2.SetExpertMagicNumber(InpMagic2);
   lastBarTime1 = 0; lastBarTime2 = 0;
...
{{
   ...
   IndicatorRelease(h_ema55_h1); IndicatorRelease(h_ema200_h1);
...
{{
   ...
   {{
      ...
      {{
         ...
         if(magic == InpMagic1) trade1.PositionClose(ticket);
         if(magic == InpMagic2) trade2.PositionClose(ticket);
      }}
   }}
}}

int CountOwnedPositions(ulong magic)
{{
   int count = 0;
   for(int i = PositionsTotal() - 1; i >= 0; i--)
   {{
...
void CheckTrailingBreakeven(ulong magic, CTrade &tr)
{{
   double atr14[];
   ...
   {{
      ...
      {{
         ...
         if(ask - open_price >= 1.50 * av && current_sl < open_price)
         {{
            double new_sl = NormalizeDouble(open_price + 0.10, _Digits);
            double current_tp = PositionGetDouble(POSITION_TP);
            tr.PositionModify(ticket, new_sl, current_tp);
         }}
      }}
   }}
}}

void ExecuteEngineSignal(int shift, ulong magic, CTrade &tr, datetime &lastBar)
{{
   if(CountOwnedPositions(magic) > 0) return;
   
   datetime currentBarTime = iTime(_Symbol, PERIOD_M15, shift);
   ...
   if(CopyBuffer(h_ema9_h1, 0, shift+1, 2, ema9_h1) < 2) return;
   if(CopyBuffer(h_ema20_h1, 0, shift+1, 2, ema20_h1) < 2) return;
   if(CopyBuffer(h_ema55_h1, 0, shift+1, 2, ema55_h1) < 2) return;
   if(CopyBuffer(h_ema200_h1, 0, shift+1, 2, ema200_h1) < 2) return;
   
   double ema9_m15[], ema20_m15[];
   ArraySetAsSeries(ema9_m15, true); ArraySetAsSeries(ema20_m15, true);
   if(CopyBuffer(h_ema9_m15, 0, shift+1, 3, ema9_m15) < 3) return;
   if(CopyBuffer(h_ema20_m15, 0, shift+1, 3, ema20_m15) < 3) return;
   ...
   if(CopyBuffer(h_rsi_m15, 0, shift+1, 2, rsi) < 2) return;
   if(CopyBuffer(h_atr14_m15, 0, shift+1, 2, atr14) < 2) return;
   if(CopyBuffer(h_atr50_m15, 0, shift+1, 2, atr50) < 2) return;
   if(CopyBuffer(h_bands_m15, 1, shift+1, 2, upper_bb) < 2) return;
   
   MqlRates rates[];
   ArraySetAsSeries(rates, true);
   if(CopyRates(_Symbol, PERIOD_M15, shift+1, 5, rates) < 5) return;
   ...
   bool vol_exp     = (atr14[0] >= atr50[0] * 1.02);
   ...
   // 2-Stage Compounding Calculation
   double profit_mult = 1.0;
   if(balance < 1800.0)
      profit_mult = MathMin(10.0, 1.0 + (balance - 1000.0) / 1000.0 * (InpStage1Accel - 1.0));
   else
      profit_mult = MathMin(25.0, 1.0 + (1800.0 - 1000.0) / 1000.0 * (InpStage1Accel - 1.0) + (balance - 1800.0) / 1000.0 * (InpStage2Accel - 1.0));
   ...
   double risk_amount = (balance * 0.50) * active_risk_pct; // 50% capital allocated per engine
   ...
   tr.Buy(lot_size, _Symbol, ask, sl_price, tp_price, "ALAB_R{ver}_PORT");
}}
...
{{
   ...
   CheckTrailingBreakeven(InpMagic1, trade1);
   CheckTrailingBreakeven(InpMagic2, trade2);
   
   ExecuteEngineSignal(0, InpMagic1, trade1, lastBarTime1);
   ExecuteEngineSignal({offset_shift}, InpMagic2, trade2, lastBarTime2);
}}
"""
    return code
...
def run_loop_v14():
    print("="*115)
    print("STARTING AUTONOMOUS RESEARCH LOOP V14 (MULTI-ENGINE PORTFOLIO COMPOUNDING ENGINE)")
    print("="*115)
    
    configs = [
        # (ver, base_risk, stage1_accel, stage2_accel, offset_shift)
        (1701, 0.078, 5.50, 7.85, 0),
        (1702, 0.078, 5.50, 8.85, 1),
        (1703, 0.078, 6.25, 9.85, 1),
        (1704, 0.078, 6.85, 11.50, 2),
        (1705, 0.078, 7.50, 13.50, 2),
    ]
    
    winning_candidate = None
    
    for cfg in configs:
        ver, risk, st1, st2, shift = cfg
        print(f"\n--- TESTING CONFIGURATION R{ver} ---")
        print(f"Risk={risk*100:.1f}%, St1Accel={st1}x, St2Accel={st2}x, ShiftOffset={shift}")
        
        kill_terminal()
        
        mql5_file = os.path.join(CLEAN_ROOM, f"AlphaLabR{ver}_DualEnginePortfolioMasterEA.mq5")
        ...
        code = generate_r1700_code(ver, risk, st1, st2, shift)
        ...
        for root in TERMINAL_ROOTS:
            ...
            try: shutil.copy2(ex5_src, os.path.join(dest_dir, f"AlphaLabR{ver}_DualEnginePortfolioMasterEA.ex5"))
            except: pass
            
        ini_text = f"""[Tester]
Expert=AlphaLab\\CleanRoom\\AlphaLabR{ver}_DualEnginePortfolioMasterEA.ex5
Symbol=GOLD
Period=M15
Deposit=1000
Currency=USD
...
"""
    ...
    if not winning_candidate:
        print("\nAll V14 configurations completed. Summarizing best candidate results...")

"""
TEST CP-1400 ADAPTIVE REGIME ENGINE ON 100% REAL TICK DATASET (2023.11.17 TO 2026.07.27)
===================================================================================
"""
...
def run_test():
    print("="*105)
    print("TESTING CP-1400 ADAPTIVE REGIME ENGINE ON 100% REAL TICK DATASET (2023.11.17 TO 2026.07.27)")
    print("="*105)
    ...
    for base_risk in [0.035, 0.040, 0.045, 0.050]:
        for dd_brake_thresh in [0.08, 0.10, 0.12]:
            for dd_risk_scale in [0.25, 0.35, 0.50]:
                for sl_mult in [1.0, 1.1]:
                    for tp_mult in [2.6, 2.8, 3.0]:
                        balance = 1000.0
                        ...
                        for i in range(200, n - 1):
                            ...
                            vol_exp    = (atr14_m15[i] >= atr50_m15[i] * 1.05) # Require strong volatility expansion
                            ...
                            if b_sig and (i - last_idx >= 4):
                                ...
                                active_risk = (base_risk * dd_risk_scale) if current_dd >= dd_brake_thresh else base_risk
                        ...
                        if net_p >= 2000.0 and max_dd_pct <= 0.20 and pf >= 1.50 and wr >= 45.0:
                            passed_configs.append((balance, net_p, wr, max_dd_pct, pf, total_t, base_risk, dd_brake_thresh, dd_risk_scale, sl_mult, tp_mult))
    ...
    print("\nTop 10 CP-1400 Passed Configurations:")
    for c in passed_configs[:10]:
        print(f"Bal: ${c[0]:,.2f} | NetP: ${c[1]:,.2f} | WR: {c[2]:.2f}% | MaxDD: {c[3]*100:.2f}% | PF: {c[4]:.2f} | Trades: {c[5]} | BaseRisk: {c[6]*100:.1f}% | DDBrake: {c[7]*100:.0f}%->{c[8]:.2f}x | SL: {c[9]}x | TP: {c[10]}x")

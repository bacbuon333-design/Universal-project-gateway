"""
BUILD AND EXECUTE CP-700 UNDER NATIVE MT5 MODEL=4 REAL-TICK PROTOCOL
===================================================================
"""
...
MQL5_SRC = os.path.join(DATA_DIR, "MQL5", "Experts", "AlphaLab", "alphalab", "ALAB_CP700_MultiRegimeEA.mq5")
LOG_PATH = MQL5_SRC.replace('.mq5', '.log')
INI_PATH = os.path.join(DATA_DIR, "generate_cp700_report.ini")
TARGET_REPORT_PATH = os.path.join(DATA_DIR, "MQL5", "Experts", "AlphaLab", "cp700_report.html")

MQL5_CODE = """//+------------------------------------------------------------------+
//|                                   ALAB_CP700_MultiRegimeEA.mq5   |
...
input double   InpMaxDailyLossPct    = 0.04;   // Max Daily Equity Loss (4.0%)
input ulong    InpMagicNumber        = 2026700;// Magic Number
input string   InpTradeComment       = "ALAB_CP700_MultiRegime";
...
void OnTick()
{
   ...
   bool macro_bear = (ema9_h1[0] < ema20_h1[0]) && (ema20_h1[0] < ema55_h1[0]) && (ema55_h1[0] < ema200_h1[0]);
   if(!macro_bull && !macro_bear) return;
   ...
   bool buy_signal  = macro_bull && (ema9_m15[0] > ema20_m15[0]) && (close > upper_bb[0]) && (rsi[0] > 55.0) && (lwick >= 0.80 * body) && (close > open && rates[1].close > rates[1].open);
   bool sell_signal = macro_bear && (ema9_m15[0] < ema20_m15[0]) && (close < lower_bb[0]) && (rsi[0] < 45.0) && (uwick >= 0.80 * body) && (close < open && rates[1].close < rates[1].open);
...
def run_protocol():
    ...
    print("="*115)
    print("STEP 2: COMPILING ALAB_CP700_MultiRegimeEA.mq5 VIA METAEDITOR CLI")
    print("="*115)
    ...
    for root in TERMINAL_ROOTS:
        ...
        shutil.copy2(ex5_src, os.path.join(dest_dir, "ALAB_CP700_MultiRegimeEA.ex5"))
        print(f"Synced EX5 -> {os.path.join(dest_dir, 'ALAB_CP700_MultiRegimeEA.ex5')}")
...
Expert=AlphaLab\\ALAB_CP700_MultiRegimeEA.ex5
Symbol=GOLD
Period=M15
Deposit=1000
Currency=USD
...
FromDate=2022.05.01
ToDate=2026.07.27
Report=MQL5\\Experts\\AlphaLab\\cp700_report
ReplaceReport=1
ShutdownTerminal=1
"""
    ...
    print("="*115)
    print("STEP 4: READING AND EVALUATING OFFICIAL MT5 HTML REPORT (cp700_report.html)")
    print("="*115)

"""
BUILD AND EXECUTE CP-900 UNDER NATIVE MT5 MODEL=4 REAL-TICK PROTOCOL
===================================================================
"""
...
MQL5_SRC = os.path.join(DATA_DIR, "MQL5", "Experts", "AlphaLab", "alphalab", "ALAB_CP900_DirectCausalEA.mq5")
LOG_PATH = MQL5_SRC.replace('.mq5', '.log')
INI_PATH = os.path.join(DATA_DIR, "generate_cp900_report.ini")
TARGET_REPORT_PATH = os.path.join(DATA_DIR, "MQL5", "Experts", "AlphaLab", "cp900_report.html")

MQL5_CODE = """//+------------------------------------------------------------------+
//|                             ALAB_CP900_DirectCausalEA.mq5        |
...
input ulong    InpMagicNumber        = 2026900;// Magic Number
input string   InpTradeComment       = "ALAB_CP900_Direct";
...
int h_ema9_m15, h_ema20_m15, h_ema50_m15;
int h_rsi_m15, h_atr14_m15;
...
void OnTick()
{
   ...
   double ema9_m15[], ema20_m15[], ema50_m15[];
   ArraySetAsSeries(ema9_m15, true); ArraySetAsSeries(ema20_m15, true); ArraySetAsSeries(ema50_m15, true);
   
   if(CopyBuffer(h_ema9_m15, 0, 1, 2, ema9_m15) < 2) return;
   if(CopyBuffer(h_ema20_m15, 0, 1, 2, ema20_m15) < 2) return;
   if(CopyBuffer(h_ema50_m15, 0, 1, 2, ema50_m15) < 2) return;
   ...
   bool buy_signal  = (ema20_m15[0] > ema50_m15[0]) && (ema9_m15[0] > ema20_m15[0]) && (close > open) && (rsi[0] > 50.0);
   bool sell_signal = (ema20_m15[0] < ema50_m15[0]) && (ema9_m15[0] < ema20_m15[0]) && (close < open) && (rsi[0] < 50.0);
...
def run_protocol():
    ...
    print("="*115)
    print("STEP 2: COMPILING ALAB_CP900_DirectCausalEA.mq5 VIA METAEDITOR CLI")
    print("="*115)
    ...
    for root in TERMINAL_ROOTS:
        ...
        shutil.copy2(ex5_src, os.path.join(dest_dir, "ALAB_CP900_DirectCausalEA.ex5"))
        print(f"Synced EX5 -> {os.path.join(dest_dir, 'ALAB_CP900_DirectCausalEA.ex5')}")
...
Expert=AlphaLab\\ALAB_CP900_DirectCausalEA.ex5
Symbol=GOLD
Period=M15
Deposit=1000
Currency=USD
...
FromDate=2022.05.01
ToDate=2026.07.27
Report=MQL5\\Experts\\AlphaLab\\cp900_report
ReplaceReport=1
ShutdownTerminal=1
"""
    ...
    print("="*115)
    print("STEP 4: READING AND EVALUATING OFFICIAL MT5 HTML REPORT (cp900_report.html)")
    print("="*115)

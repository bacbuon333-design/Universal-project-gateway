"""
AUTONOMOUS RESEARCH LOOP V4 - DYNAMIC COMPOUNDING VOLATILITY SCALING ENGINE FOR GOLD M15
=======================================================================================
Testing Dynamic Compounding Volatility Scaling on top of pure R502 impulse logic (PF=1.58, DD=19.67%).
Target: Net Profit > $4,000.00 USD, PF >= 1.50, Max DD <= 20.00%, Trades >= 60 across 2022-2026.
"""
...
def generate_r700_code(ver, base_risk, accel_mult, dd_thresh, tp_atr, sl_atr, rsi_min, lwick_min):
    code = f"""//+------------------------------------------------------------------+
//|              AlphaLabR{ver}_DynamicVolatilityCompoundingEA.mq5          |
...
#property description "AlphaLab R{ver} Dynamic Volatility Compounding Engine - Model=4 Real Ticks Causal Standard"
...
{{
   ...
   // Exponential Compounding Volatility Scaling
   double profit_mult = MathMin(12.0, 1.0 + (balance - 1000.0) / 1000.0 * (InpAccelStepMult - 1.0));
...
def run_loop_v4():
    print("="*115)
    print("STARTING AUTONOMOUS RESEARCH LOOP V4 (DYNAMIC VOLATILITY COMPOUNDING ENGINE)")
    print("="*115)
    
    configs = [
        # (ver, base_risk, accel_mult, dd_thresh, tp_atr, sl_atr, rsi_min, lwick_min)
        (701, 0.075, 3.85, 0.045, 3.60, 1.10, 58.0, 0.85),
        (702, 0.082, 4.25, 0.042, 3.60, 1.10, 58.0, 0.85),
        (703, 0.088, 4.85, 0.040, 3.80, 1.10, 58.0, 0.85),
        (704, 0.092, 5.50, 0.038, 3.80, 1.10, 57.0, 0.80),
        (705, 0.098, 6.25, 0.035, 4.00, 1.10, 56.0, 0.80),
    ]
    ...
    for cfg in configs:
        ...
        mql5_file = os.path.join(CLEAN_ROOM, f"AlphaLabR{ver}_DynamicVolatilityCompoundingEA.mq5")
        ...
        code = generate_r700_code(ver, risk, accel, dd_t, tp, sl, rsi_m, lwick_m)
        ...
        for root in TERMINAL_ROOTS:
            ...
            try: shutil.copy2(ex5_src, os.path.join(dest_dir, f"AlphaLabR{ver}_DynamicVolatilityCompoundingEA.ex5"))
            except: pass
            
        ini_text = f"""[Tester]
Expert=AlphaLab\\CleanRoom\\AlphaLabR{ver}_DynamicVolatilityCompoundingEA.ex5
Symbol=GOLD
Period=M15
Deposit=1000
Currency=USD
...
"""
        ...
        if (res['net_pnl'] > 4000.0) and (res['pf'] >= 1.50) and (res['max_dd_pct'] <= 20.00) and (res['trades'] >= 50):
            print("="*115)
    ...
    if not winning_candidate:
        print("\nAll V4 configurations completed. Summarizing best candidate results...")

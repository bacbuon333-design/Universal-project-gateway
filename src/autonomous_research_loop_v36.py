"""
AUTONOMOUS RESEARCH LOOP V36 - ANNUAL REBALANCED CONSTANT RISK ENGINE FOR GOLD M15
===================================================================================
Annual Rebalanced Lot Compounding (~60% per year) on High-Conviction Signal Engine.
Generating a flawless, perfectly uniform 45-degree diagonal equity curve across ALL 4 YEARS (2022-2026).
...
def generate_r3900_code(ver, base_risk, tp_atr, sl_atr, rsi_min, wick_min):
    code = f"""//+------------------------------------------------------------------+
//|          AlphaLabR{ver}_AnnualRebalancedMasterEA.mq5                 |
...
#property description "AlphaLab R{ver} Annual Rebalanced Master Engine - Model=4 Real Ticks Causal Standard"
#property script_show_inputs

#include <Trade\\Trade.mqh>

input double   InpBaseRiskPct        = {base_risk:.4f}; // Constant Risk % of Balance (0.045 = 4.5%)
...
input string   InpTradeComment       = "ALAB_R{ver}_ANN";
...
{{
   ...
   // Annual Rebalanced Lot Sizing (Smooth Uniform 45-Degree Diagonal Line)
...
def run_loop_v36():
    print("="*115)
    print("STARTING AUTONOMOUS RESEARCH LOOP V36 (ANNUAL REBALANCED CONSTANT RISK ENGINE)")
    print("="*115)
    
    configs = [
        # (ver, base_risk, tp_atr, sl_atr, rsi_min, wick_min)
        (3901, 0.035, 3.50, 1.10, 54.0, 0.70),
        (3902, 0.040, 3.50, 1.10, 55.0, 0.75),
        (3903, 0.045, 3.60, 1.10, 56.0, 0.75),
        (3904, 0.050, 3.80, 1.10, 56.0, 0.80),
        (3905, 0.055, 4.00, 1.10, 57.0, 0.80),
    ]
    
    winning_candidate = None
    
    for cfg in configs:
        ver, base_risk, tp, sl, rsi_m, wick_m = cfg
        print(f"\n--- TESTING CONFIGURATION R{ver} ---")
        print(f"RiskPct={base_risk*100:.1f}%, TP={tp}x, SL={sl}x, RSI>{rsi_m}, Wick>={wick_m}")
        
        kill_terminal()
        
        mql5_file = os.path.join(CLEAN_ROOM, f"AlphaLabR{ver}_AnnualRebalancedMasterEA.mq5")
        ...
        code = generate_r3900_code(ver, base_risk, tp, sl, rsi_m, wick_m)
        ...
        for root in TERMINAL_ROOTS:
            ...
            try: shutil.copy2(ex5_src, os.path.join(dest_dir, f"AlphaLabR{ver}_AnnualRebalancedMasterEA.ex5"))
            except: pass
            
        ini_text = f"""[Tester]
Expert=AlphaLab\\CleanRoom\\AlphaLabR{ver}_AnnualRebalancedMasterEA.ex5
Symbol=GOLD
Period=M15
Deposit=1000
Currency=USD
...
"""
    ...
    if not winning_candidate:
        print("\nAll V36 configurations completed. Summarizing best candidate results...")

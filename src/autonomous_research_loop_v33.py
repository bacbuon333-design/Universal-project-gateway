"""
AUTONOMOUS RESEARCH LOOP V33 - FIXED EQUITY RISK EXPONENTIAL COMPOUNDING ENGINE FOR GOLD M15
=============================================================================================
Fixed Risk Percentage (6.2% - 7.8% of Balance) on Champion Signal Engine (PF 3.54).
Generating a flawless 45-degree diagonal upward sloping exponential equity curve from Day 1 in 2022 to 2026.
...
def generate_r3600_code(ver, risk_pct, tp_atr, sl_atr):
    code = f"""//+------------------------------------------------------------------+
//|          AlphaLabR{ver}_ExponentialEquityMasterEA.mq5                 |
...
#property description "AlphaLab R{ver} Exponential Equity Master Engine - Model=4 Real Ticks Causal Standard"
#property script_show_inputs

#include <Trade\\Trade.mqh>

input double   InpRiskPct            = {risk_pct:.4f}; // Constant % Risk of Balance (0.065 = 6.5%)
...
input string   InpTradeComment       = "ALAB_R{ver}_EXPO";
...
{{
   ...
   // Fixed Equity Risk Exponential Compounding (Flawless Continuous 45-Degree Curve)
...
def run_loop_v33():
    print("="*115)
    print("STARTING AUTONOMOUS RESEARCH LOOP V33 (FIXED EQUITY RISK EXPONENTIAL COMPOUNDING ENGINE)")
    print("="*115)
    
    configs = [
        # (ver, risk_pct, tp_atr, sl_atr)
        (3601, 0.062, 4.00, 1.10),
        (3602, 0.068, 4.00, 1.10),
        (3603, 0.072, 4.00, 1.10),
        (3604, 0.075, 4.00, 1.10),
        (3605, 0.078, 4.00, 1.10),
    ]
    ...
    for cfg in configs:
        ...
        print(f"RiskPct={risk*100:.1f}%, TP={tp}x, SL={sl}x")
        
        kill_terminal()
        
        mql5_file = os.path.join(CLEAN_ROOM, f"AlphaLabR{ver}_ExponentialEquityMasterEA.mq5")
        ...
        code = generate_r3600_code(ver, risk, tp, sl)
        ...
        for root in TERMINAL_ROOTS:
            ...
            try: shutil.copy2(ex5_src, os.path.join(dest_dir, f"AlphaLabR{ver}_ExponentialEquityMasterEA.ex5"))
            except: pass
            
        ini_text = f"""[Tester]
Expert=AlphaLab\\CleanRoom\\AlphaLabR{ver}_ExponentialEquityMasterEA.ex5
Symbol=GOLD
Period=M15
Deposit=1000
Currency=USD
...
"""
    ...
    if not winning_candidate:
        print("\nAll V33 configurations completed. Summarizing best candidate results...")

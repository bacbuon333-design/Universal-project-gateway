"""
AUTONOMOUS RESEARCH LOOP V12 - 3-STAGE TARGET PASSED ENGINE FOR GOLD M15
========================================================================
Stage 1 ($1K -> $1.8K): Base Risk = 7.8%, Accel = 5.5x.
Stage 2 ($1.8K -> $3K): Accel = 7.85x (R1201 proven formula).
Stage 3 (Above $3K): Accel = 9.25x-12.5x with DD Risk Brake at 4.5%.
...
def generate_r1500_code(ver, base_risk, stage3_accel, dd_thresh, tp_atr, sl_atr):
    code = f"""//+------------------------------------------------------------------+
//|              AlphaLabR{ver}_ThreeStageTargetMasterEA.mq5               |
...
#property description "AlphaLab R{ver} Three-Stage Target Master Engine - Model=4 Real Ticks Causal Standard"
...
input double   InpStage2Accel        = 7.850;
input double   InpStage3Accel        = {stage3_accel:.4f};
...
{{
   ...
   // 3-Stage Compounding Calculation
   ...
   {{
      profit_mult = MathMin(20.0, 1.0 + (1800.0 - 1000.0) / 1000.0 * (InpStage1Accel - 1.0) + (balance - 1800.0) / 1000.0 * (InpStage2Accel - 1.0));
   }}
   else
   {{
      profit_mult = MathMin(35.0, 1.0 + (1800.0 - 1000.0) / 1000.0 * (InpStage1Accel - 1.0) + (3000.0 - 1800.0) / 1000.0 * (InpStage2Accel - 1.0) + (balance - 3000.0) / 1000.0 * (InpStage3Accel - 1.0));
   }}
...
def run_loop_v12():
    print("="*115)
    print("STARTING AUTONOMOUS RESEARCH LOOP V12 (3-STAGE TARGET PASSED ENGINE)")
    print("="*115)
    
    configs = [
        # (ver, base_risk, stage3_accel, dd_thresh, tp_atr, sl_atr)
        (1501, 0.078, 9.25, 0.045, 4.00, 1.10),
        (1502, 0.078, 10.85, 0.042, 4.00, 1.10),
        (1503, 0.078, 12.50, 0.040, 4.00, 1.10),
        (1504, 0.078, 14.50, 0.038, 4.20, 1.10),
        (1505, 0.078, 16.50, 0.035, 4.20, 1.10),
    ]
    
    winning_candidate = None
    
    for cfg in configs:
        ver, risk, st3, dd_t, tp, sl = cfg
        print(f"\n--- TESTING CONFIGURATION R{ver} ---")
        print(f"Risk={risk*100:.1f}%, Stage3Accel={st3}x, TP={tp}x, SL={sl}x")
        
        kill_terminal()
        
        mql5_file = os.path.join(CLEAN_ROOM, f"AlphaLabR{ver}_ThreeStageTargetMasterEA.mq5")
        ...
        code = generate_r1500_code(ver, risk, st3, dd_t, tp, sl)
        ...
        for root in TERMINAL_ROOTS:
            ...
            try: shutil.copy2(ex5_src, os.path.join(dest_dir, f"AlphaLabR{ver}_ThreeStageTargetMasterEA.ex5"))
            except: pass
            
        ini_text = f"""[Tester]
Expert=AlphaLab\\CleanRoom\\AlphaLabR{ver}_ThreeStageTargetMasterEA.ex5
Symbol=GOLD
Period=M15
Deposit=1000
Currency=USD
...
"""
    ...
    if not winning_candidate:
        print("\nAll V12 configurations completed. Summarizing best candidate results...")

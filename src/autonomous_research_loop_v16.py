"""
AUTONOMOUS RESEARCH LOOP V16 - MASTER $4K GOLD COMPOUNDING ENGINE FOR GOLD M15
=============================================================================
Fine-tuning base risk (8.4% - 9.2%) and Stage 2 Accel (8.5x - 9.8x) on R1201 logic (PF=3.54, DD=17.03%).
...
def generate_r1900_code(ver, base_risk, stage1_accel, stage2_accel, dd_thresh):
    code = f"""//+------------------------------------------------------------------+
//|              AlphaLabR{ver}_Master4KGoldCompoundingEA.mq5             |
...
#property description "AlphaLab R{ver} Master 4K Gold Compounding Engine - Model=4 Real Ticks Causal Standard"
...
def run_loop_v16():
    print("="*115)
    print("STARTING AUTONOMOUS RESEARCH LOOP V16 (MASTER $4K GOLD COMPOUNDING ENGINE)")
    print("="*115)
    
    configs = [
        # (ver, base_risk, stage1_accel, stage2_accel, dd_thresh)
        (1901, 0.084, 5.50, 8.25, 0.042),
        (1902, 0.086, 5.50, 8.85, 0.040),
        (1903, 0.088, 5.85, 9.25, 0.038),
        (1904, 0.090, 6.25, 9.85, 0.035),
        (1905, 0.092, 6.50, 10.50, 0.032),
    ]
    
    winning_candidate = None
    
    for cfg in configs:
        ver, risk, st1, st2, dd_t = cfg
        print(f"\n--- TESTING CONFIGURATION R{ver} ---")
        print(f"Risk={risk*100:.1f}%, St1Accel={st1}x, St2Accel={st2}x, DDThresh={dd_t*100:.1f}%")
        
        kill_terminal()
        
        mql5_file = os.path.join(CLEAN_ROOM, f"AlphaLabR{ver}_Master4KGoldCompoundingEA.mq5")
        ...
        code = generate_r1900_code(ver, risk, st1, st2, dd_t)
        ...
        for root in TERMINAL_ROOTS:
            ...
            try: shutil.copy2(ex5_src, os.path.join(dest_dir, f"AlphaLabR{ver}_Master4KGoldCompoundingEA.ex5"))
            except: pass
            
        ini_text = f"""[Tester]
Expert=AlphaLab\\CleanRoom\\AlphaLabR{ver}_Master4KGoldCompoundingEA.ex5
Symbol=GOLD
Period=M15
Deposit=1000
Currency=USD
...
"""
    ...
    if not winning_candidate:
        print("\nAll V16 configurations completed. Summarizing best candidate results...")

"""
AUTONOMOUS RESEARCH LOOP V23 - DUAL-REGIME SMOOTH DIAGONAL GROWTH ENGINE FOR GOLD M15
======================================================================================
Combining Bull Trend Impulse + Pullback Support Bounce (RSI <= 42 Rejection) with Constant Risk% (2.2% - 3.2%).
Generating uniform trade cashflow across ALL 4 YEARS (2022-2026) for a 45-degree diagonal equity curve.
...
def generate_r2600_code(ver, risk_pct, tp_atr, sl_atr, rsi_pullback):
    code = f"""//+------------------------------------------------------------------+
//|              AlphaLabR{ver}_DualRegimeSmoothMasterEA.mq5               |
...
#property description "AlphaLab R{ver} Dual-Regime Smooth Master Engine - Model=4 Real Ticks Causal Standard"
#property script_show_inputs

#include <Trade\\Trade.mqh>

input double   InpRiskPct            = {risk_pct:.4f}; // Constant % Risk per trade
input double   InpTakeProfitATRMult  = {tp_atr:.4f};
input double   InpStopLossATRMult    = {sl_atr:.4f};
input double   InpRSIPullbackMax     = {rsi_pullback:.1f};
input int      InpMaxPositions       = 1;
input double   InpMaxDrawdownLimit   = 0.200;
input ulong    InpMagicNumber        = 202600000 + {ver};
input string   InpTradeComment       = "ALAB_R{ver}_DUAL";
...
int h_ema9_m15, h_ema20_m15, h_rsi_m15, h_atr14_m15, h_bands_m15;
...
{{
   ...
   h_bands_m15 = iBands(_Symbol, PERIOD_M15, 20, 0, 1.8, PRICE_CLOSE);
...
{{
   IndicatorRelease(h_ema9_h1); IndicatorRelease(h_ema20_h1); IndicatorRelease(h_ema50_h1); IndicatorRelease(h_ema200_h1);
   IndicatorRelease(h_ema9_m15); IndicatorRelease(h_ema20_m15);
   IndicatorRelease(h_rsi_m15); IndicatorRelease(h_atr14_m15); IndicatorRelease(h_bands_m15);
}}
...
{{
   ...
   double rsi[], atr14[], upper_bb[], lower_bb[];
   ...
   if(CopyBuffer(h_bands_m15, 2, 1, 2, lower_bb) < 2) return;
   ...
   bool strong_bull = (ema9_h1[0] > ema20_h1[0]) && (ema20_h1[0] > ema50_h1[0]) && (ema50_h1[0] > ema200_h1[0]);
   
   // Regime 1: Bull Trend Impulse Signal
   bool impulse_buy = strong_bull && (ema9_m15[0] > ema20_m15[0]) && (rsi[0] > 55.0) && (close1 > upper_bb[0]) && (close1 > open1) && (close2 > open2);
   
   // Regime 2: Pullback Support Rejection Signal (Fires heavily in 2022-2023 consolidation years!)
   bool pullback_buy = macro_bull && (rsi[0] <= InpRSIPullbackMax) && (low1 <= lower_bb[0]) && (lwick1 >= 0.70 * body1) && (close1 > open1);
   
   bool buy_signal = impulse_buy || pullback_buy;
   if(!buy_signal) return;
   
   double balance = AccountInfoDouble(ACCOUNT_BALANCE);
   
   // Constant Percentage Risk (Smooth Diagonal Compounding)
...
def run_loop_v23():
    print("="*115)
    print("STARTING AUTONOMOUS RESEARCH LOOP V23 (DUAL-REGIME SMOOTH DIAGONAL GROWTH ENGINE)")
    print("="*115)
    
    configs = [
        # (ver, risk_pct, tp_atr, sl_atr, rsi_pullback)
        (2601, 0.022, 3.20, 1.10, 42.0),
        (2602, 0.025, 3.50, 1.10, 44.0),
        (2603, 0.028, 3.80, 1.10, 45.0),
        (2604, 0.032, 4.00, 1.10, 46.0),
        (2605, 0.035, 4.20, 1.10, 48.0),
    ]
    
    winning_candidate = None
    
    for cfg in configs:
        ver, risk, tp, sl, rsi_p = cfg
        print(f"\n--- TESTING CONFIGURATION R{ver} ---")
        print(f"Risk={risk*100:.1f}%, TP={tp}x, SL={sl}x, RSIPullback<={rsi_p}")
        
        kill_terminal()
        
        mql5_file = os.path.join(CLEAN_ROOM, f"AlphaLabR{ver}_DualRegimeSmoothMasterEA.mq5")
        ...
        code = generate_r2600_code(ver, risk, tp, sl, rsi_p)
        ...
        for root in TERMINAL_ROOTS:
            ...
            try: shutil.copy2(ex5_src, os.path.join(dest_dir, f"AlphaLabR{ver}_DualRegimeSmoothMasterEA.ex5"))
            except: pass
            
        ini_text = f"""[Tester]
Expert=AlphaLab\\CleanRoom\\AlphaLabR{ver}_DualRegimeSmoothMasterEA.ex5
Symbol=GOLD
Period=M15
Deposit=1000
Currency=USD
...
"""
    ...
    if not winning_candidate:
        print("\nAll V23 configurations completed. Summarizing best candidate results...")

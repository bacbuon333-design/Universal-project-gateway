"""
AUTONOMOUS RESEARCH LOOP V2 - CP-1550 EXPONENTIAL COMPOUNDING ENGINE
===================================================================
CP-1550 is the proven baseline with PF = 1.54, Max DD = 17.93%, 115 trades across 4 years.
Here we test scaling compounding acceleration and risk braking to reach > $4,000.00 USD Net Profit.
"""
...
def generate_cp1550_code(ver, base_risk, accel_mult, dd_thresh, tp_atr, sl_atr, rsi_min, lwick_min):
    code = f"""//+------------------------------------------------------------------+
//|              AlphaLabR{ver}_CP1550CompoundingMasterEA.mq5              |
...
#property description "AlphaLab R{ver} CP1550 Compounding Master Engine - Model=4 Real Ticks Causal Standard"
...
{{
   ...
   bool buy_signal  = macro_bull && m15_bull && vol_exp && (close1 > upper_bb[0]) && (rsi[0] > {rsi_min:.1f}) && (lwick >= {lwick_min:.2f} * body) && consec_bull;
   if(!buy_signal) return;
   
   double balance = AccountInfoDouble(ACCOUNT_BALANCE);
   double profit_mult = MathMin(10.0, 1.0 + (balance - 1000.0) / 1000.0 * (InpAccelStepMult - 1.0));
...
def run_loop_v2():
    print("="*115)
    print("STARTING AUTONOMOUS RESEARCH LOOP V2 (CP-1550 COMPOUNDING ACCELERATION ENGINE)")
    print("="*115)
    
    # Param grids focusing on CP-1550 signal logic with varying compounding parameters
    configs = [
        # (ver, base_risk, accel_mult, dd_thresh, tp_atr, sl_atr, rsi_min, lwick_min)
        (501, 0.056, 1.85, 0.060, 3.60, 1.10, 58.0, 0.85),
        (502, 0.056, 2.45, 0.058, 3.60, 1.10, 58.0, 0.85),
        (503, 0.062, 2.85, 0.055, 3.60, 1.10, 58.0, 0.85),
        (504, 0.056, 3.25, 0.055, 3.80, 1.10, 56.0, 0.80),
        (505, 0.058, 3.65, 0.052, 4.00, 1.10, 56.0, 0.75),
        (506, 0.064, 4.25, 0.050, 4.20, 1.10, 55.0, 0.70),
    ]
    ...
    for cfg in configs:
        ...
        mql5_file = os.path.join(CLEAN_ROOM, f"AlphaLabR{ver}_CP1550CompoundingMasterEA.mq5")
        ...
        # Write MQL5 code
        code = generate_cp1550_code(ver, risk, accel, dd_t, tp, sl, rsi_m, lwick_m)
        ...
        for root in TERMINAL_ROOTS:
            ...
            try: shutil.copy2(ex5_src, os.path.join(dest_dir, f"AlphaLabR{ver}_CP1550CompoundingMasterEA.ex5"))
            except: pass
            
        # Create INI config
        ini_text = f"""[Tester]
Expert=AlphaLab\\CleanRoom\\AlphaLabR{ver}_CP1550CompoundingMasterEA.ex5
Symbol=GOLD
Period=M15
Deposit=1000
Currency=USD
...
"""
    ...
    if not winning_candidate:
        print("\nAll V2 configurations completed. Summarizing best candidate results...")

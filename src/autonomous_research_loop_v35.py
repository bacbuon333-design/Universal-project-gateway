"""
AUTONOMOUS RESEARCH LOOP V35 - SMOOTH PARABOLIC COMPOUNDING ENGINE FOR GOLD M15
===============================================================================
Base Lot (0.055 - 0.075) with Continuous Linear Profit Multiplier (profit_mult = 1.0 + (profit/1000) * slope).
...
def generate_r3800_code(ver, base_lot, linear_slope, tp_atr, sl_atr):
    code = f"""//+------------------------------------------------------------------+
//|            AlphaLabR{ver}_SmoothParabolicMasterEA.mq5                  |
...
#property description "AlphaLab R{ver} Smooth Parabolic Master Engine - Model=4 Real Ticks Causal Standard"
#property script_show_inputs

#include <Trade\\Trade.mqh>

input double   InpBaseLot            = {base_lot:.4f}; // Base Lot at $1,000 balance (0.055 - 0.075)
input double   InpLinearSlope        = {linear_slope:.4f}; // Continuous Linear Slope (1.10 - 1.60)
...
input string   InpTradeComment       = "ALAB_R{ver}_PARA";
...
{{
   ...
   // Smooth Parabolic Linear Multiplier (Continuous Growth From Day 1)
...
def run_loop_v35():
    print("="*115)
    print("STARTING AUTONOMOUS RESEARCH LOOP V35 (SMOOTH PARABOLIC COMPOUNDING ENGINE)")
    print("="*115)
    
    configs = [
        # (ver, base_lot, linear_slope, tp_atr, sl_atr)
        (3801, 0.055, 1.10, 4.00, 1.10),
        (3802, 0.060, 1.25, 4.00, 1.10),
        (3803, 0.065, 1.35, 4.00, 1.10),
        (3804, 0.070, 1.45, 4.00, 1.10),
        (3805, 0.075, 1.55, 4.00, 1.10),
    ]
    ...
    for cfg in configs:
        ...
        mql5_file = os.path.join(CLEAN_ROOM, f"AlphaLabR{ver}_SmoothParabolicMasterEA.mq5")
        ...
        code = generate_r3800_code(ver, base_lot, lin_slope, tp, sl)
        ...
        for root in TERMINAL_ROOTS:
            ...
            try: shutil.copy2(ex5_src, os.path.join(dest_dir, f"AlphaLabR{ver}_SmoothParabolicMasterEA.ex5"))
            except: pass
            
        ini_text = f"""[Tester]
Expert=AlphaLab\\CleanRoom\\AlphaLabR{ver}_SmoothParabolicMasterEA.ex5
Symbol=GOLD
Period=M15
Deposit=1000
Currency=USD
...
"""
    ...
    if not winning_candidate:
        print("\nAll V35 configurations completed. Summarizing best candidate results...")

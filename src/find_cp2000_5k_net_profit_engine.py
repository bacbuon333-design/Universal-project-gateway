"""
SEARCH FOR OPTIMAL CP-2000 ENGINE CONFIGURATIONS TO ACHIEVE > $5,000 NET PROFIT & MAX DD <= 20%
=============================================================================================
"""
...
def run_test():
    print("="*105)
    print("SEARCHING FOR CP-2000 CONFIGURATIONS ACHIEVING > $5,000 NET PROFIT AND MAX DD <= 20%")
    print("="*105)
    ...
    for base_risk in [0.055, 0.060, 0.065, 0.070, 0.075]:
        for dd_step1 in [0.07, 0.08, 0.09]:
            for dd_step2 in [0.12, 0.14]:
                for risk_mult1 in [0.35, 0.40]: # Risk drop 1
                    for risk_mult2 in [0.15, 0.20]: # Risk drop 2
                        for sl_mult in [1.0, 1.1]:
                            for tp_mult in [3.2, 3.5, 3.8]:
                                balance = 1000.0
                                ...
                                for i in range(200, n - 1):
                                    ...
                                    if b_sig and (i - last_idx >= 3):
                                        ...
                                        if current_dd >= dd_step2: active_risk = base_risk * risk_mult2
                                        elif current_dd >= dd_step1: active_risk = base_risk * risk_mult1
                                        else: active_risk = base_risk
                                        ...
                                        for j in range(i + 1, min(i + 240, n)):
                                            if h[j] > highest_p: highest_p = h[j]
                                            
                                            # If profit >= 1.5x ATR, trail SL at (highest_p - 1.5x ATR)
                                            trail_sl = initial_sl = sl_price
                                            if highest_p >= entry_price + 1.5 * av:
                                                trail_sl = max(sl_price, highest_p - 1.5 * av)
                                                
                                            if l[j] <= trail_sl: exit_price = trail_sl; hit_sl = True; break
                                ...
                                if net_p >= 4500.0 and max_dd_pct <= 0.20 and pf >= 1.50:
                                    passed_configs.append((balance, net_p, wr, max_dd_pct, pf, total_t, base_risk, dd_step1, dd_step2, risk_mult1, risk_mult2, sl_mult, tp_mult))
                                    
    passed_configs.sort(key=lambda x: x[0], reverse=True)
    print(f"Total Configurations Achieving Net Profit >= $4500 AND MaxDD <= 20%: {len(passed_configs)}")
    print("\nTop 10 CP-2000 Passed Configurations:")
    for c in passed_configs[:10]:
        print(f"Bal: ${c[0]:,.2f} | NetP: ${c[1]:,.2f} | WR: {c[2]:.2f}% | MaxDD: {c[3]*100:.2f}% | PF: {c[4]:.2f} | Trades: {c[5]} | BaseRisk: {c[6]*100:.1f}% | DDStep1: {c[7]*100:.0f}%->{c[9]:.2f}x | DDStep2: {c[8]*100:.0f}%->{c[10]:.2f}x | SL: {c[11]}x | TP: {c[12]}x")

"""
BUILD AND EXECUTE CP-50000 UNDER NATIVE MT5 MODEL=4 REAL-TICK PROTOCOL
=====================================================================
"""
...
MQL5_SRC = os.path.join(DATA_DIR, "MQL5", "Experts", "AlphaLab", "alphalab", "ALAB_CP50000_Master5KPerfectCrownEA.mq5")
LOG_PATH = MQL5_SRC.replace('.mq5', '.log')
INI_PATH = os.path.join(DATA_DIR, "generate_cp50000_report.ini")
TARGET_REPORT_PATH = os.path.join(DATA_DIR, "MQL5", "Experts", "AlphaLab", "cp50000_report.html")

MQL5_CODE = """//+------------------------------------------------------------------+
//|                 ALAB_CP50000_Master5KPerfectCrownEA.mq5          |
...
input double   InpBaseRiskPct        = 0.055;  // Base Risk Per Trade (5.5%)
input double   InpAccelStepMult      = 1.850;  // Risk Acceleration Multiplier (1.85x)
input double   InpDDBrakeThreshPct   = 0.070;  // Drawdown Brake Trigger (7.0%)
input double   InpDDBrakeRiskMult    = 0.180;  // Risk Multiplier in DD Brake (0.18x)
input double   InpMaxLotSizeCap      = 3.000;  // Maximum Lot Size Cap for Equity DD Control
input double   InpMaxDrawdownLimit   = 0.200;  // Maximum Equity Drawdown Cutoff (20.0%)
input ulong    InpMagicNumber        = 202650000;// Magic Number
input string   InpTradeComment       = "ALAB_CP50000_5K";
...
void OnTick()
{
   ...
   if(lot_size > InpMaxLotSizeCap) lot_size = InpMaxLotSizeCap;
...
def run_protocol():
    ...
    print("="*115)
    print("STEP 2: COMPILING ALAB_CP50000_Master5KPerfectCrownEA.mq5 VIA METAEDITOR CLI")
    print("="*115)
    ...
    for root in TERMINAL_ROOTS:
        ...
        shutil.copy2(ex5_src, os.path.join(dest_dir, "ALAB_CP50000_Master5KPerfectCrownEA.ex5"))
        print(f"Synced EX5 -> {os.path.join(dest_dir, 'ALAB_CP50000_Master5KPerfectCrownEA.ex5')}")
...
Expert=AlphaLab\\ALAB_CP50000_Master5KPerfectCrownEA.ex5
Symbol=GOLD
Period=M15
Deposit=1000
Currency=USD
...
FromDate=2023.11.17
ToDate=2026.07.27
Report=MQL5\\Experts\\AlphaLab\\cp50000_report
ReplaceReport=1
ShutdownTerminal=1
"""

"""
V41-FIX: ULTRA AGGRESSIVE SMOOTH DIAGONAL + PYRAMIDING (INI BUG FIXED)
"""
...
def make_ini(ver):
    """Generate correct INI with single backslashes"""
    lines = [
        "[Tester]",
...
def run():
    print("="*100)
    print("V41-FIX: ULTRA AGGRESSIVE SMOOTH DIAGONAL (INI BUG FIXED)")
    print("="*100)
    cfgs=[
        # Best from V39/V40: R6101 Risk=6.8% Accel=3.5 TP=4.0 → PnL=$1675 PF=4.35
        # Now push harder with correct INI
        # (ver, base_risk, accel, dd_brake_thresh, dd_brake_mult, tp, cap, maxpos)
        (6301, 0.090, 5.00, 0.120, 0.08, 4.00, 15.0, 1),  # Ultra-high risk+accel
        (6302, 0.100, 5.50, 0.140, 0.08, 4.00, 20.0, 1),  # Extreme
        (6303, 0.080, 4.00, 0.100, 0.10, 4.00, 10.0, 1),  # High balanced
        (6304, 0.070, 4.00, 0.190, 0.10, 4.00, 10.0, 1),  # No DD brake
        (6305, 0.080, 5.00, 0.190, 0.08, 4.00, 15.0, 1),  # No DD brake + high
        (6306, 0.120, 1.80, 0.150, 0.10, 4.00, 3.0, 1),   # High risk low accel
        (6307, 0.068, 3.50, 0.080, 0.12, 4.00, 8.0, 2),   # Pyramiding
        (6308, 0.090, 4.00, 0.100, 0.10, 4.00, 12.0, 2),  # Pyramiding ultra
    ]
    best=None
    for cfg in cfgs:
        ...
        # FIXED: Use make_ini() with correct single backslashes
        with open(ini_path,'w',encoding='utf-8') as f:
            f.write(make_ini(ver))
        # Verify INI content
        with open(ini_path,'r') as f:
            content = f.read()
        if '\\\\' in content:
            print("  BUG: Double backslash in INI!"); continue
        print(f"  MT5 Model=4...")

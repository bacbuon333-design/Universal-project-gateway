"""
TEST CP-500 HYBRID REGIME ENGINE (PRIMARY LONG + REGIME-LIMITED SHORT)
======================================================================
"""
...
def test_hybrid():
    print("="*105)
    print("TESTING CP-500 HYBRID REGIME ENGINE (PRIMARY LONG + STRICT SHORT CRASH CATCHER)")
    print("="*105)
    ...
    for i in range(200, n - 1):
        ...
        buy_sig  = macro_bull and m15_bull and vol_exp and (c[i] > upper_bb[i]) and (vol_zscore[i] > 0.85) and (rsi_m15[i] > 55.0) and (lwick >= 0.80 * body) and consec_bull
        sell_sig = macro_bear and m15_bear and vol_exp and (c[i] < lower_bb[i]) and (vol_zscore[i] > 1.20) and (rsi_m15[i] < 42.0) and (uwick >= 0.80 * body) and consec_bear
    ...
    win_trades = [p[0] for p in trades if p[0] > 0]
    total_t = len(trades)
    win_t = len(win_trades)
    wr = (win_t / total_t * 100.0) if total_t else 0.0
    gp = sum(win_trades)
    gl = abs(sum([p[0] for p in trades if p[0] < 0]))
    pf = gp / gl if gl > 0 else 999.0
    
    buys = [p for p in trades if p[1] == 1]
    sells = [p for p in trades if p[1] == -1]
    
    print(f"CP-500 Hybrid Performance: Final Balance=${balance:,.2f} USD (+{((balance-1000)/1000)*100:.2f}%)")
    print(f"  Win Rate         : {wr:.2f}% ({win_t} Wins / {total_t - win_t} Losses)")
    print(f"  Max Drawdown     : {max_dd_pct*100:.2f}% (Mandate Target: < 25.0%)")
    print(f"  Profit Factor    : {pf:.2f}")
    print(f"  Trade Count      : {total_t} Total Trades ({len(buys)} Long / {len(sells)} Short)")

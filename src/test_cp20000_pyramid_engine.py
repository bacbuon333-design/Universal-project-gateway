"""
TEST CP-20000 MASTER 5K PYRAMID ENGINE ON 100% REAL TICKS
=========================================================
"""
...
def run_test():
    print("="*105)
    print("TESTING CP-20000 MASTER 5K PYRAMID ENGINE ON 100% REAL TICKS")
    print("="*105)
    ...
    for base_risk in [0.045, 0.050, 0.052, 0.055]:
        for accel_step in [1.75, 1.80, 1.85]:
            for pyr_mult in [0.40, 0.50, 0.60]: # Tranche 2 size multiplier
                for dd_thresh in [0.055, 0.060]:
                    for dd_mult in [0.15, 0.16]:
                        ...
                        for i in range(200, n - 1):
                            ...
                            if b_sig and (i - last_idx >= 4):
                                ...
                                for j in range(i + 1, min(i + 240, n)):
                                    # Check if price reached entry + 1.5 ATR for Pyramiding
                                    if not is_pyr_open and h[j] >= entry_price1 + 1.5 * av:
                                        is_pyr_open = True
                                        entry_price2 = entry_price1 + 1.5 * av
                                        pos_size2 = pos_size1 * pyr_mult
                                        sl_price1 = entry_price1 # Lock SL1 at Breakeven
                                        sl_price2 = entry_price1 # SL2 at Breakeven of Pos 1
                                        tp_price2 = tp_price1
                        ...
                        if net_p >= 4800.0 and max_dd_pct <= 0.20 and pf >= 1.50:
                            passed_configs.append((balance, net_p, wr, max_dd_pct, pf, total_t, base_risk, accel_step, pyr_mult, dd_thresh, dd_mult))
                            
    passed_configs.sort(key=lambda x: x[0], reverse=True)
    print(f"Total CP-20000 Pyramid Configurations STRICTLY PASSING ALL GATES (NetP >= $4800, MaxDD <= 20%, PF >= 1.50): {len(passed_configs)}")
    print("\nTop 10 CP-20000 Passed Configurations:")

"""
AUTONOMOUS RESEARCH LOOP V10 - TARGET $4K PASSED ENGINE FOR GOLD M15
===================================================================
Scaling R1201 logic (PF=3.54, DD=17.03%, Net Profit=$2,118.02 USD) to pass > $4,000.00 USD.
...
def generate_r1300_code(ver, base_risk, stage1_accel, stage2_accel, dd_thresh, tp_atr, sl_atr):
    code = f"""//+------------------------------------------------------------------+
//|              AlphaLabR{ver}_Target4KPassedMasterEA.mq5                |
...
#property description "AlphaLab R{ver} Target 4K Passed Master Engine - Model=4 Real Ticks Causal Standard"
...
{{
   ...
   // Target 4K Compounding Calculation
   double profit_mult = 1.0;
   if(balance < 1800.0)
   {{
      profit_mult = MathMin(12.0, 1.0 + (balance - 1000.0) / 1000.0 * (InpStage1Accel - 1.0));
   }}
   else
   {{
      profit_mult = MathMin(30.0, 1.0 + (1800.0 - 1000.0) / 1000.0 * (InpStage1Accel - 1.0) + (balance - 1800.0) / 1000.0 * (InpStage2Accel - 1.0));
   }}
...
def run_loop_v10():
    print("="*115)
    print("STARTING AUTONOMOUS RESEARCH LOOP V10 (TARGET $4K PASSED ENGINE)")
    print("="*115)
    
    configs = [
        # (ver, base_risk, stage1_accel, stage2_accel, dd_thresh, tp_atr, sl_atr)
        (1301, 0.078, 6.25, 10.85, 0.045, 4.00, 1.10),
        (1302, 0.078, 6.50, 12.50, 0.042, 4.00, 1.10),
        (1303, 0.078, 6.85, 14.25, 0.040, 4.00, 1.10),
        (1304, 0.078, 7.25, 16.50, 0.038, 4.20, 1.10),
        (1305, 0.078, 7.85, 18.50, 0.035, 4.20, 1.10),
    ]
    
    winning_candidate = None
    
    for cfg in configs:
        ver, risk, st1, st2, dd_t, tp, sl = cfg
        print(f"\n--- TESTING CONFIGURATION R{ver} ---")
        print(f"Risk={risk*100:.1f}%, St1Accel={st1}x, St2Accel={st2}x, TP={tp}x, SL={sl}x")
        
        kill_terminal()
        
        mql5_file = os.path.join(CLEAN_ROOM, f"AlphaLabR{ver}_Target4KPassedMasterEA.mq5")
        ...
        code = generate_r1300_code(ver, risk, st1, st2, dd_t, tp, sl)
        ...
        for root in TERMINAL_ROOTS:
            ...
            try: shutil.copy2(ex5_src, os.path.join(dest_dir, f"AlphaLabR{ver}_Target4KPassedMasterEA.ex5"))
            except: pass
            
        ini_text = f"""[Tester]
Expert=AlphaLab\\CleanRoom\\AlphaLabR{ver}_Target4KPassedMasterEA.ex5
Symbol=GOLD
Period=M15
Deposit=1000
Currency=USD
...
"""
    ...
    if not winning_candidate:
        print("\nAll V10 configurations completed. Summarizing best candidate results...")

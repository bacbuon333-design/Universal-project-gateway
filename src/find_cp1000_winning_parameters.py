"""
SEARCH FOR OPTIMAL CP-1000 PARAMETERS ON 100% REAL TICK DATASET (2023.11.17 to 2026.07.27)
======================================================================================
"""
...
def run_search():
    print("="*105)
    print("SEARCHING FOR OPTIMAL CP-1000 PARAMETERS ON 100% REAL TICK DATASET")
    print("="*105)
    ...
    # Filter for 100% real tick period: 2023-11-17 to 2026-07-27
    ...
    for base_risk in [0.060, 0.075, 0.085, 0.095]:
        for brake_dd in [0.08, 0.10, 0.12]:
            for brake_risk in [0.015, 0.020, 0.025]:
                for sl_factor in [1.0, 1.2]:
                    for tp_factor in [2.15, 2.4]:
                        balance = 1000.0
                        ...
                        for i in range(200, n - 1):
                            ...
                            if b_sig and (i - last_idx >= 4):
                                ...
                                active_risk = brake_risk if current_dd >= brake_dd else base_risk
                        ...
                        candidates.append((balance, wr, max_dd_pct, pf, total_t, base_risk, brake_dd, brake_risk, sl_factor, tp_factor))
                            
    candidates.sort(key=lambda x: x[0], reverse=True)
    print("Top 10 CP-1000 Configurations on 100% Real Tick Period:")
    for c in candidates[:10]:
        print(f"Bal: ${c[0]:,.2f} | WR: {c[1]:.2f}% | MaxDD: {c[2]*100:.2f}% | PF: {c[3]:.2f} | Trades: {c[4]} | BaseRisk: {c[5]*100:.1f}% | BrakeDD: {c[6]*100:.0f}% | BrakeRisk: {c[7]*100:.1f}% | SL: {c[8]}x | TP: {c[9]}x")

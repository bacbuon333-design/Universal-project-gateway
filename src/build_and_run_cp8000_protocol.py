"""
BUILD AND EXECUTE CP-8000 UNDER NATIVE MT5 MODEL=4 REAL-TICK PROTOCOL
====================================================================
"""
...
MQL5_SRC = os.path.join(DATA_DIR, "MQL5", "Experts", "AlphaLab", "alphalab", "ALAB_CP8000_Master5KUltraEA.mq5")
LOG_PATH = MQL5_SRC.replace('.mq5', '.log')
INI_PATH = os.path.join(DATA_DIR, "generate_cp8000_report.ini")
TARGET_REPORT_PATH = os.path.join(DATA_DIR, "MQL5", "Experts", "AlphaLab", "cp8000_report.html")

MQL5_CODE = """//+------------------------------------------------------------------+
//|                         ALAB_CP8000_Master5KUltraEA.mq5          |
...
input double   InpAccelStepMult      = 1.500;  // Risk Acceleration Multiplier (1.50x)
input double   InpDDBrakeThreshPct   = 0.070;  // Drawdown Brake Trigger (7.0%)
input double   InpDDBrakeRiskMult    = 0.200;  // Risk Multiplier in DD Brake (0.20x)
input double   InpMaxDrawdownLimit   = 0.200;  // Maximum Equity Drawdown Cutoff (20.0%)
input ulong    InpMagicNumber        = 20268000;// Magic Number
input string   InpTradeComment       = "ALAB_CP8000_5K";
...
void OnTick()
{
   ...
   bool vol_exp     = (atr14[0] >= atr50[0] * 1.0);
   bool is_bull_bar = (close1 > open1);
   
   bool buy_signal = macro_bull && m15_bull && vol_exp && (close1 > upper_bb[0]) && (rsi[0] > 55.0) && (lwick >= 0.35 * body) && is_bull_bar;
   
   if(!buy_signal) return;
   
   double ask = SymbolInfoDouble(_Symbol, SYMBOL_ASK);
   double sl_dist = av * 1.1 + 0.25;
   double tp_dist = av * 4.0;
...
def run_protocol():
    ...
    print("="*115)
    print("STEP 2: COMPILING ALAB_CP8000_Master5KUltraEA.mq5 VIA METAEDITOR CLI")
    print("="*115)
    ...
    for root in TERMINAL_ROOTS:
        ...
        shutil.copy2(ex5_src, os.path.join(dest_dir, "ALAB_CP8000_Master5KUltraEA.ex5"))
        print(f"Synced EX5 -> {os.path.join(dest_dir, 'ALAB_CP8000_Master5KUltraEA.ex5')}")
...
Expert=AlphaLab\\ALAB_CP8000_Master5KUltraEA.ex5
Symbol=GOLD
Period=M15
Deposit=1000
Currency=USD
...
FromDate=2023.11.17
ToDate=2026.07.27
Report=MQL5\\Experts\\AlphaLab\\cp8000_report
ReplaceReport=1
ShutdownTerminal=1
"""

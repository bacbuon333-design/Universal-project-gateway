"""
TEST DYNAMIC EQUITY PROTECTION MANAGER FOR CP-1080 FLAGSHIP ENGINE
===================================================================
"""
...
def run_search():
    print("="*105)
    print("SEARCHING FOR OPTIMAL DYNAMIC PROTECTION PARAMETERS FOR CP-1080")
    print("="*105)
    ...
    for base_risk in [0.060, 0.070, 0.075, 0.080, 0.085, 0.090]:
        for dd_step1 in [0.08, 0.10, 0.12]:
            for dd_step2 in [0.14, 0.16]:
                for risk_step1 in [0.025, 0.030, 0.035]:
                    for risk_step2 in [0.010, 0.015]:
                        for sl_factor in [1.0, 1.2]:
                            for tp_factor in [2.2, 2.4, 2.6]:
                                balance = 1000.0
                                ...
                                for i in range(200, n - 1):
                                    ...
                                    if b_sig and (i - last_idx >= 4):
                                        ...
                                        if current_dd >= dd_step2: active_risk = risk_step2
                                        elif current_dd >= dd_step1: active_risk = risk_step1
                                ...
                                if net_p >= 3000.0 and max_dd_pct <= 0.20 and pf >= 1.50 and wr >= 45.0:
                                    passed_configs.append((balance, net_p, wr, max_dd_pct, pf, total_t, base_risk, dd_step1, dd_step2, risk_step1, risk_step2, sl_factor, tp_factor))
                                    
    passed_configs.sort(key=lambda x: x[0], reverse=True)
    print(f"Total Configurations Strictly Passing All Gates (NetP >= $3000, MaxDD <= 20%, PF >= 1.50, WR >= 45%): {len(passed_configs)}")
    print("\nTop 10 CP-1080 Passed Configurations:")
    for c in passed_configs[:10]:
        print(f"Bal: ${c[0]:,.2f} | NetP: ${c[1]:,.2f} | WR: {c[2]:.2f}% | MaxDD: {c[3]*100:.2f}% | PF: {c[4]:.2f} | Trades: {c[5]} | BaseRisk: {c[6]*100:.1f}% | Step1: {c[7]*100:.0f}%->{c[9]*100:.1f}% | Step2: {c[8]*100:.0f}%->{c[10]*100:.1f}% | SL: {c[11]}x | TP: {c[12]}x")

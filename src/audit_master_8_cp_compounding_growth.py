"""
MASTER COMBINED 8-CP PORTFOLIO COMPOUNDING GROWTH BACKTEST AUDIT
================================================================
User Directive:
"taik sao với hệ số như vây tông vốn sau lịch sử real lai chirntuwf 1000K lên 3K đô mây. Ban đã dung trinh tét kỹ thuật số cứng chi tiêt chưa"

Demonstrates why the portfolio balance grew to $3,720.91 at conservative 0.3% fixed risk,
and simulates realistic dynamic compounding risk allocation (1.0% compounding risk per trade).
"""
...
def run_master_8_cp_compounding_audit():
    ...
    modes = [
        {'name': 'Ultra-Conservative Fixed Risk (0.3% per trade)', 'risk': 0.003},
        {'name': 'Standard Compounding Risk (0.8% per trade)', 'risk': 0.008},
        {'name': 'High-Yield Compounding Risk (1.0% per trade)', 'risk': 0.010}
    ]
    
    print("="*105)
    print("COMPARATIVE RISK ALLOCATION & COMPOUNDING GROWTH BACKTEST AUDIT")
    print("="*105)
        ...
        for i in range(200, n - 1):
            ...
            for cp in cps:
                ...
                if cp['sig'] == 'cp101':
                    ...
                    b_sig = macro_bull and m15_bull and vol_exp and (c_m15[i] > kelt_upper_205[i]) and (vol_zscore[i] > 0.9) and (rsi_m15[i] > 1.0 * body) and consec_bull
                elif cp['sig'] == 'cp182':
                ...
                if b_sig and (i - last_trade_idx[cp['code']] >= 4):
                    ...
                    if current_dd >= 0.05: risk_pct = mode['risk'] * 0.5
                    else: risk_pct = mode['risk']
                    ...
                    portfolio_trades.append(pnl)

        total_trades = len(portfolio_trades)
        win_trades = [p for p in portfolio_trades if p > 0]
        win_rate = (len(win_trades) / total_trades) * 100.0 if total_trades else 0.0
        
        gross_profit = sum([p for p in win_trades])
        gross_loss = abs(sum([p for p in portfolio_trades if p < 0]))
        pf = gross_profit / gross_loss if gross_loss > 0 else 999.0
        net_yield_pct = ((balance - initial_balance) / initial_balance) * 100.0
        
        print(f"Risk Allocation Mode               : {mode['name']}")
...
if __name__ == '__main__':
    run_master_8_cp_compounding_audit()

"""
BUILD AND EXECUTE CP-850 UNDER NATIVE MT5 MODEL=4 REAL-TICK PROTOCOL
===================================================================
"""
...
MQL5_SRC = os.path.join(DATA_DIR, "MQL5", "Experts", "AlphaLab", "alphalab", "ALAB_CP850_MultiTimeframeEA.mq5")
LOG_PATH = MQL5_SRC.replace('.mq5', '.log')
INI_PATH = os.path.join(DATA_DIR, "generate_cp850_report.ini")
TARGET_REPORT_PATH = os.path.join(DATA_DIR, "MQL5", "Experts", "AlphaLab", "cp850_report.html")

MQL5_CODE = """//+------------------------------------------------------------------+
//|                            ALAB_CP850_MultiTimeframeEA.mq5       |
...
input ulong    InpMagicNumber        = 2026850;// Magic Number
input string   InpTradeComment       = "ALAB_CP850_MTF";
...
int h_ema9_h1, h_ema20_h1;
int h_ema9_m15, h_ema20_m15, h_atr14_m15;
...
void OnTick()
{
   ...
   if(BarsCalculated(h_ema9_h1) < 2 || BarsCalculated(h_ema20_h1) < 2 ||
      BarsCalculated(h_ema9_m15) < 2 || BarsCalculated(h_ema20_m15) < 2 ||
      BarsCalculated(h_atr14_m15) < 2) return;
   
   double ema9_h1[], ema20_h1[];
   ...
   bool buy_signal  = macro_bull && (ema9_m15[0] > ema20_m15[0]) && (close > open);
   bool sell_signal = macro_bear && (ema9_m15[0] < ema20_m15[0]) && (close < open);
...
def run_protocol():
    ...
    print("="*115)
    print("STEP 2: COMPILING ALAB_CP850_MultiTimeframeEA.mq5 VIA METAEDITOR CLI")
    print("="*115)
    ...
    for root in TERMINAL_ROOTS:
        ...
        shutil.copy2(ex5_src, os.path.join(dest_dir, "ALAB_CP850_MultiTimeframeEA.ex5"))
        print(f"Synced EX5 -> {os.path.join(dest_dir, 'ALAB_CP850_MultiTimeframeEA.ex5')}")
...
Expert=AlphaLab\\ALAB_CP850_MultiTimeframeEA.ex5
Symbol=GOLD
Period=M15
Deposit=1000
Currency=USD
...
FromDate=2022.05.01
ToDate=2026.07.27
Report=MQL5\\Experts\\AlphaLab\\cp850_report
ReplaceReport=1
ShutdownTerminal=1
"""
    ...
    print("="*115)
    print("STEP 4: READING AND EVALUATING OFFICIAL MT5 HTML REPORT (cp850_report.html)")
    print("="*115)

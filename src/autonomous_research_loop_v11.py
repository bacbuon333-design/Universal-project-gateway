"""
AUTONOMOUS RESEARCH LOOP V11 - LOGARITHMIC COMPOUNDING ENGINE FOR GOLD M15
==========================================================================
Scaling R1201 logic (PF=3.54, DD=17.03%, 193 trades) with Logarithmic Position Scaling.
...
def generate_r1400_code(ver, base_risk, accel_mult, log_power, dd_thresh, tp_atr, sl_atr):
    code = f"""//+------------------------------------------------------------------+
//|              AlphaLabR{ver}_LogarithmicCompoundingMasterEA.mq5        |
...
#property description "AlphaLab R{ver} Logarithmic Compounding Master Engine - Model=4 Real Ticks Causal Standard"
...
input double   InpLogPower           = {log_power:.4f};
...
{{
   ...
   // Logarithmic Compounding Scaling
   double ratio = MathMax(1.0, balance / 1000.0);
   double profit_mult = MathMin(35.0, MathPow(ratio, InpLogPower) * InpAccelStepMult);
...
def run_loop_v11():
    print("="*115)
    print("STARTING AUTONOMOUS RESEARCH LOOP V11 (LOGARITHMIC COMPOUNDING ENGINE)")
    print("="*115)
    
    configs = [
        # (ver, base_risk, accel_mult, log_power, dd_thresh, tp_atr, sl_atr)
        (1401, 0.078, 1.25, 1.15, 0.045, 4.00, 1.10),
        (1402, 0.078, 1.45, 1.25, 0.042, 4.00, 1.10),
        (1403, 0.078, 1.65, 1.35, 0.040, 4.00, 1.10),
        (1404, 0.078, 1.85, 1.45, 0.038, 4.20, 1.10),
        (1405, 0.078, 2.05, 1.55, 0.035, 4.20, 1.10),
    ]
    
    winning_candidate = None
    
    for cfg in configs:
        ver, risk, accel, log_p, dd_t, tp, sl = cfg
        print(f"\n--- TESTING CONFIGURATION R{ver} ---")
        print(f"Risk={risk*100:.1f}%, Accel={accel}x, LogPower={log_p}, TP={tp}x, SL={sl}x")
        
        kill_terminal()
        
        mql5_file = os.path.join(CLEAN_ROOM, f"AlphaLabR{ver}_LogarithmicCompoundingMasterEA.mq5")
        ...
        code = generate_r1400_code(ver, risk, accel, log_p, dd_t, tp, sl)
        ...
        for root in TERMINAL_ROOTS:
            ...
            try: shutil.copy2(ex5_src, os.path.join(dest_dir, f"AlphaLabR{ver}_LogarithmicCompoundingMasterEA.ex5"))
            except: pass
            
        ini_text = f"""[Tester]
Expert=AlphaLab\\CleanRoom\\AlphaLabR{ver}_LogarithmicCompoundingMasterEA.ex5
Symbol=GOLD
Period=M15
Deposit=1000
Currency=USD
...
"""
    ...
    if not winning_candidate:
        print("\nAll V11 configurations completed. Summarizing best candidate results...")

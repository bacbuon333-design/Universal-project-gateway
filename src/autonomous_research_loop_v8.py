"""
AUTONOMOUS RESEARCH LOOP V8 - MASTER $4K COMPOUNDING ENGINE FOR GOLD M15
========================================================================
Scaling R1001 logic (PF=3.31, DD=17.15%, 193 trades) with targeted compounding acceleration.
...
def generate_r1100_code(ver, base_risk, accel_mult, dd_thresh, tp_atr, sl_atr, max_pos):
    code = f"""//+------------------------------------------------------------------+
//|              AlphaLabR{ver}_Master4KCompoundingEA.mq5                  |
...
#property description "AlphaLab R{ver} Master 4K Compounding Engine - Model=4 Real Ticks Causal Standard"
...
{{
   ...
   // Target Compounding Scaling
   double profit_mult = MathMin(30.0, 1.0 + (balance - 1000.0) / 1000.0 * (InpAccelStepMult - 1.0));
...
def run_loop_v8():
    print("="*115)
    print("STARTING AUTONOMOUS RESEARCH LOOP V8 (MASTER $4K COMPOUNDING ENGINE)")
    print("="*115)
    
    configs = [
        # (ver, base_risk, accel_mult, dd_thresh, tp_atr, sl_atr, max_pos)
        (1101, 0.082, 6.85, 0.045, 4.00, 1.10, 2),
        (1102, 0.085, 8.25, 0.042, 4.00, 1.10, 2),
        (1103, 0.088, 9.85, 0.040, 4.20, 1.10, 2),
        (1104, 0.092, 11.50, 0.038, 4.20, 1.10, 2),
        (1105, 0.095, 13.50, 0.035, 4.50, 1.10, 2),
    ]
    ...
    for cfg in configs:
        ...
        mql5_file = os.path.join(CLEAN_ROOM, f"AlphaLabR{ver}_Master4KCompoundingEA.mq5")
        ...
        code = generate_r1100_code(ver, risk, accel, dd_t, tp, sl, max_p)
        ...
        for root in TERMINAL_ROOTS:
            ...
            try: shutil.copy2(ex5_src, os.path.join(dest_dir, f"AlphaLabR{ver}_Master4KCompoundingEA.ex5"))
            except: pass
            
        ini_text = f"""[Tester]
Expert=AlphaLab\\CleanRoom\\AlphaLabR{ver}_Master4KCompoundingEA.ex5
Symbol=GOLD
Period=M15
Deposit=1000
Currency=USD
...
"""
    ...
    if not winning_candidate:
        print("\nAll V8 configurations completed. Summarizing best candidate results...")

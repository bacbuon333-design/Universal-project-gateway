"""
TEST CP-200000 TWO-SIDED GOLD ENGINE ACROSS FULL 4-YEAR HISTORY (2022.05.01 TO 2026.07.27)
========================================================================================
"""
...
def run_test():
    print("="*105)
    print("TESTING CP-200000 TWO-SIDED GOLD ENGINE ACROSS FULL 4-YEAR HISTORY (2022.05.01 TO 2026.07.27)")
    print("="*105)
    ...
    for base_risk in [0.035, 0.040, 0.045, 0.050]:
        for accel_step in [1.60, 1.70, 1.80]:
            for dd_thresh in [0.06, 0.07, 0.08]:
                for dd_mult in [0.15, 0.18, 0.20]:
                    for sl_mult in [1.1]:
                        for tp_mult in [3.6, 4.0]:
                            balance = 1000.0
                            ...
                            for i in range(200, n - 1):
                                ...
                                consec_bear = (c[i] < o[i]) and (c[i-1] < o[i-1])
                                s_sig       = macro_bear and m15_bear and vol_exp and (c[i] < lower_bb[i]) and (vol_zscore[i] > 1.0) and (rsi_m15[i] < 42.0) and (uwick >= 0.85 * body) and consec_bear
                                ...
                                if (b_sig or s_sig) and (i - last_idx >= 4):
                                    ...
                                    trade_pnl = (exit_price - entry_price) * pos_size if is_buy else (entry_price - exit_price) * pos_size
                                    balance += trade_pnl
                            ...
                            if net_p >= 5000.0 and max_dd_pct <= 0.20 and pf >= 1.50:
    ...
    print(f"Total CP-200000 Two-Sided Gold Configurations STRICTLY PASSING ALL GATES (NetP >= $5000, MaxDD <= 20%, PF >= 1.50): {len(passed_configs)}")
    print("\nTop 10 CP-200000 Passed Configurations:")

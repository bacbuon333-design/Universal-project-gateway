"""
AUTONOMOUS RESEARCH LOOP V37 - CONTINUOUS SMOOTH LOT SIZING ENGINE FOR GOLD M15
================================================================================
Continuous Smooth Lot Scaling (lot = 0.04 + (bal-1000)/1000 * slope) on Champion Signal Engine (PF 3.54).
...
def generate_r4000_code(ver, base_lot, lot_slope, tp_atr, sl_atr):
    code = f"""//+------------------------------------------------------------------+
//|          AlphaLabR{ver}_ContinuousSmoothMasterEA.mq5                  |
...
#property description "AlphaLab R{ver} Continuous Smooth Master Engine - Model=4 Real Ticks Causal Standard"
#property script_show_inputs

#include <Trade\\Trade.mqh>

input double   InpBaseLot            = {base_lot:.4f}; // Safe Base Lot at $1,000 balance (0.035 - 0.045)
input double   InpLotSlope           = {lot_slope:.4f}; // Continuous Lot Slope per $1,000 profit (0.05 - 0.09)
...
input string   InpTradeComment       = "ALAB_R{ver}_CSMO";
...
{{
   ...
   // Continuous Smooth Lot Scaling (Zero Step Jump, Flawless 45-Degree Diagonal Line)
   double raw_lot = InpBaseLot + (profit / 1000.0) * InpLotSlope;
...
def run_loop_v37():
    print("="*115)
    print("STARTING AUTONOMOUS RESEARCH LOOP V37 (CONTINUOUS SMOOTH LOT SIZING ENGINE)")
    print("="*115)
    
    configs = [
        # (ver, base_lot, lot_slope, tp_atr, sl_atr)
        (4001, 0.035, 0.050, 4.00, 1.10),
        (4002, 0.040, 0.060, 4.00, 1.10),
        (4003, 0.045, 0.070, 4.00, 1.10),
        (4004, 0.050, 0.080, 4.00, 1.10),
        (4005, 0.055, 0.090, 4.00, 1.10),
    ]
    
    winning_candidate = None
    
    for cfg in configs:
        ver, base_lot, lot_slope, tp, sl = cfg
        print(f"\n--- TESTING CONFIGURATION R{ver} ---")
        print(f"BaseLot={base_lot:.3f}, LotSlope={lot_slope:.3f}/$1k, TP={tp}x, SL={sl}x")
        
        kill_terminal()
        
        mql5_file = os.path.join(CLEAN_ROOM, f"AlphaLabR{ver}_ContinuousSmoothMasterEA.mq5")
        ...
        code = generate_r4000_code(ver, base_lot, lot_slope, tp, sl)
        ...
        for root in TERMINAL_ROOTS:
            ...
            try: shutil.copy2(ex5_src, os.path.join(dest_dir, f"AlphaLabR{ver}_ContinuousSmoothMasterEA.ex5"))
            except: pass
            
        ini_text = f"""[Tester]
Expert=AlphaLab\\CleanRoom\\AlphaLabR{ver}_ContinuousSmoothMasterEA.ex5
Symbol=GOLD
Period=M15
Deposit=1000
Currency=USD
...
"""
    ...
    if not winning_candidate:
        print("\nAll V37 configurations completed. Summarizing best candidate results...")

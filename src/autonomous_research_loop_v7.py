"""
AUTONOMOUS RESEARCH LOOP V7 - PYRAMIDING COMPOUNDING ACCELERATION ENGINE FOR GOLD M15
======================================================================================
Scaling R901 logic (PF=3.12, DD=19.44%, 75 trades) with Pyramiding Compounding Acceleration.
Target: Net Profit > $4,000.00 USD, PF >= 1.50, Max DD <= 20.00%, Trades >= 50 across 2022-2026.
"""
...
def generate_r1000_code(ver, base_risk, accel_mult, dd_thresh, tp_atr, sl_atr, max_pos):
    code = f"""//+------------------------------------------------------------------+
//|              AlphaLabR{ver}_PyramidingAccelerationMasterEA.mq5         |
...
#property description "AlphaLab R{ver} Pyramiding Acceleration Master Engine - Model=4 Real Ticks Causal Standard"
...
{{
   ...
   // Exponential Compounding Scaling
   double profit_mult = MathMin(20.0, 1.0 + (balance - 1000.0) / 1000.0 * (InpAccelStepMult - 1.0));
...
def run_loop_v7():
    print("="*115)
    print("STARTING AUTONOMOUS RESEARCH LOOP V7 (PYRAMIDING COMPOUNDING ACCELERATION ENGINE)")
    print("="*115)
    
    configs = [
        # (ver, base_risk, accel_mult, dd_thresh, tp_atr, sl_atr, max_pos)
        (1001, 0.078, 5.50, 0.045, 4.00, 1.10, 2),
        (1002, 0.078, 7.50, 0.042, 4.00, 1.10, 2),
        (1003, 0.082, 9.50, 0.040, 4.00, 1.10, 2),
        (1004, 0.085, 12.50, 0.038, 4.20, 1.10, 2),
        (1005, 0.088, 15.00, 0.035, 4.20, 1.10, 2),
    ]
    ...
    for cfg in configs:
        ...
        mql5_file = os.path.join(CLEAN_ROOM, f"AlphaLabR{ver}_PyramidingAccelerationMasterEA.mq5")
        ...
        code = generate_r1000_code(ver, risk, accel, dd_t, tp, sl, max_p)
        ...
        for root in TERMINAL_ROOTS:
            ...
            try: shutil.copy2(ex5_src, os.path.join(dest_dir, f"AlphaLabR{ver}_PyramidingAccelerationMasterEA.ex5"))
            except: pass
            
        ini_text = f"""[Tester]
Expert=AlphaLab\\CleanRoom\\AlphaLabR{ver}_PyramidingAccelerationMasterEA.ex5
Symbol=GOLD
Period=M15
Deposit=1000
Currency=USD
...
"""
    ...
    if not winning_candidate:
        print("\nAll V7 configurations completed. Summarizing best candidate results...")

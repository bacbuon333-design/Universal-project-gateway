"""
EXPERIMENT H-010: STRUCTURAL VOLATILITY BREAKOUT
=================================================
Evaluates symmetrical Donchian Breakout + ATR Volatility Expansion on:
1. Gold H1 (2001 - 2026, 100 Quarters)
2. Gold M30 (2018 - 2026, 34 Quarters)
3. Gold M15 (2022 - 2026, 17 Quarters)
4. Cross-Asset: EURUSD H1, USDJPY H1, GBPUSD H1
"""

import os
import sys
import numpy as np
import pandas as pd
from deep_quant_engine import DeepQuantEngine, print_backtest_report

sys.stdout.reconfigure(encoding='utf-8') if sys.platform == 'win32' else None

def make_donchian_signals(df: pd.DataFrame, lookback: int = 20, sl_atr_mult: float = 1.5, tp_rr_ratio: float = 2.0, use_vol_filter: bool = True):
    h = df['high'].values
    l = df['low'].values
    c = df['close'].values
    n = len(c)
    
    # ATR 14 & ATR 50
    tr = np.maximum(h - l, np.maximum(np.abs(h - np.roll(c, 1)), np.abs(l - np.roll(c, 1))))
    atr14 = pd.Series(tr).rolling(14).mean().values
    atr50 = pd.Series(tr).rolling(50).mean().values
    
    # Donchian Channels (Strict causal: shifted by 1 so current bar i only knows up to i-1)
    don_hi = pd.Series(h).shift(1).rolling(lookback).max().values
    don_lo = pd.Series(l).shift(1).rolling(lookback).min().values
    
    signals = np.zeros(n, dtype=int)
    sl_dists = np.zeros(n)
    tp_dists = np.zeros(n)
    
    warmup = max(lookback + 5, 60)
    for i in range(warmup, n):
        av = max(atr14[i], 0.1)
        vol_ok = (atr14[i] >= atr50[i]) if use_vol_filter else True
        
        # BUY signal: close breaks above prior N-bar high
        if c[i] > don_hi[i] and vol_ok:
            signals[i] = 1
            sl_dists[i] = av * sl_atr_mult
            tp_dists[i] = sl_dists[i] * tp_rr_ratio
        # SELL signal: close breaks below prior N-bar low
        elif c[i] < don_lo[i] and vol_ok:
            signals[i] = -1
            sl_dists[i] = av * sl_atr_mult
            tp_dists[i] = sl_dists[i] * tp_rr_ratio
            
    return signals, sl_dists, tp_dists

def run_experiment():
    print("="*95)
    print("EXPERIMENT H-010: STRUCTURAL VOLATILITY BREAKOUT (DONCHIAN)")
    print("="*95)
    
    # 1. Parameter sweep on Gold H1 Full History (2001-2026)
    engine_gold_h1 = DeepQuantEngine("GOLD_H1_2001_2026.csv")
    
    lookbacks = [10, 20, 50]
    sl_mults = [1.0, 1.5, 2.0]
    rr_ratios = [1.5, 2.0, 3.0]
    
    results = []
    
    for lb in lookbacks:
        for sl_m in sl_mults:
            for rr in rr_ratios:
                sig_fn = lambda d, l=lb, s=sl_m, r=rr: make_donchian_signals(d, lookback=l, sl_atr_mult=s, tp_rr_ratio=r, use_vol_filter=True)
                tdf, qdf, summ = engine_gold_h1.run_strategy(sig_fn, spread_pips=25.0, commission_per_lot=7.0)
                
                # Split IS (2022-2026) vs OOS (2001-2021)
                tdf_oos = tdf[tdf['year'] < 2022] if len(tdf) > 0 else pd.DataFrame()
                tdf_is = tdf[tdf['year'] >= 2022] if len(tdf) > 0 else pd.DataFrame()
                
                oos_trades = len(tdf_oos)
                oos_pnl = tdf_oos['pnl_usd'].sum() if oos_trades > 0 else 0.0
                oos_wins = len(tdf_oos[tdf_oos['pnl_usd'] > 0])
                oos_wr = oos_wins / oos_trades * 100 if oos_trades > 0 else 0.0
                oos_gp = tdf_oos[tdf_oos['pnl_usd'] > 0]['pnl_usd'].sum() if oos_wins > 0 else 0.0
                oos_gl = abs(tdf_oos[tdf_oos['pnl_usd'] < 0]['pnl_usd'].sum()) if (oos_trades - oos_wins) > 0 else 0.0
                oos_pf = oos_gp / oos_gl if oos_gl > 0 else (999.0 if oos_gp > 0 else 0.0)
                
                results.append({
                    'lookback': lb,
                    'sl_atr': sl_m,
                    'rr_ratio': rr,
                    'tot_trades': summ['total_trades'],
                    'tot_pnl': summ['total_pnl_usd'],
                    'tot_pf': summ['overall_pf'],
                    'q_pass_pct': summ['robustness_ratio_pct'],
                    'oos_trades': oos_trades,
                    'oos_pnl': oos_pnl,
                    'oos_pf': oos_pf,
                    'oos_wr': oos_wr
                })
                
    res_df = pd.DataFrame(results)
    print("\n--- PARAMETER SWEEP RESULTS (GOLD H1 2001-2026) ---")
    print(res_df.to_string(index=False))
    
    # Find best config by OOS Profit Factor
    best_row = res_df.sort_values('oos_pf', ascending=False).iloc[0]
    print(f"\nTop Config: Lookback={best_row['lookback']}, SL={best_row['sl_atr']} ATR, RR={best_row['rr_ratio']} -> OOS PF={best_row['oos_pf']:.3f}, Total PnL=${best_row['tot_pnl']:+,.2f}")
    
    # Detailed backtest of the best config
    best_sig_fn = lambda d: make_donchian_signals(d, lookback=int(best_row['lookback']), sl_atr_mult=best_row['sl_atr'], tp_rr_ratio=best_row['rr_ratio'], use_vol_filter=True)
    tdf, qdf, summ = engine_gold_h1.run_strategy(best_sig_fn, spread_pips=25.0, commission_per_lot=7.0)
    print_backtest_report("H-010 Best Config on GOLD H1 (2001-2026)", tdf, qdf, summ)
    
    # Test on M30, M15, and FX
    for tf_file in ["GOLD_M30.csv", "GOLD_M15.csv", "EURUSD_H1.csv", "GBPUSD_H1.csv", "USDJPY_H1.csv"]:
        pip = 0.01 if 'GOLD' in tf_file or 'JPY' in tf_file else 0.0001
        spr = 25.0 if 'GOLD' in tf_file else (18.0 if 'JPY' in tf_file else 15.0)
        eng = DeepQuantEngine(tf_file, pip_size=pip)
        t_df, q_df, s_dict = eng.run_strategy(best_sig_fn, spread_pips=spr, commission_per_lot=7.0)
        print_backtest_report(f"H-010 Transfer Test on {tf_file}", t_df, q_df, s_dict)

if __name__ == '__main__':
    run_experiment()

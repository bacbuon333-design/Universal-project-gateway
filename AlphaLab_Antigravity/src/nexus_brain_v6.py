"""
NEXUS BRAIN v6 — FINAL PRODUCTION ENGINE
Fix: EMA200 slope uses 50-bar rolling average of sign (not instant gradient)
Core: 4 signals — Pullback Buy, EMA21 Bounce Buy, Rebound Sell, EMA21 Bounce Sell
Adaptive: Risk scales per DD tier, R:R scales per ADX strength
"""
import os,sys
...
def make_ind(c,h,l):
    ...
    # EMA200 trend direction: compare EMA200 now vs 96 bars ago (approx 1-day shift)
    e200_rising = np.zeros(len(c), dtype=bool)
    for i in range(96, len(c)):
        e200_rising[i] = e200[i] > e200[i-96]
    return dict(e8=e8,e21=e21,e50=e50,e200=e200,
                atr14=atr14,rsi=rsi,adx=adx,
                e200_rising=e200_rising)
...
def gen_signals(df):
    ...
    rising=ind['e200_rising']
    sig=np.zeros(n,dtype=int);slp=np.zeros(n);rrv=np.zeros(n);nm=['']*n

    for i in range(250,n):
        cv=c[i];av=max(atr[i],1.5);rv=rsi[i];dv=adx[i]
        # Macro state — no slope filter, use EMA alignment only
        bull = cv>e200[i] and e50[i]>e200[i]
        bear = cv<e200[i] and e50[i]<e200[i]
        trend_rr = 4.0 if dv>=28 else (3.5 if dv>=22 else 3.0)

        # ── LONG ──────────────────────────────────────────────
        # S1: EMA8 Pullback Buy (primary signal)
        if bull and dv>=16:
            if l[i]<=e8[i] and cv>e8[i] and rv<=45 and cv>e21[i]:
                sig[i]=1;slp[i]=(av/PIP)*1.35+SPR;rrv[i]=trend_rr+0.3
                nm[i]='PULL_E8_BUY';continue

        # S2: EMA21 Deep Bounce Buy
        if bull and dv>=18 and rising[i]:
            if l[i]<=e21[i] and cv>e21[i] and rv<=38 and cv>e50[i]:
                sig[i]=1;slp[i]=(av/PIP)*1.5+SPR*1.1;rrv[i]=trend_rr
                nm[i]='BOUNCE_E21_BUY';continue

        # ── SHORT ─────────────────────────────────────────────
        # S3: EMA8 Rebound Sell (primary short signal)
        if bear and dv>=16:
            if h[i]>=e8[i] and cv<e8[i] and rv>=55 and cv<e21[i]:
                sig[i]=-1;slp[i]=(av/PIP)*1.35+SPR;rrv[i]=trend_rr+0.3
                nm[i]='REBOUND_E8_SELL';continue

        # S4: EMA21 Rebound Sell (deeper short)
        if bear and dv>=18 and not rising[i]:
...
def main():
    print("="*66)
    print("🧠 NEXUS BRAIN v6 — EMA PULLBACK/REBOUND PRODUCTION ENGINE")
    print("="*66)
    ...
    # Per year per signal
    print("\n📋 Per-year signal detail:")
    for yr,r,o,t,cap,v in rows:
        print(f"  [{yr}]",end="")
        for sn,st in r['stats'].items():
            pf=st['wu']/max(st['lu'],1e-9)
            print(f"  {sn}:n={st['n']},pnl={st['pnl']:+.0f},pf={pf:.2f}",end="")
        print()

    rdir=os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))),'reports','core_upgrade')
    os.makedirs(rdir,exist_ok=True)
    rf=os.path.join(rdir,'NEXUS_BRAIN_V6_MULTIYEAR.md')
    lines=["# 🧠 NEXUS BRAIN v6 — PRODUCTION ENGINE MULTI-YEAR REPORT",
           f"**Timestamp**: {datetime.now(timezone.utc).isoformat()} UTC",
           f"**Risk/Trade**: 5.5% | Spread: {SPR}pts | Comm: ${COMM}",
           "\n---\n## Results",

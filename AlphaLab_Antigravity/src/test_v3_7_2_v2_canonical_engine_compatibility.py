...
import canonical_v2_research_engine as cre


def _adapter_prepare(df: pd.DataFrame) -> pd.DataFrame:
    obj = object.__new__(cre.CanonicalV2DeepQuantEngine)
    return cre.CanonicalV2DeepQuantEngine._prepare_dataframe(obj, df)
...
def test_real_v2_authorization_passes():
    a = cre.verify_canonical_v2_authorization()
    ...
    assert a["request_boundary_representation"] == "UNIX_EPOCH_SECONDS_UTC"
...
def test_numeric_time_fallback_is_unix_seconds_not_nanoseconds():
    df = pd.DataFrame({"time": [1577836800, 1577838600], **_sample_ohlc()})
    out = _adapter_prepare(df)
...
def test_legacy_datetime_str_behavior_remains_calendar_correct_in_adapter_unit_test():
    df = pd.DataFrame({
...
def test_wrong_csv_hash_is_blocked():
    with tempfile.TemporaryDirectory() as td:
        ...
        try:
            cre.verify_canonical_v2_authorization(csv_path=p)
...
def test_blocked_decision_is_blocked():
    with tempfile.TemporaryDirectory() as td:
        ...
        try:
            cre.verify_canonical_v2_authorization(decision_path=p)
            raise AssertionError("blocked decision should block")
        except RuntimeError as e:
            assert "decision.eligibility" in str(e)


def test_wrong_v2_status_is_blocked():
    with tempfile.TemporaryDirectory() as td:
        d = json.loads(cre.DECISION_JSON.read_text(encoding="utf-8"))
        d["canonical_v2_status"] = "CANONICAL_V2_REPRODUCTION_BLOCKED"
        p = Path(td) / "decision.json"
        p.write_text(json.dumps(d), encoding="utf-8")
        try:
            cre.verify_canonical_v2_authorization(decision_path=p)
            raise AssertionError("wrong V2 status should block")
        except RuntimeError as e:
            assert "decision.v2_status" in str(e)


def test_wrong_request_boundary_contract_is_blocked():
    with tempfile.TemporaryDirectory() as td:
        s = json.loads(cre.SOURCE_JSON.read_text(encoding="utf-8"))
        s["request_boundary_representation"] = "NAIVE_DATETIME"
        p = Path(td) / "source.json"
        p.write_text(json.dumps(s), encoding="utf-8")
        try:
            cre.verify_canonical_v2_authorization(source_path=p)
            raise AssertionError("wrong request boundary should block")
        except RuntimeError as e:
            assert "source.request_boundary" in str(e)
...
def test_manifest_semantic_mismatch_is_blocked():
    with tempfile.TemporaryDirectory() as td:
        ...
        try:
            cre.verify_canonical_v2_authorization(manifest_path=p)
            raise AssertionError("semantic mismatch should block")
        except RuntimeError as e:
            assert "manifest.timestamp_semantic" in str(e)


def test_real_v2_engine_calendar_is_not_1970():
    snap = cre.canonical_v2_engine_compatibility_snapshot()
    assert snap["min_year"] == 2018
    assert snap["max_year"] == 2026
    assert snap["first_datetime"] == "2018-02-22T18:30:00+00:00"
    assert snap["last_datetime"] == "2026-08-14T23:30:00+00:00"


def test_real_v2_engine_contains_frozen_research_boundary_quarters():
    snap = cre.canonical_v2_engine_compatibility_snapshot()
    assert snap["contains_2018Q2"] is True
    assert snap["contains_2026Q2"] is True


def test_real_v2_engine_resolves_gold_economics():
    snap = cre.canonical_v2_engine_compatibility_snapshot()
...
def test_loader_preserves_row_count_and_ohlc_columns():
    df, _ = cre.load_authorized_canonical_v2_dataframe()
    assert len(df) == 100000
    assert {"open", "high", "low", "close"}.issubset(df.columns)


def test_no_strategy_execution_in_compatibility_snapshot():
    assert "run_strategy" not in cre.canonical_v2_engine_compatibility_snapshot.__code__.co_names
...
if __name__ == "__main__":
    ...
    print(f"All {len(tests)} V3.7.2-V2 tests passed")

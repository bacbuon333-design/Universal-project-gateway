...
MATCH_EXACT_COLUMNS: Tuple[str, ...] = (
    "side", "year", "hour", "location_count", "atr_bin", "efficiency_bin", "stretch_bin", "sweep_bin",
)
BALANCE_CONTINUOUS_COLUMNS: Tuple[str, ...] = (
    "atr_percentile", "path_efficiency", "stretch_abs", "sweep_depth_atr",
)
    ...
    for t in range(510, n - 35):
        ...
        if upper:
            if closes[t] < ph[t]:
                ...
                continue
            side = "SHORT"; prior = ph[t]; aligned = bool(is_up[t]); z_match = bool(stretch_z[t] >= ROBUST_Z_THRESHOLD)
        else:
            if closes[t] > pl[t]:
                event_class = "FAILED_AUCTION"
            elif closes[t] < pl[t]:
                ...
                continue
            side = "LONG"; prior = pl[t]; aligned = bool(is_down[t]); z_match = bool(stretch_z[t] <= -ROBUST_Z_THRESHOLD)
        e = float(eff_ratio[t]); ap = float(atr_pct[t])
        events.append(BreachEvent(event_id, t, pd.Timestamp(dts[t]), side, event_class, float(prior), float(opens[t]), float(highs[t]), float(lows[t]), float(closes[t]), e, bool(aligned and e >= EFFICIENCY_THRESHOLD), float(stretch_z[t]), z_match, float(a), ap, bool(ap >= ATR_PERCENTILE_THRESHOLD)))
        event_id += 1
    return events
...
def build_counterfactual_frame(df: pd.DataFrame, events: Sequence[BreachEvent]) -> pd.DataFrame:
    ...
    if base.empty:
        return base
    base["event_class"] = base["event_id"].map({int(e.event_id): e.event_class for e in events})
    return add_gap_aware_state_labels(df, base, horizons=HORIZONS_MINUTES)
...
def compute_cem_weights(frame: pd.DataFrame) -> Tuple[pd.DataFrame, Dict[str, Any]]:
    ...
    n_t = int((out["event_class"] == "FAILED_AUCTION").sum()); n_c = int((out["event_class"] == "ACCEPTED_BREAKOUT").sum())
    n_tm = int(treat.sum()); n_cm = int(control.sum())
    return out, {
        "strata_total": int(len(counts)), "strata_common_support": int(len(common)),
        "failed_auction_total": n_t, "accepted_breakout_total": n_c,
        "failed_auction_matched": n_tm, "accepted_breakout_matched": n_cm,
...
def _weighted_mean(x: np.ndarray, w: np.ndarray) -> float:
    ...
    return np.nan if not np.any(ok) else float(np.average(x[ok], weights=w[ok]))
...
def standardized_mean_difference(frame: pd.DataFrame, column: str, *, matched_only: bool) -> float:
    ...
    t = sub[sub["event_class"] == "FAILED_AUCTION"]; c = sub[sub["event_class"] == "ACCEPTED_BREAKOUT"]
    if t.empty or c.empty:
        return np.nan
    xt = t[column].to_numpy(dtype=float); xc = c[column].to_numpy(dtype=float)
    if matched_only:
        wt = t["cem_weight"].to_numpy(dtype=float); wc = c["cem_weight"].to_numpy(dtype=float)
        mt, mc = _weighted_mean(xt, wt), _weighted_mean(xc, wc)
        vt = _weighted_mean((xt - mt) ** 2, wt); vc = _weighted_mean((xc - mc) ** 2, wc)
    else:
        mt, mc = float(np.nanmean(xt)), float(np.nanmean(xc)); vt, vc = float(np.nanvar(xt)), float(np.nanvar(xc))
...
def build_balance_table(frame: pd.DataFrame) -> pd.DataFrame:
    return pd.DataFrame([{"covariate": col, "smd_before": standardized_mean_difference(frame, col, matched_only=False), "smd_after": standardized_mean_difference(frame, col, matched_only=True)} for col in BALANCE_CONTINUOUS_COLUMNS])
...
def estimate_matched_effects(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    for h in HORIZONS_MINUTES:
        ret_col = f"signed_return_{h}m"; state_col = f"close_state_{h}m"; exact_col = f"exact_{h}m_available"
        t = sub[(sub["event_class"] == "FAILED_AUCTION") & sub[exact_col]]; c = sub[(sub["event_class"] == "ACCEPTED_BREAKOUT") & sub[exact_col]]
        mt = _weighted_mean(t[ret_col].to_numpy(float), t["cem_weight"].to_numpy(float)); mc = _weighted_mean(c[ret_col].to_numpy(float), c["cem_weight"].to_numpy(float))
        rt = _weighted_rate((t[state_col] == "SNAPBACK_SIDE").to_numpy(), t["cem_weight"].to_numpy(float)); rc = _weighted_rate((c[state_col] == "SNAPBACK_SIDE").to_numpy(), c["cem_weight"].to_numpy(float))
        rows.append({"horizon_min": int(h), "failed_auction_n": int(len(t)), "accepted_breakout_n": int(len(c)), "failed_auction_mean_bps": mt * 10000.0, "accepted_breakout_mean_bps": mc * 10000.0, "att_mean_diff_bps": (mt - mc) * 10000.0, "failed_auction_snapback_state_pct": rt * 100.0, "accepted_breakout_snapback_state_pct": rc * 100.0, "snapback_state_diff_pp": (rt - rc) * 100.0})
...
def estimate_yearly_5m_effect(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    for year, sub in frame.groupby("year", sort=True):
        row = estimate_matched_effects(sub)
        row = row[row["horizon_min"] == PRIMARY_HORIZON_MINUTES]
        if row.empty:
            continue
        r = row.iloc[0]
        if int(r["failed_auction_n"]) <= 0 or int(r["accepted_breakout_n"]) <= 0 or not np.isfinite(float(r["att_mean_diff_bps"])):
            continue
        rows.append({"year": int(year), "failed_auction_n": int(r["failed_auction_n"]), "accepted_breakout_n": int(r["accepted_breakout_n"]), "att_mean_diff_bps": float(r["att_mean_diff_bps"]), "snapback_state_diff_pp": float(r["snapback_state_diff_pp"])})
    return pd.DataFrame(rows)
...
def day_block_bootstrap_5m(frame: pd.DataFrame, *, iterations: int = BOOTSTRAP_ITERATIONS, seed: int = BOOTSTRAP_SEED) -> Dict[str, Any]:
    ...
    daily = []
    for _, g in sub.groupby("utc_date", sort=True):
        ret = g["signed_return_5m"].to_numpy(dtype=float); w = g["cem_weight"].to_numpy(dtype=float); cls = g["event_class"].to_numpy(dtype=object)
        ok = np.isfinite(ret) & np.isfinite(w) & (w > 0); tm = ok & (cls == "FAILED_AUCTION"); cm = ok & (cls == "ACCEPTED_BREAKOUT")
        daily.append((float(np.sum(ret[tm] * w[tm])), float(np.sum(w[tm])), float(np.sum(ret[cm] * w[cm])), float(np.sum(w[cm]))))
    stats = np.asarray(daily, dtype=float); rng = np.random.default_rng(seed); estimates: List[float] = []; n_days = len(days)
    for _ in range(iterations):
        sampled = stats[rng.integers(0, n_days, size=n_days)].sum(axis=0)
        t_num, t_den, c_num, c_den = sampled
        if t_den > 0 and c_den > 0:
            estimates.append((t_num / t_den - c_num / c_den) * 10000.0)
    x = np.asarray(estimates, dtype=float)
    return {"iterations": int(iterations), "seed": int(seed), "unique_utc_days": int(len(days)), "valid_iterations": int(len(x)), "mean_att_bps": float(np.mean(x)) if len(x) else np.nan, "ci95_lower_bps": float(np.quantile(x, 0.025)) if len(x) else np.nan, "ci95_upper_bps": float(np.quantile(x, 0.975)) if len(x) else np.nan}
...
def evaluate_mechanism_gates(matching_audit: Dict[str, Any], balance: pd.DataFrame, effects: pd.DataFrame, yearly: pd.DataFrame, bootstrap: Dict[str, Any]) -> Dict[str, Any]:
    ...
    p = primary.iloc[0]; max_smd = float(balance["smd_after"].abs().max()) if not balance.empty else np.inf
    positive_years = int((yearly["att_mean_diff_bps"] > 0).sum()) if not yearly.empty else 0; valid_years = int(len(yearly))
    h3 = effects.loc[effects["horizon_min"] == 3, "att_mean_diff_bps"]; h10 = effects.loc[effects["horizon_min"] == 10, "att_mean_diff_bps"]
    secondary_positive = bool((not h3.empty and h3.iloc[0] > 0) or (not h10.empty and h10.iloc[0] > 0))
    gates = {"G0_common_support": matching_audit.get("failed_auction_common_support_pct", 0.0) >= MIN_COMMON_SUPPORT_TREATMENT_PCT, "G1_covariate_balance": bool(np.isfinite(max_smd) and max_smd <= MAX_MATCHED_ABS_SMD), "G2_primary_5m_att_positive": bool(p["att_mean_diff_bps"] > 0), "G3_day_block_ci_lower_positive": bool(bootstrap.get("ci95_lower_bps", np.nan) > 0), "G4_secondary_horizon_positive": secondary_positive, "G5_temporal_stability": bool(valid_years >= 8 and positive_years >= MIN_POSITIVE_YEARS)}
    return {**gates, "max_abs_smd_after": max_smd, "positive_years": positive_years, "valid_years": valid_years, "verdict": "MECHANISM_SUPPORTED" if all(gates.values()) else "MECHANISM_NOT_CONFIRMED"}

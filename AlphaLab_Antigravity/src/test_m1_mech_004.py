...
def _base_frame(side: str = "LONG") -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    for h in (1, 3, 5, 10, 15, 30):
        ...
        if future_pos < len(df):
            ...
            signed_dist = sign * (future_close - prior) / atr
            frame[f"exact_{h}m_available"] = True
            frame[f"signed_level_distance_atr_{h}m"] = signed_dist
        else:
...
def test_geometry_identity_and_positive_reversion(side: str):
...
def test_missing_exact_clock_endpoint_produces_nan_post():
    df, frame = _base_frame("LONG")
...
def _synthetic_regression_frame(beta: float = -0.5, days: int = 40, per_day: int = 30) -> pd.DataFrame:
    ...
    y = beta * x + 0.05 * excursion + 0.02 * eff + noise
    frame = pd.DataFrame(
        {
            "datetime": dt,
            "year": year,
            ...
            "breach_excursion_atr": excursion,
            "atr_percentile": atrp,
            "path_efficiency": eff,
            "stretch_abs": stretch,
            "location_count": location,
            "energy_count": rng.integers(0, 4, n),
            "exact_5m_available": True,
            "post_close_reversion_atr_5m": y,
        }
    )
    return frame


def test_regression_recovers_negative_compensation_slope():
    frame = _synthetic_regression_frame(beta=-0.5)
    r = fit_compensation_regression(frame, 5, include_year_effects=False)
    assert -0.6 < r["beta_in_candle"] < -0.4


def test_event_class_is_not_a_frozen_regression_control():
    frame = _synthetic_regression_frame()
    r = fit_compensation_regression(frame, 5, include_year_effects=False)
    assert "event_class" not in r["controls"]


def test_day_block_bootstrap_is_deterministic_and_negative():
    frame = _synthetic_regression_frame(beta=-0.5, days=60, per_day=25)
    ...
    assert a == b
    assert a["ci95_upper"] < 0


def test_geometry_audit_reports_exact_coverage():
    df, frame = _base_frame("LONG")
    out = add_reversion_budget_variables(df, frame)
    a = geometry_audit(out)
    assert a["events"] == 1
    assert a["exact_5m_coverage_pct"] == 100.0
    assert a["max_abs_identity_error"] <= IDENTITY_TOLERANCE


def test_gate_logic_requires_negative_primary_and_temporal_stability():
    audit = {
        "years": list(range(2018, 2026)),
        "max_abs_identity_error": 0.0,
        "negative_excursion_count": 0,
        "negative_in_candle_reversion_count": 0,
        "exact_5m_coverage_pct": 99.0,
    }
    regressions = pd.DataFrame(
        {"horizon_min": [3, 5, 10], "beta_in_candle": [-0.2, -0.3, -0.1]}
    )
    yearly = pd.DataFrame(
        {"year": list(range(2018, 2026)), "beta_in_candle_5m": [-0.2] * 7 + [0.1]}
    )
    bootstrap = {"ci95_upper": -0.05}
...
def test_gate_logic_fails_when_bootstrap_crosses_zero():
    audit = {
        "years": list(range(2018, 2026)),
        "max_abs_identity_error": 0.0,
        "negative_excursion_count": 0,
    ...
    yearly = pd.DataFrame(
        {"year": list(range(2018, 2026)), "beta_in_candle_5m": [-0.2] * 8}
    )
    bootstrap = {"ci95_upper": 0.01}
    gates = evaluate_gates(audit, regressions, yearly, bootstrap)
    assert not gates["G3_day_block_ci_upper_negative"]

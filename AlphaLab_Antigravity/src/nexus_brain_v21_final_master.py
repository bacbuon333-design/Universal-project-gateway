"""
NEXUS BRAIN v21 — FINAL MASTER QUANT ENGINE
=============================================
Combining the 2 strongest empirical discoveries from all tests:
1. QUANT EXPANSION FILTER (from v18):
   - Volatility Z-Score > 1.0 (Surge)
   - Path Efficiency SNR > 0.22
   - Shannon Entropy < 0.82
   - Donchian(20) High Breakout
2. MACRO REGIME GATE (from v11/v15):
   - Macro Bull Gate: H1 EMA500 Slope > 0 (H1 Close > H1 EMA500 & EMA500 > EMA500[120])
   - Filters out all counter-trend fake breakouts during Bear regimes (May-Nov 2022).
3. ADAPTIVE TRAILING STOP:
   - Activates at 1.4x Risk, trails at 2.4x ATR.
"""
...
def compute_v21_metrics(close: np.ndarray, high: np.ndarray, low: np.ndarray):
    n = len(close)
    ...
    # EMAs
    e200 = pd.Series(close).ewm(span=200, adjust=False).mean().values
    e500 = pd.Series(close).ewm(span=500, adjust=False).mean().values
    e500_p = np.zeros(n)
...
def gen_v21_signals(c: np.ndarray, h: np.ndarray, l: np.ndarray, o: np.ndarray, nh: int, cfg: Dict[str, Any]):
    ind = compute_v21_metrics(c, h, l)
    entropy = ind['entropy']
    snr = ind['snr']
    ...
    for i in range(500, nh):
        ...
        # MACRO BULL GATE
        macro_bull = (c[i] > e500[i]) and (e500[i] > e500_p[i]) and (c[i] > e200[i])

        if not macro_bull:
            continue

        # QUANT EXPANSION SIGNAL
        if vz > 0.9 and snr_val > 0.20 and e_val < 0.83:
            if c[i] > dc20_hi[i]:
                conv = 7.0 + vz
                ...
                slp[i] = sl_dist
                rrv[i] = rr
                sname[i] = 'MASTER_QUANT_EXPANSION_BUY'
...
def run_v21_gauntlet(cfg: Dict[str, Any], risk: float = 0.055):
    ...
    sig, slp, rrv, sname, conviction, ind = gen_v21_signals(h1c, h1h, h1l, h1o, nh1, cfg)
    atr14 = ind['atr']
    rows = []
...
def main():
    print("=" * 75)
    print("🧠 NEXUS BRAIN v21 — FINAL MASTER QUANT ENGINE")
    print("=" * 75)
    ...
    for r in [0.045, 0.055, 0.065]:
        for cd in [18, 24, 36, 48]:
            cfg = {'cooldown': cd}
            rows = run_v21_gauntlet(cfg, risk=r)
    ...
    if best_rows is None:
        best_rows = run_v21_gauntlet({'cooldown': 24}, risk=0.055)

    print("\n" + "=" * 75)
    print(f"🏆 BEST V21 MASTER QUANT ENGINE REPORT")
    print("=" * 75)
    ...
    print("\n📊 Master Quant Expansion Breakdown:")
    ...
    rf = os.path.join(rdir, 'NEXUS_BRAIN_V21_MASTER_QUANT.md')

    lines = [
        "# 🧠 NEXUS BRAIN v21 — FINAL MASTER QUANT ENGINE REPORT",
        f"**Timestamp**: {datetime.now(timezone.utc).isoformat()} UTC",
        "**Core Math**: Volatility Z-Score + Path Efficiency SNR + Shannon Entropy + H1 EMA500 Macro Gate",
        "**Execution**: Adaptive Risk Kelly Sizing & 2.4x ATR Wave Trailing Stop",

"""
NEXUS BRAIN v26 — LIQUIDITY TRAP & EXPANSION QUANT ENGINE
=========================================================
Empirical Breakthrough:
- Shorting Gold (SELL) fails continuously (PF 0.318) because Gold pullbacks are violent liquidity traps.
- Instead of shorting the dip (which loses), v26 BUYS THE LIQUIDITY TRAP (Bear Trap Reversal)!

Architecture:
1. `LONG_TREND_EXPANSION`: Captures macro bull breakouts and wave accelerations.
2. `LONG_LIQUIDITY_FLUSH`: Captures extreme bear traps / liquidity flushes (RSI < 32 + Donchian Low Sweep + Bullish Rejection Wick).
3. Cooldown: 72 H1 bars (~3 trading days) per trade.
4. Risk Management: Dynamic Kelly Sizing with Drawdown Protection Floor (<20% Max DD).
"""
...
def compute_v26_indicators(h1c, h1h, h1l, h1o, nh1):
    e8 = ema(h1c, 8)
    e21 = ema(h1c, 21)
    e50 = ema(h1c, 50)
    e200 = ema(h1c, 200)
...
def is_pin_bull(o, h, l, c, atr):
    b = abs(c - o) + 1e-9
    lw = min(o, c) - l
    return (lw >= 1.8 * b and lw >= 0.45 * (h - l + 1e-9) and (c - l) / (h - l + 1e-9) >= 0.45 and b > 0.01 * atr)

def is_bull_engulf(o1, c1, o2, c2):
    return (c1 < o1 and c2 > o2 and c2 >= (o1 - abs(o1 - c1) * 0.15))

def gen_v26_signals(h1c, h1h, h1l, h1o, nh1, cfg):
    ind = compute_v26_indicators(h1c, h1h, h1l, h1o, nh1)
    ...
    for i in range(250, nh1):
        ...
        if macro_bull:
            ...
            if parabolic and dc20_hi[i] > 0 and h1c[i] > dc20_hi[i] and h1c[i-1] <= dc20_hi[i-1] and dv >= 28 and div > 8 and rv <= 72:
                ...
                rrv[i] = rr
                sname[i] = 'PARABOLIC_WAVE_EXPANSION'
                conviction[i] = 8
                last_bar = i
                continue
            ...
            if dv >= 16 and div > 0:
                ...
                if sc >= min_score and rv <= 68:
                    ...
                    rrv[i] = rr
                    sname[i] = 'STRUCTURAL_PULLBACK_BUY'
                    conviction[i] = sc
                    last_bar = i
                    continue

        # ── 2. LIQUIDITY TRAP REVERSAL (BUY THE BEAR FLUSH) ──────────────────────
        # When price flushes into Donchian Low + RSI < 35 (Oversold Bear Trap), BUY the reversal!
        liquidity_flush = (rv <= 35.0 and lo <= dc20_lo[i] and cv > dc20_lo[i])
        if liquidity_flush and i >= 1:
            o1, c1 = h1o[i-1], h1c[i-1]
            if is_pin_bull(oi, hi, lo, cv, av) or is_bull_engulf(o1, c1, oi, cv) or cv > oi:
                sig[i] = 1
                slp[i] = (av / PIP) * 1.2 + SPR
                rrv[i] = 3.0
                sname[i] = 'LIQUIDITY_TRAP_BUY'
                conviction[i] = 7
                last_bar = i
...
def run_v26_gauntlet(cfg: Dict[str, Any], risk: float = 0.055):
    ...
    sig, slp, rrv, sname, conviction, ind = gen_v26_signals(h1c, h1h, h1l, h1o, nh1, cfg)
    atr14 = ind['atr14']
    rows = []
...
def main():
    print("=" * 80)
    print("🧠 NEXUS BRAIN v26 — LIQUIDITY TRAP & EXPANSION QUANT ENGINE")
    print("=" * 80)

    cfg = {'cooldown': 72, 'min_score': 4, 'pivot_lb': 100, 'zone_atr': 0.8}
    rows = run_v26_gauntlet(cfg, risk=0.055)
    ...
    print("\n📊 Liquidity Trap Signal Breakdown:")
    ...
    rf = os.path.join(rdir, 'NEXUS_BRAIN_V26_LIQUIDITY_TRAP.md')

    lines = [
        "# 🧠 NEXUS BRAIN v26 — LIQUIDITY TRAP & EXPANSION QUANT ENGINE REPORT",
        f"**Timestamp**: {datetime.now(timezone.utc).isoformat()} UTC",
        "**Architecture**: Trend Expansion + Liquidity Trap Reversal (Buying Bear Flushes)",
    ...
    with open(rf, 'w', encoding='utf-8') as f:
        f.write('\n'.join(lines))
    print(f"\n📄 Saved Liquidity Trap Report to: {rf}")

"""
NEXUS BRAIN v23 — DUAL-SIDE STRUCTURAL QUANT ENGINE (BUY & SELL)
=================================================================
Architecture:
- High-precision structural execution for BOTH Buy and Sell directions.
- BUY: Structural Pivot Support Retest / EMA21 Pullback with Bullish Pin Bar or Engulfing.
- SELL: Structural Pivot Resistance Retest / Overextended Donchian High Liquidity Sweep
        with Bearish Pin Bar or Engulfing Rejection.
- Controlled Cooldown: 48 H1 bars (~2 trading days) per trade to eliminate overtrading.
- Adaptive Risk & Trailing Stop: 2.4x ATR trailing stop with drawdown protection floor (<20% Max DD).
"""
...
def compute_v23_indicators(h1c, h1h, h1l, h1o, nh1):
    e8 = ema(h1c, 8)
    e21 = ema(h1c, 21)
    e50 = ema(h1c, 50)
    e200 = ema(h1c, 200)
    ...
    for i in range(480, nh1):
        mom1m[i] = h1c[i] > h1c[i - 480]

    # Donchian 20 & 50 High/Low
    ...
    for i in range(120, nh1, 4):
        chunk_r = log_ret[max(0, i - 48):i]
        entropy[i:min(i+4, nh1)] = calculate_shannon_entropy(chunk_r, bins=8)

    return dict(
        e8=e8, e21=e21, e50=e50, e200=e200,
        atr14=atr14, atr_slow=atr_slow, rsi=rsi, adx=adx, di=di,
        vol_zscore=vol_zscore, entropy=entropy,
        mom3m=mom3m, mom1m=mom1m,
        dc20_hi=dc20_hi, dc20_lo=dc20_lo, dc50_hi=dc50_hi,
...
def gen_v23_dualside_signals(h1c, h1h, h1l, h1o, nh1, cfg):
    ind = compute_v23_indicators(h1c, h1h, h1l, h1o, nh1)
    ...
    mom3m = ind['mom3m']; mom1m = ind['mom1m']
    dc20_hi = ind['dc20_hi']; dc50_hi = ind['dc50_hi']
    ...
    for i in range(250, nh1):
        ...
        as_ = max(atr_s[i], 1.5); ar = av / as_; vz = vol_z[i]

        macro_bull = mom3m[i] and cv > e200[i]
        macro_bear = (not mom3m[i]) and cv < e200[i]

        # ── 1. BUY EXECUTION (High Conviction Structural Dip / Expansion) ────────
        if macro_bull:
        ...
        # ── 2. SELL EXECUTION (High Conviction Structural Rejection / Sweep) ─────
        # Conditions for Sell: Either Macro Bear OR Structural Overextended Distribution Top
        overextended_top = (rv >= 68.0 and vz >= 0.8 and hi >= dc50_hi[i])
        
        if macro_bear or overextended_top:
            if dv >= 15 and div < 5:
                ...
                if dv >= 20 and div < -5: sc_sell += 1
                if at_resist or overextended_top: sc_sell += 2
                ...
                if sc_sell >= min_score and rv >= 35:
...
def run_v23_gauntlet(cfg: Dict[str, Any], risk: float = 0.055):
    ...
    sig, slp, rrv, sname, conviction, ind = gen_v23_dualside_signals(h1c, h1h, h1l, h1o, nh1, cfg)
    atr14 = ind['atr14']
    rows = []
...
def main():
    print("=" * 80)
    print("🧠 NEXUS BRAIN v23 — DUAL-SIDE STRUCTURAL QUANT ENGINE (BUY & SELL)")
    print("=" * 80)

    cfg = {'cooldown': 48, 'min_score': 4, 'pivot_lb': 100, 'zone_atr': 0.8}
    rows = run_v23_gauntlet(cfg, risk=0.055)
    ...
    print("\n📊 Dual-Side Structural Signal Performance Breakdown:")
    ...
    rf = os.path.join(rdir, 'NEXUS_BRAIN_V23_DUALSIDE_FINAL.md')

    lines = [
        "# 🧠 NEXUS BRAIN v23 — DUAL-SIDE STRUCTURAL QUANT ENGINE REPORT",
        f"**Timestamp**: {datetime.now(timezone.utc).isoformat()} UTC",
        "**Architecture**: High Precision Bidirectional Structural Engine (BUY & SELL)",
        "**BUY Engine**: Structural Pivot Retest + H1 Donchian Expansion Breakout",
        "**SELL Engine**: Macro Bear Trend & Overextended Distribution Top Liquidity Sweep",
    ...
    with open(rf, 'w', encoding='utf-8') as f:
        f.write('\n'.join(lines))
    print(f"\n📄 Saved Dual-Side Final Report to: {rf}")

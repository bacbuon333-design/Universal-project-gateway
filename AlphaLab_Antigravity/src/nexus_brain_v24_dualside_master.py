"""
NEXUS BRAIN v24 — HIGH-PRECISION DUAL-SIDE QUANT ENGINE
========================================================
Refining Bidirectional BUY & SELL execution for Gold:

1. BUY ENGINE:
   - Macro Bull Alignment (3-Month Momentum / EMA200).
   - Structural Pivot Support Retest or EMA21 Dip with Pin Bar / Engulfing.
   - Parabolic Breakout when Volatility expands.

2. SELL ENGINE (PRECISION EXHAUSTION & BEAR REGIME ONLY):
   - Macro Bear Alignment (H1 Close < EMA200 & 3-Month Bear Momentum).
   - Distribution Top Exhaustion: Active ONLY when ADX drops below 25 (trend dying)
     + Market Structure Shift (H1 Close < EMA21) + Bearish Rejection Wick.
   - Cooldown: 72 H1 bars (~3 trading days) per side to enforce zero overtrading.
"""
...
def compute_v24_indicators(h1c, h1h, h1l, h1o, nh1):
    e8 = ema(h1c, 8)
    e21 = ema(h1c, 21)
    e50 = ema(h1c, 50)
    e200 = ema(h1c, 200)
    ...
    return dict(
        ...
        dc20_hi=dc20_hi, dc50_hi=dc50_hi,
...
def gen_v24_dualside_signals(h1c, h1h, h1l, h1o, nh1, cfg):
    ind = compute_v24_indicators(h1c, h1h, h1l, h1o, nh1)
    ...
    for i in range(250, nh1):
        ...
        # ── 2. SELL EXECUTION (High Conviction Exhaustion Rejection & Bear Regime)
        # Sell is active ONLY during:
        # A) Macro Bear Regime (e.g. May-Nov 2022)
        # B) True Trend Exhaustion Top (ADX < 24 + Structure Shift + Bearish Pin/Engulfing)
        trend_exhaustion = (dv < 24 and div < 0 and rv >= 65.0 and h1c[i] < e21[i] and hi >= dc50_hi[i])

        if macro_bear or trend_exhaustion:
            ...
            if div < -5: sc_sell += 1
            if at_resist or trend_exhaustion: sc_sell += 2
            ...
            if sc_sell >= min_score and rv >= 40:
...
def run_v24_gauntlet(cfg: Dict[str, Any], risk: float = 0.055):
    ...
    sig, slp, rrv, sname, conviction, ind = gen_v24_dualside_signals(h1c, h1h, h1l, h1o, nh1, cfg)
    atr14 = ind['atr14']
    rows = []
...
def main():
    print("=" * 80)
    print("🧠 NEXUS BRAIN v24 — HIGH-PRECISION DUAL-SIDE QUANT ENGINE (BUY & SELL)")
    print("=" * 80)

    cfg = {'cooldown': 72, 'min_score': 4, 'pivot_lb': 100, 'zone_atr': 0.8}
    rows = run_v24_gauntlet(cfg, risk=0.055)
    ...
    print("\n📊 High-Precision Dual-Side Performance Breakdown:")
    ...
    rf = os.path.join(rdir, 'NEXUS_BRAIN_V24_DUALSIDE_MASTER.md')

    lines = [
        "# 🧠 NEXUS BRAIN v24 — HIGH-PRECISION DUAL-SIDE QUANT ENGINE REPORT",
        f"**Timestamp**: {datetime.now(timezone.utc).isoformat()} UTC",
        "**Architecture**: High-Precision Bidirectional Execution (BUY & SELL)",
        "**BUY Engine**: Structural Pivot Retest + Parabolic Wave Expansion Breakout",
        "**SELL Engine**: Macro Bear Trend & Exhaustion Top Structure Shift Rejection",
        "**Selectivity**: 72 H1 Bar Cooldown (~3 Trading Days per Trade)",
        "**Risk Control**: Dynamic Kelly Sizing with Drawdown Protection Floor (<20% Max DD)",
    ...
    with open(rf, 'w', encoding='utf-8') as f:
        f.write('\n'.join(lines))
    print(f"\n📄 Saved High-Precision Dual-Side Master Report to: {rf}")

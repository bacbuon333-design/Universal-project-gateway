"""
NEXUS BRAIN v17 — QUANT VOLATILITY-ENTROPY EXPANSION ENGINE
============================================================
Key Discovery from v16 Quantitative Math Audit:
  - Mean-Reversion (`MATH_MEAN_REV_BUY`) fails on Gold (PF 0.583).
  - Volatility-Entropy Expansion (`MATH_VOL_BREAK_BUY`) achieved:
    --> Profit Factor = 2.212
    --> Win Rate = 57.1%
    --> Net PnL = +$408.50 on minimal trades.

Architecture of v17 (Pure Wave Expansion Engine):
1. Detect Entropy Squeeze (Low Shannon Entropy = Market Energy Compression)
2. Detect Volatility Z-Score Explosion (Z_vol > 1.0)
3. Detect Trend Persistence (Hurst Exponent H > 0.52 & SNR > 0.22)
4. Execute Directional Breakout & Trail with Adaptive ATR Stop
5. Zero Mean-Reversion, Zero Static Pullbacks
"""
...
def calculate_hurst(ts: np.ndarray, max_lag: int = 16) -> float:
    if len(ts) < max_lag * 2:
        return 0.5
...
def compute_quant_expansion_metrics(close: np.ndarray, high: np.ndarray, low: np.ndarray):
    n = len(close)
    ...
    # Rolling Donchian High/Low (20-period H1)
    dc20_hi = pd.Series(high).rolling(20).max().shift(1).values
    dc20_lo = pd.Series(low).rolling(20).min().shift(1).values

    hurst = np.full(n, 0.5)
    entropy = np.full(n, 1.0)

    for i in range(120, n, 4):
        chunk_c = close[max(0, i - 120):i]
        chunk_r = log_ret[max(0, i - 48):i]
        h_val = calculate_hurst(chunk_c, max_lag=14)
        e_val = calculate_shannon_entropy(chunk_r, bins=8)
        hurst[i:min(i+4, n)] = h_val
        entropy[i:min(i+4, n)] = e_val
...
def gen_v17_signals(c: np.ndarray, h: np.ndarray, l: np.ndarray, o: np.ndarray, nh: int, cfg: Dict[str, Any]):
    ind = compute_quant_expansion_metrics(c, h, l)
    hurst = ind['hurst']
    entropy = ind['entropy']
    snr = ind['snr']
    ...
    # Moving averages for macro trend gate
    ...
    for i in range(200, nh):
        ...
        # ── 1. VOLATILITY-ENTROPY BREAKOUT ENGINE ────────────────────────────────
        # Criteria: High Vol Z-Score + Low Entropy + High SNR + Break of Donchian High
        if vz > 0.8 and snr_val > 0.20 and e_val < 0.82:
            if c[i] > e200[i] and c[i] > dc20_hi[i]:
                conv = 6.0 + vz * 1.5 + snr_val * 5.0
                sl_dist = (av / PIP) * 1.35 + SPR
                rr = 3.5 + min(2.0, av_ratio * 0.8)

                sig[i] = 1
                slp[i] = sl_dist
                rrv[i] = rr
                sname[i] = 'QUANT_VOL_EXPANSION_BUY'
                conviction[i] = min(conv, 10.0)
                last_trade_bar = i
                continue

        # ── 2. PERSISTENT TREND MOMENTUM ACCELERATION ────────────────────────────
        # Criteria: Hurst > 0.54 (Strong trend persistence) + SNR > 0.25
        elif h_val > 0.54 and snr_val > 0.25 and e_val < 0.78:
            if c[i] > e200[i] and c[i] > dc20_hi[i]:
                conv = 7.0 + (h_val - 0.50) * 10.0
                sl_dist = (av / PIP) * 1.45 + SPR
                rr = 4.0

                sig[i] = 1
                slp[i] = sl_dist
                rrv[i] = rr
                sname[i] = 'QUANT_TREND_PERSISTENCE_BUY'
                conviction[i] = min(conv, 10.0)
                last_trade_bar = i
                continue
...
def eval_window(h1c, h1h, h1l, atr14, sig, slp, rrv, sname, conviction, start_i, end_i, risk=0.055):
    ...
    for i in range(start_i, end_i):
        if pos:
            ...
            # Activate adaptive trail after 1.5x Risk distance
...
def run_v17_gauntlet(cfg: Dict[str, Any], risk: float = 0.055):
    ...
    sig, slp, rrv, sname, conviction, ind = gen_v17_signals(h1c, h1h, h1l, h1o, nh1, cfg)
    atr14 = ind['atr']
    rows = []
...
def main():
    print("=" * 75)
    print("🧠 NEXUS BRAIN v17 — QUANT VOLATILITY-ENTROPY EXPANSION ENGINE")
    print("=" * 75)
    ...
    for r in [0.045, 0.055, 0.065]:
        for cd in [24, 36, 48, 60]:
            cfg = {'cooldown': cd}
            rows = run_v17_gauntlet(cfg, risk=r)
            ...
            if pos_years >= 3 and max_dd <= 22.0 and tot_pnl > best_pnl:
                best_pnl = tot_pnl
                best_config = (r, cd)
                best_rows = rows

    if best_rows is None:
        best_rows = run_v17_gauntlet({'cooldown': 36}, risk=0.055)

    print("\n" + "=" * 75)
    print(f"🏆 BEST V17 QUANT ENGINE REPORT")
    print("=" * 75)
    ...
    rf = os.path.join(rdir, 'NEXUS_BRAIN_V17_QUANT_EXPANSION.md')

    lines = [
        "# 🧠 NEXUS BRAIN v17 — QUANT VOLATILITY-ENTROPY EXPANSION ENGINE REPORT",
        f"**Timestamp**: {datetime.now(timezone.utc).isoformat()} UTC",
        "**Core Math**: Hurst Trend Persistence + Shannon Entropy Squeeze + Volatility Z-Score Explosion",
        "**Execution**: High Probability Wave Expansion Breakout & Adaptive ATR Trailing Stop",

...
The module studies how much snapback-direction movement is realized inside the
breach candle versus after that candle closes.
"""
...
def add_reversion_budget_variables(
    ...
    """Attach event geometry and exact-clock reversion decomposition."""
    if frame.empty:
        return frame.copy()
...
def build_counterfactual_reversion_frame(
    ...
    """Build the full continuous breach-event frame without strategy semantics."""
...
def geometry_audit(frame: pd.DataFrame) -> Dict[str, Any]:
    ...
    return {
        ...
        "years": years,
        "max_abs_identity_error": float(np.nanmax(err)),
...
def build_fixed_reclaim_bins(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    for h in HORIZONS_MINUTES:
        for bucket in RECLAIM_LABELS:
            sub = frame[
                (frame["in_candle_reversion_bin"] == bucket)
                & frame[f"exact_{h}m_available"].astype(bool)
            ]
        ...
        rows.append(
            {
                ...
                "energy_count": int(energy),
                "location_count": int(location),
                "n": int(len(sub)),
...
def _control_matrix(frame: pd.DataFrame, include_year_effects: bool = True) -> Tuple[np.ndarray, List[str]]:
    """Frozen nuisance-control matrix; intentionally excludes event_class."""
    n = len(frame)
    ...
    names = [
        "intercept",
        "breach_excursion_atr",
        "atr_percentile_scaled",
        "path_efficiency",
        "stretch_abs",
        "location_count",
        "side_LONG",
...
def _valid_regression_frame(frame: pd.DataFrame, horizon: int) -> pd.DataFrame:
    needed = [
        "in_candle_reversion_atr",
        f"post_close_reversion_atr_{horizon}m",
        f"exact_{horizon}m_available",
        "breach_excursion_atr",
        "atr_percentile",
        "path_efficiency",
        "stretch_abs",
...
def fit_compensation_regression(
    frame: pd.DataFrame,
    horizon: int,
    *,
    ...
    """FWL estimate of the in-candle coefficient under the frozen control set."""
    sub = _valid_regression_frame(frame, horizon)
    if len(sub) < 50:
        return {"horizon_min": int(horizon), "n": int(len(sub)), "beta_in_candle": np.nan}

    x = sub["in_candle_reversion_atr"].to_numpy(dtype=float)
    y = sub[f"post_close_reversion_atr_{horizon}m"].to_numpy(dtype=float)
    ...
    beta = np.nan if den <= 1e-15 else float(np.dot(rx, ry) / den)
    ...
    result: Dict[str, Any] = {
        ...
        "r_in_mean": float(np.mean(x)),
        "r_post_mean": float(np.mean(y)),
    }
    if return_residuals:
        result["_rx"] = rx
        result["_ry"] = ry
...
def build_regression_table(frame: pd.DataFrame) -> pd.DataFrame:
    rows = []
    for h in HORIZONS_MINUTES:
        r = fit_compensation_regression(frame, h, include_year_effects=True)
...
def build_yearly_5m_slopes(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    for year in range(2018, 2026):
        ...
        r = fit_compensation_regression(sub, PRIMARY_HORIZON_MINUTES, include_year_effects=False)
        rows.append(
            {
                "year": int(year),
                "n": int(r.get("n", 0)),
                "beta_in_candle_5m": float(r.get("beta_in_candle", np.nan)),
            }
        )
    return pd.DataFrame(rows)
...
def fixed_residual_day_block_bootstrap(
    ...
    *,
    ...
    """UTC-day bootstrap of FWL residual cross-products with nuisance fit held fixed."""
    fit = fit_compensation_regression(
        frame,
        PRIMARY_HORIZON_MINUTES,
        include_year_effects=True,
    ...
    work = pd.DataFrame(
        {
            "utc_date": pd.DatetimeIndex(dts).date,
            "xy": rx * ry,
            "xx": rx * rx,
        }
    )
    daily = work.groupby("utc_date", sort=True)[["xy", "xx"]].sum()
    stats = daily.to_numpy(dtype=float)
    n_days = len(stats)
    ...
    return {
        ...
        "seed": int(seed),
        "unique_utc_days": int(n_days),
        "valid_iterations": int(len(arr)),
        "point_beta": float(fit.get("beta_in_candle", np.nan)),
        "mean_beta": float(np.mean(arr)) if len(arr) else np.nan,
...
def evaluate_gates(
    ...
    beta = {
        int(r["horizon_min"]): float(r["beta_in_candle"])
        for _, r in regressions.iterrows()
        if np.isfinite(float(r["beta_in_candle"]))
    }
    yearly_beta = yearly["beta_in_candle_5m"].to_numpy(dtype=float)
    valid_years = int(np.isfinite(yearly_beta).sum())
    negative_years = int(np.sum(np.isfinite(yearly_beta) & (yearly_beta < 0.0)))

    g0 = years == set(range(2018, 2026))
    g1 = (
        float(audit.get("max_abs_identity_error", np.inf)) <= IDENTITY_TOLERANCE
    ...
    g2 = bool(beta.get(5, np.nan) < 0.0)
    g3 = bool(float(bootstrap.get("ci95_upper", np.nan)) < 0.0)
    g4 = bool(beta.get(3, np.nan) < 0.0 and beta.get(10, np.nan) < 0.0)
    g5 = bool(valid_years == 8 and negative_years >= MIN_NEGATIVE_YEARS)
    passed = all((g0, g1, g2, g3, g4, g5))
    return {
        "G0_dataset_and_years": g0,
        "G1_geometry_and_exact_clock": g1,
        "G2_primary_beta_5m_negative": g2,
        "G3_day_block_ci_upper_negative": g3,
        "G4_3m_and_10m_negative": g4,
        ...
        "verdict": (
            ...
            if passed
            else "REVERSION_COMPENSATION_NOT_CONFIRMED"
        ),
    }

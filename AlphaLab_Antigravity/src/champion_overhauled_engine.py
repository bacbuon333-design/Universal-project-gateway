"""
===================================================================
CHAMPION OVERHAULED QUANT ENGINE (PRUNED & OVERHAULED)
===================================================================
Overhauls entry execution architecture by pruning failing strategies (S7, S12, S14)
and concentrating capital exclusively on High-Expectancy Champions:

1. S10_CONFLUENCE_BUY: HTF EMA 200 Uptrend + EMA 8/21 Alignment + RSI Pullback (<= 38) + ADX >= 22.
2. S3_BOS_MOMENTUM_BUY: Donchian 20-bar High Breakout + Volatility Expansion (ATR14/ATR100 >= 1.20).
...
class AdaptiveRiskManager:
    @staticmethod
    def calc_lot(equity: float, sl_pts: float, risk_pct: float = 0.01) -> float:
...
class ChampionOverhauledEngine:
    ...
    def generate_overhauled_signals(self, df: pd.DataFrame):
        ...
        # 5. Donchian High (20-bar lookback)
        ...
        for i in range(201, n):
            ...
            # --- CHAMPION 1: S10_CONFLUENCE_BUY (HIGH-EXPECTANCY PULLBACK) ---
            ...
            # --- CHAMPION 2: S3_BOS_MOMENTUM_BUY (VOLATILITY BREAKOUT) ---
            if c_val > ema_200[i] and c_val > high_20[i] and vol_ratio >= 1.20 and adx_val >= 20:
                signals[i] = 1 # BUY
                sl_pts[i] = (a_val / PIP_SIZE) * 1.4 + (self.spread_pts * 1.2)
                rr_arr[i] = 4.0
                strat_names[i] = "S3_BOS_MOMENTUM_BUY"
                continue
    ...
    def run_backtest(self, df: pd.DataFrame, stress_multiplier: float = 1.0) -> Dict:
        ...
        signals, sl_pts_arr, rr_arr, strat_names = self.generate_overhauled_signals(df)
        ...
        for i in range(201, n):
            if open_trade is not None:
                ...
                if t.direction == "BUY":
                    # Breakeven Lock: If profit reaches 1.8x SL, move SL to Entry + Spread
                    if not t.be_locked and hi >= t.entry + (t.sl_pts * PIP_SIZE * 1.8):
                        t.sl = t.entry + (eff_spread_pts * PIP_SIZE)
                        t.be_locked = True
            ...
            if open_trade is None and signals[i] != 0:
                ...
                if sp > 0 and rr_target > 0:
                    lot = AdaptiveRiskManager.calc_lot(equity, sp, self.risk_pct)
                    c_val = close[i]
...
def run_champion_overhauled_validation():
    print("==========================================================")
    print("🚀 EXECUTING CHAMPION OVERHAULED ENGINE VALIDATION")
    print("==========================================================")
    ...
    engine = ChampionOverhauledEngine(initial_balance=1000.0, risk_pct=0.01, spread_pts=25.0)
    ...
    if os.path.exists(m15_csv):
        ...
        print(f"   Final Balance   : ${res_1y['final_balance']:.2f}")
        ...
        print(f"   PROFIT FACTOR   : {res_1y['profit_factor']:.3f} {'🏆 PASSED (PF >= 1.50)' if res_1y['profit_factor'] >= 1.50 else '❌'}")
        print(f"   Max Drawdown    : {res_1y['max_dd_pct']:.2f}% (${res_1y['max_dd_usd']:.2f})")
        print(f"   Sharpe Ratio    : {res_1y['sharpe_ratio']:.2f}")

        # 2. FULL 4-YEAR M15 RETEST (2022 -> 2026)
        print(f"\n📊 2. FULL 4-YEAR GOLD M15 RETEST ({len(df_m15):,} bars | 2022-05-02 -> 2026-07-24):")
    ...
    # 3. 25-YEAR GOLD H1 RETEST (2001 -> 2026)
    if os.path.exists(h1_csv):
        ...
        print(f"\n📊 3. 25-YEAR GOLD H1 RETEST ({len(df_h1):,} bars | {df_h1.index[0].strftime('%Y-%m-%d')} -> {df_h1.index[-1].strftime('%Y-%m-%d')}):")
    ...
    report_file = os.path.join(reports_dir, "CHAMPION_OVERHAULED_REPORT.md")
    
    lines = []
    lines.append("# 🏆 CHAMPION OVERHAULED QUANT ENGINE REPORT")
    lines.append(f"**Timestamp**: {datetime.now(timezone.utc).isoformat()} UTC")
    lines.append("**Core Architecture**: Pruned S7/S12/S14 + Concentrated S10_Confluence & S3_BOS_Momentum")
    lines.append("\n---")
    lines.append("\n## 📊 Summary Results")
    ...
    lines.append("| :--- | :---: | :---: | :---: | :---: | :---: | :---: | :---: | :---: |")
    lines.append(f"| **Strict 1-Year (2024-2025)** | $1,000.00 | **${res_1y['final_balance']:.2f}** | **${res_1y['total_pnl']:+.2f}** | **{res_1y['profit_factor']:.3f}** | {res_1y['max_dd_pct']:.2f}% | {res_1y['total_trades']} | {res_1y['win_rate']:.1f}% | {'🏆 **PASSED**' if res_1y['profit_factor'] >= 1.50 else '✅ Stable'} |")
    ...
    print(f"\n📄 Saved Champion Overhauled Report to: {report_file}")

if __name__ == "__main__":
    run_champion_overhauled_validation()

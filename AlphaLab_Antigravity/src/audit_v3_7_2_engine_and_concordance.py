"""V3.7.2 engine-interface and legacy/canonical concordance audit.
...
from canonical_research_engine import (
    ROOT,
    CANONICAL_CSV,
    FROZEN_SHA256,
    canonical_engine_compatibility_snapshot,
    load_authorized_canonical_dataframe,
)
...
def run_audit() -> Dict[str, Any]:
    canonical, auth = load_authorized_canonical_dataframe()
    snapshot = canonical_engine_compatibility_snapshot()
    ...
    concordance = {
        ...
        "canonical_sha256": auth["dataset_sha256"],
        ...
        "interpretation_guard": "Highest match view is descriptive only. It does not establish the original legacy exporter, timezone, or source lineage.",
    }
    ...
    compatibility_checks = {
        ...
        "first_timestamp_correct": snapshot["first_datetime"].startswith("2018-02-22T18:30:00"),
        "last_timestamp_correct": snapshot["last_datetime"].startswith("2026-08-14T23:30:00"),
    ...
    status = "CANONICAL_ENGINE_COMPATIBILITY_PASS" if all(compatibility_checks.values()) else "CANONICAL_ENGINE_COMPATIBILITY_BLOCKED"
    ...
    (OUT_DIR / "canonical_engine_snapshot.json").write_text(json.dumps(snapshot, indent=2) + "\n", encoding="utf-8")
    (OUT_DIR / "legacy_canonical_concordance.json").write_text(json.dumps(concordance, indent=2) + "\n", encoding="utf-8")
    (OUT_DIR / "V3_7_2_ENGINE_COMPATIBILITY_DECISION.json").write_text(json.dumps(decision, indent=2) + "\n", encoding="utf-8")

"""
NEXUS BRAIN v20 — ULTIMATE QUANT EXPANSION ENGINE (FINAL SOLUTION)
==================================================================
Empirical Learnings Synthesized:
1. Gold MUST HAVE ROOM TO BREATHE: Trailing distance = 2.6x ATR (activation at 1.5x Risk).
2. Tight trailing stops (v19 at 1.0x/2.0x ATR) get choked by market noise.
3. Overbought Protection: Block entries if H1 RSI > 68 (prevents buying micro-tops).
4. High-Precision Quant Expansion Filter (from v18):
   - Volatility Z-Score > 1.2
   - Signal-to-Noise Ratio (SNR) > 0.25
   - Shannon Entropy < 0.80
   - Macro Trend Alignment: H1 Close > H1 EMA200 & Donchian(20) High Breakout
"""
...
def compute_v20_metrics(close: np.ndarray, high: np.ndarray, low: np.ndarray):
    n = len(close)
...
def gen_v20_signals(c: np.ndarray, h: np.ndarray, l: np.ndarray, o: np.ndarray, nh: int, cfg: Dict[str, Any]):
    ind = compute_v20_metrics(c, h, l)
    entropy = ind['entropy']
    snr = ind['snr']
    ...
    for i in range(200, nh):
        ...
        # ── ULTIMATE QUANT EXPANSION FILTER ──────────────────────────────────
        if vz > 1.1 and snr_val > 0.24 and e_val < 0.81 and r_val < 68.0:
            if c[i] > e200[i] and c[i] > dc20_hi[i]:
                conv = 7.0 + vz
                sl_dist = (av / PIP) * 1.45 + SPR
                rr = 3.8 + max(0.0, av_ratio * 0.5)

                sig[i] = 1
                slp[i] = sl_dist
                rrv[i] = rr
                sname[i] = 'ULTIMATE_QUANT_EXPANSION_BUY'
...
def eval_window(h1c, h1h, h1l, atr14, sig, slp, rrv, sname, conviction, start_i, end_i, risk=0.055):
    ...
    for i in range(start_i, end_i):
        if pos:
            ...
            # Activate trailing stop after 1.5x Risk distance
            if not pos.trail_on and (h1h[i] - pos.entry) >= 1.5 * pos.sl_pts * PIP:
                pos.trail_on = True

            if pos.trail_on:
                # WIDE 2.6x ATR trailing distance allows wave expansion to breathe
                new_sl = pos.hwm - 2.6 * curr_atr
                if new_sl > pos.sl:
                    pos.sl = new_sl
...
def run_v20_gauntlet(cfg: Dict[str, Any], risk: float = 0.055):
    ...
    sig, slp, rrv, sname, conviction, ind = gen_v20_signals(h1c, h1h, h1l, h1o, nh1, cfg)
    atr14 = ind['atr']
    rows = []
...
def main():
    print("=" * 75)
    print("🧠 NEXUS BRAIN v20 — ULTIMATE QUANT EXPANSION ENGINE")
    print("=" * 75)
    ...
    for r in [0.045, 0.055, 0.065]:
        for cd in [18, 24, 36, 48]:
            cfg = {'cooldown': cd}
            rows = run_v20_gauntlet(cfg, risk=r)
    ...
    if best_rows is None:
        best_rows = run_v20_gauntlet({'cooldown': 24}, risk=0.055)

    print("\n" + "=" * 75)
    print(f"🏆 BEST V20 ULTIMATE QUANT ENGINE REPORT")
    print("=" * 75)
    ...
    print("\n📊 Quantitative Expansion Breakdown:")
    ...
    rf = os.path.join(rdir, 'NEXUS_BRAIN_V20_ULTIMATE_QUANT.md')

    lines = [
        "# 🧠 NEXUS BRAIN v20 — ULTIMATE QUANT EXPANSION ENGINE REPORT",
        f"**Timestamp**: {datetime.now(timezone.utc).isoformat()} UTC",
        "**Core Math**: Volatility Z-Score (>1.1) + Shannon Entropy (<0.81) + Path Efficiency SNR (>0.24)",
        "**Execution**: Overbought Guard (RSI<68) + Wide 2.6x ATR Wave Trailing Stop",
